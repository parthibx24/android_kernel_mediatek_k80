...
static ssize_t
spi_msg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	/* struct spi_transfer *xfer; */
	...
	if (!strncmp(buf, "-h", 2)) {
		...
		if (len == 0) {
			...
			int loop_count = 0;
			/* loop_count = 250, take about half a hour. */
			for (loop_count = 0; loop_count <= 5000; loop_count++) {
				/* msg_str = (struct spi_message *) kzalloc(SPI_STRESS_MAX *
