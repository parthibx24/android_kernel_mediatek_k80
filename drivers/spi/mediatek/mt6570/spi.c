...
#ifdef SPI_REC_DEBUG
#define SPI_CLOCK_PERIED 104000	/* kHz */
...
void mt_spi_workqueue_handler(void *data)
{
	...
	for (i = 0; i < SPI_REC_NUM; i++) {
		pr_alert("spi-rec%3d-%3d:%s\n", rec_count,
		       rec_count_tmp, msg_rec[rec_count_tmp]);
...
static int mt_spi_transfer(struct spi_device *spidev, struct spi_message *msg)
{
	...
	list_for_each_entry(xfer, &msg->transfers, transfer_list) {
		SPI_INFO(&spidev->dev, "xfer %p: len %04u tx %p/%08x rx %p/%08x\n", xfer,
			 xfer->len, xfer->tx_buf, xfer->tx_dma, xfer->rx_buf, xfer->rx_dma);
	}
#endif
...
#ifdef SPI_VERBOSE
	u32 speed;
#define SPI_MODULE_CLOCK 104000
...
#ifdef SPI_REC_DEBUG
	/* spi_speed = 104000/(chip_config->low_time + chip_config->high_time); */
#endif
...
static int mt_spi_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(pdev->dev.of_node, "mediatek,spi-padmacro", &pad_macro)) {
...
static int mt_spi_suspend(struct platform_device *pdev, pm_message_t message)
{
	...
	/* pr_alert("the pm status is:0x%x.\n", message.event); */
	return 0;
}
...
struct platform_driver mt_spi_driver = {
	.driver = {
		.name = "mt-spi",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(mt_spi_of_match),
	},
	.probe = mt_spi_probe,
