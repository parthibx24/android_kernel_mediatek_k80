...
int mt6356_get_auxadc_value(u8 channel)
{
	...
	auxadc_channel =
		&mt6356_auxadc_channel[channel-AUXADC_LIST_START];
...
void mt6356_auxadc_init(void)
{
	...
	PMICLOG("%s\n", __func__);
	wakeup_source_init(&pmic_auxadc_wake_lock, "MT6356 AuxADC wakelock");
	mutex_init(&pmic_adc_mutex);
	wakeup_source_init(&mdrt_wakelock, "MT6356 MTS Monitor wakelock");
