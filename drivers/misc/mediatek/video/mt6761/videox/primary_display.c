...
#if 0//ndef CONFIG_FPGA_EARLY_PORTING /* fpga has no vsync */
		if (islcmconnected)
			     ...
			     struct cmdqRecStruct *cmdq_handle)
{
	...
	if (primary_display_is_video_mode() &&
		!primary_display_is_decouple_mode()) {
		/* Cervino last OVL is DISP_MODULE_OVL0_2L */
...
int primary_display_force_set_vsync_fps(unsigned int fps, unsigned int scenario)
{
	...
	DISPMSG(
		"primary_display_force_set_vsync_fps scenario=%d, fps=%d\n",
		scenario, fps);
...
int primary_display_setbacklight(unsigned int level)
{
	...
	if (pgc->state == DISP_SLEPT) {
		DISPWARN("Sleep State set backlight invald\n");
	} else {
		primary_display_idlemgr_kick(__func__, 0);
		if (primary_display_cmdq_enabled()) {
			if (primary_display_is_video_mode()) {
