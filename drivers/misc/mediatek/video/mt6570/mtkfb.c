...
static int update_test_kthread(void *data)
{
	...
	for (;;) {
		if (kthread_should_stop())
			break;
		msleep(1000); /* 2s */
		pr_debug("mtkfb update test thread work,offset = %d\n", i);
	...
	pr_debug("mtkfb exit update_test_kthread()\n");
	return 0;
}
#endif
...
static int mtkfb_probe(struct device *dev)
{
	...
	DISPMSG("mtkfb MTK_FB_XRES=%d, MTKFB_YRES=%d, MTKFB_BPP=%d, MTK_FB_PAGES=%d, MTKFB_LINE=%d, MTKFB_SIZEV=%d\n",
	...
	DISPMSG("[FB Driver] mtkfb fbdev->fb_pa_base = %p, fbdev->fb_va_base = %p\n",
		  &(fbdev->fb_pa_base), fbdev->fb_va_base);

	if (!fbdev->fb_va_base) {
		DISPERR("mtkfb unable to allocate memory for frame buffer\n");
		r = -ENOMEM;
		goto cleanup;
	}
	init_state++; /* 2 */
