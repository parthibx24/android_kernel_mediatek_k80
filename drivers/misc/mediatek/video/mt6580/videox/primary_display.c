...
static struct disp_internal_buffer_info *allocate_decouple_buffer(int size)
{
	void *buffer_va = NULL;
...
static int _DC_switch_to_DL_fast(void)
{
	...
	struct disp_ddp_path_config *data_config_dl = NULL;
	struct disp_ddp_path_config *data_config_dc = NULL;
	struct OVL_CONFIG_STRUCT ovl_config[4];
	enum DDP_SCENARIO_ENUM old_scenario, new_scenario;
			   ...
			   fence_release_callback callback, unsigned int data)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->ovl_trigger, MMPROFILE_FLAG_PULSE, 0,
		       data);
	return 0;
}
...
int primary_display_esd_check(void)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->esd_check_t, MMPROFILE_FLAG_START, 0,
		       0);
	...
	if (primary_display_cmdq_enabled()) {
		...
		mmprofile_log_ex(ddp_mmp_get_events()->esd_rdlcm, MMPROFILE_FLAG_PULSE, primary_display_is_video_mode(), 3);
		if (ret == 1) {
			/* cmdq fail */
			if (_need_wait_esd_eof()) {
				/* Need set esd check eof synctoken to let trigger loop go. */
...
destroy_cmdq:
	...
	mmprofile_log_ex(ddp_mmp_get_events()->esd_rdlcm, MMPROFILE_FLAG_END, 0,
		       ret);

done:
	mmprofile_log_ex(ddp_mmp_get_events()->esd_check_t, MMPROFILE_FLAG_END, 0,
		       ret);
...
{
	mmprofile_log_ex(ddp_mmp_get_events()->esd_vdo_eint, MMPROFILE_FLAG_PULSE,
		       0, 0);
...
int primary_display_esd_recovery(void)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->esd_recovery_t, MMPROFILE_FLAG_START,
		       0, 0);
	...
	mmprofile_log_ex(ddp_mmp_get_events()->esd_recovery_t, MMPROFILE_FLAG_END,
		       0, 0);
...
void decouple_mirror_wdma0_irq_callback(enum DISP_MODULE_ENUM module,
...
static void primary_display_frame_update_irq_callback(enum DISP_MODULE_ENUM module,
						      unsigned int param)
{
					 ...
					 int need_lock)
{
	...
	if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE
		...
		DISPMSG("primary display is %s mode now\n",
			session_mode_spy(pgc->session_mode));
		mmprofile_log_ex(ddp_mmp_get_events()->primary_switch_mode,
			       MMPROFILE_FLAG_PULSE, pgc->session_mode,
			       sess_mode);
...
int primary_display_suspend(void)
{
	...
	dim_layer_mva = 0;
	mmprofile_log_ex(ddp_mmp_get_events()->primary_suspend, MMPROFILE_FLAG_END,
		       0, 0);
...
int primary_display_resume(void)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_resume, MMPROFILE_FLAG_START,
		       0, 0);
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_resume, MMPROFILE_FLAG_PULSE,
		       0, 1);
...
#endif
	mmprofile_log_ex(ddp_mmp_get_events()->primary_resume, MMPROFILE_FLAG_END,
		       0, 0);
...
int primary_display_config_output(struct disp_mem_output_config *output)
{
	int ret = 0;
	struct disp_ddp_path_config *pconfig = NULL;
	...
	if (primary_display_is_decouple_mode()) {
		...
		mmprofile_log_ex(ddp_mmp_get_events()->primary_wdma_config,
			       MMPROFILE_FLAG_PULSE, output->buff_idx,
...
static int _config_rdma_input(struct disp_session_input_config *session_input,
...
void primary_display_idlemgr_enter_idle(int need_lock)
{
	...
	if (primary_display_is_video_mode()) {
		...
		/* spm_sodi_mempll_pwr_mode(1); */
		mmprofile_log_ex(ddp_mmp_get_events()->sodi_enable,
			       MMPROFILE_FLAG_PULSE, 0, 0);
	}
}
...
void primary_display_idlemgr_kick(char *source, int need_lock)
{
	...
	if (primary_display_is_idle()) {
		...
		mmprofile_log_ex(ddp_mmp_get_events()->idlemgr, MMPROFILE_FLAG_END,
			       0, 0);
...
static int _primary_path_idlemgr_monitor_thread(void *data)
{
	...
	while (1) {
		...
		if (((sched_clock() - idlemgr_last_kick_time) / 1000) <
		    500 * 1000) {
...
void primary_display_debug_path_status(int mutex_id)
{
	_primary_path_lock(__func__);

	dpmgr_debug_path_status(mutex_id);
...
int _set_backlight_by_cmdq(unsigned int level)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_set_bl, MMPROFILE_FLAG_PULSE,
		       1, 5);
	return ret;
}
...
int primary_display_setbacklight(unsigned int level)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_set_bl, MMPROFILE_FLAG_START,
		       0, 0);
#ifdef DISP_SWITCH_DST_MODE
	_primary_path_switch_dst_lock();
#endif
	...
	_primary_path_switch_dst_lock();
#endif
	mmprofile_log_ex(ddp_mmp_get_events()->primary_set_bl, MMPROFILE_FLAG_END,
		       0, 0);
	return ret;
}
...
int Panel_Master_dsi_config_entry(const char *name, void *config_value)
{
	...
	struct LCM_PARAMS *lcm_param = NULL;
	struct LCM_DRIVER *pLcm_drv = NULL;
...
struct LCM_PARAMS *lcm_param2 = NULL;
