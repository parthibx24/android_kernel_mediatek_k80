...
int ovl2mem_get_info(void *info)
{
	...
	} else if (dispif_info->displayWidth * dispif_info->displayHeight <=
		   320 * 480) {
		dispif_info->physicalHeight = dispif_info->physicalWidth = 0;
	} else if (dispif_info->displayWidth * dispif_info->displayHeight <=
		   480 * 854) {
...
static int _convert_disp_input_to_ovl(struct OVL_CONFIG_STRUCT *dst,
				      struct primary_disp_input_config *src)
{
	if (src && dst) {
		dst->layer = src->layer;
...
static int ovl2mem_callback(unsigned int userdata)
{
	...
	for (layid = 0; layid < (HW_OVERLAY_COUNT + 1); layid++) {
		fence_idx =
		    mtkfb_query_idx_by_ticket(pgc->session, layid, userdata);
		if (fence_idx < 0) {
			if (fence_idx == -1) {
				;
			...
			if (data_config && (layid == 4)) {
				...
				wdma_layer.dstAddress =
				    mtkfb_query_buf_mva(pgc->session, layid,
							fence_idx);
...
int ovl2mem_init(unsigned int session)
{
...
#if 0
	ret =
	    cmdqCoreRegisterCB(CMDQ_GROUP_DISP, cmdqDdpClockOn, cmdqDdpDumpInfo,
			       cmdqDdpResetEng, cmdqDdpClockOff);
	if (ret) {
	...
	pgc->dpmgr_handle =
	    dpmgr_create_path(DDP_SCENARIO_SUB_OVL_MEMOUT,
			      pgc->cmdq_handle_config);
...
int ovl2mem_input_config(struct ovl2mem_in_config *input)
{
	...
	for (i = 0; i < HW_OVERLAY_COUNT; i++) {
		dprec_logger_start(DPREC_LOGGER_PRIMARY_CONFIG,
				   input->layer | (input->layer_en << 16),
				   input->addr);

		if (input[i].layer_en) {
			if (input[i].vaddr) {
				;
				/* / _debug_pattern(0x00000000, input[i].vaddr,
						input[i].dst_w, input[i].dst_h,
						input[i].src_pitch, 0x00000000,
						input[i].layer, input[i].buff_idx); */
			} else {
				/* _debug_pattern(input[i].addr, 0x00000000,
						input[i].dst_w, input[i].dst_h,
						input[i].src_pitch, 0x00000000,
						input[i].layer, input[i].buff_idx); */
			}
		...
		dprec_logger_done(DPREC_LOGGER_PRIMARY_CONFIG, input->src_x,
				  input->src_y);
...
int ovl2mem_output_config(struct disp_mem_output_config *out)
{
	...
	if (pgc->dpmgr_handle == NULL) {
		DISPERR("ovl2mem_output_config dpmgr_handle is NULL");
		goto Exit;
	}
	/* all dirty should be cleared in dpmgr_path_get_last_config() */
	data_config = dpmgr_path_get_last_config(pgc->dpmgr_handle);
...
void ovl2mem_wait_done(void)
{
	...
	if ((atomic_read(&g_trigger_ticket) - atomic_read(&g_release_ticket)) <=
	    1)
		return;
