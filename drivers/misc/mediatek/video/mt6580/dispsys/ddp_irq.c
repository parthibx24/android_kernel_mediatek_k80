		...
		if (irq == dispsys_irq[DISP_REG_DSI0]) {
			...
			if (reg_val & (1 << 1)) {
				mmprofile_log_ex(ddp_mmp_get_events()->SCREEN_UPDATE[index],
					       MMPROFILE_FLAG_START, reg_val, 0);
				mmprofile_log_ex(ddp_mmp_get_events()->layer[0], MMPROFILE_FLAG_PULSE,
					       DISP_REG_GET(DISP_REG_OVL_L0_ADDR),
					       DISP_REG_GET(DISP_REG_OVL_SRC_CON) & 0x1);
				mmprofile_log_ex(ddp_mmp_get_events()->layer[1], MMPROFILE_FLAG_PULSE,
					       DISP_REG_GET(DISP_REG_OVL_L1_ADDR),
					       DISP_REG_GET(DISP_REG_OVL_SRC_CON) & 0x2);
				mmprofile_log_ex(ddp_mmp_get_events()->layer[2], MMPROFILE_FLAG_PULSE,
					       DISP_REG_GET(DISP_REG_OVL_L2_ADDR),
					       DISP_REG_GET(DISP_REG_OVL_SRC_CON) & 0x4);
				mmprofile_log_ex(ddp_mmp_get_events()->layer[3], MMPROFILE_FLAG_PULSE,
			...
			for (mutexID = 0; mutexID < 5; mutexID++) {
				if (reg_val & (0x1 << mutexID)) {
					...
					mutex_start_irq_cnt++;
					mmprofile_log_ex(ddp_mmp_get_events()->MUTEX_IRQ[index],
