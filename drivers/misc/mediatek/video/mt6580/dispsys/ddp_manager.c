...
static int path_top_clock_off(void)
{
	...
	if (context->power_sate) {
		for (i = 0; i < DDP_MAX_MANAGER_HANDLE; i++) {
			if (context->handle_pool[i] && context->handle_pool[i]->power_sate) 
				return 0;
		}
...
static int module_is_dsi(enum DISP_MODULE_ENUM module)
{
...
static int assign_default_irqs_table(enum DDP_SCENARIO_ENUM scenario, struct DDP_IRQ_EVENT_MAPPING *irq_events)
{
	...
	DISPRCD("assign default irqs table index %d\n", idx);
...
static int acquire_mutex(enum DDP_SCENARIO_ENUM scenario)
{
	...
	DISPRCD("scenario %s acquire mutex %d, left mutex 0x%x!\n",
...
static int release_mutex(int mutex_idx)
{
	...
	DISPRCD("release mutex %d, left mutex 0x%x!\n",
...
int dpmgr_path_set_video_mode(disp_path_handle dp_handle, int is_vdo_mode)
{
	...
	DISPRCD("set scenario %s mode: %s\n", ddp_get_scenario_name(handle->scenario),
		is_vdo_mode ? "Video_Mode" : "Cmd_Mode");
	return 0;
}

disp_path_handle dpmgr_create_path(enum DDP_SCENARIO_ENUM scenario, cmdqRecHandle cmdq_handle)
{
	int i = 0;
	int module_name;
	...
	path_handle = kzalloc(sizeof(struct ddp_path_handle), GFP_KERNEL);
	if (NULL != path_handle) {
		...
		DISPRCD("create handle %p on scenario %s\n", path_handle, ddp_get_scenario_name(scenario));
		for (i = 0; i < module_num; i++) {
			module_name = modules[i];
			DISPRCD(" scenario %s include module %s\n",
...
int dpmgr_modify_path(disp_path_handle dp_handle, enum DDP_SCENARIO_ENUM new_scenario,
		      cmdqRecHandle cmdq_handle, enum DDP_MODE isvdomode, int sw_only)
{
	struct ddp_path_handle *handle = (struct ddp_path_handle *) dp_handle;
	enum DDP_SCENARIO_ENUM old_scenario = handle->scenario;

	handle->cmdqhandle = cmdq_handle;
	handle->scenario = new_scenario;
	ASSERT(dp_handle != NULL);
	DISPRCD("modify handle %p from %s to %s\n", handle, ddp_get_scenario_name(old_scenario),
...
int dpmgr_destroy_path_handle(disp_path_handle dp_handle)
{
	...
	DISPRCD("destroy path handle %p on scenario %s\n", handle,
		   ddp_get_scenario_name(handle->scenario));
	if (handle != NULL) {
...
int dpmgr_path_connect(disp_path_handle dp_handle, int encmdq)
{
	...
	DISPRCD("dpmgr_path_connect on scenario %s\n", ddp_get_scenario_name(handle->scenario));
...
int dpmgr_path_disconnect(disp_path_handle dp_handle, int encmdq)
{
	...
	handle = (struct ddp_path_handle *) dp_handle;
	cmdqHandle = encmdq ? handle->cmdqhandle : NULL;
	DISPRCD("dpmgr_path_disconnect on scenario %s\n", ddp_get_scenario_name(handle->scenario));
	...
	for (i = 0; i < module_num; i++) {
		...
		if (ddp_modules_driver[module_name] != 0) {
			if (ddp_modules_driver[module_name]->init != 0) {
				DISPRCD("scenario %s init module  %s\n",
					   ddp_get_scenario_name(handle->scenario),
					   ddp_get_module_name(module_name));
...
int dpmgr_path_deinit(disp_path_handle dp_handle, int encmdq)
{
	...
	DISPRCD("path deinit on scenario %s\n", ddp_get_scenario_name(handle->scenario));
	ddp_mutex_Interrupt_disable(handle->hwmutexid, cmdqHandle);
	ddp_mutex_clear(handle->hwmutexid, cmdqHandle);
	ddp_disconnect_path(handle->scenario, cmdqHandle);
	for (i = 0; i < module_num; i++) {
		...
		if (ddp_modules_driver[module_name] != 0) {
			if (ddp_modules_driver[module_name]->deinit != 0) {
				DISPRCD("scenario %s deinit module  %s\n",
					   ddp_get_scenario_name(handle->scenario),
					   ddp_get_module_name(module_name));
...
int dpmgr_path_start(disp_path_handle dp_handle, int encmdq)
{
	...
	DISPRCD("path start on scenario %s\n", ddp_get_scenario_name(handle->scenario));
	for (i = 0; i < module_num; i++) {
		module_name = modules[i];
		if (ddp_modules_driver[module_name] != 0) {
...
int dpmgr_path_stop(disp_path_handle dp_handle, int encmdq)
{
	...
	DISPRCD("path stop on scenario %s\n", ddp_get_scenario_name(handle->scenario));
		     ...
		     unsigned long *params)
{
	...
	modules = ddp_get_scenario_list(handle->scenario);
	module_num = ddp_get_module_num(handle->scenario);
	DISPRCD("path IOCTL on scenario %s\n", ddp_get_scenario_name(handle->scenario));
	for (i = module_num - 1; i >= 0; i--) {
		module_name = modules[i];
		if (ddp_modules_driver[module_name] != 0) {
...
int dpmgr_path_enable_irq(disp_path_handle dp_handle, void *cmdq_handle, enum DDP_IRQ_LEVEL irq_level)
{
	...
	DISPRCD("path enable irq on scenario %s, level %d\n",
	...
	for (i = module_num - 1; i >= 0; i--) {
		...
		if (ddp_modules_driver[module_name] != 0) {
			if (ddp_modules_driver[module_name]->enable_irq != 0) {
				DISPRCD("scenario %s, module %s enable irq level %d\n",
				       ddp_get_scenario_name(handle->scenario),
...
int dpmgr_path_reset(disp_path_handle dp_handle, int encmdq)
{
	...
	DISPRCD("path reset on scenario %s\n", ddp_get_scenario_name(handle->scenario));
	/* first reset mutex */
	...
	for (i = 0; i < module_num; i++) {
		...
		if (ddp_modules_driver[module_name] != 0) {
			if (ddp_modules_driver[module_name]->reset != 0) {
				DISPRCD("scenario %s  reset module %s\n",
					   ddp_get_scenario_name(handle->scenario),
					   ddp_get_module_name(module_name));
				ret =
...
int dpmgr_path_config(disp_path_handle dp_handle, struct disp_ddp_path_config *config, void *cmdq_handle)
{
	...
	DISPRCD("path config ovl %d, rdma %d, wdma %d, dst %d on handle %p scenario %s\n",
		   config->ovl_dirty,
		   config->rdma_dirty,
		   config->wdma_dirty,
	...
	for (i = 0; i < module_num; i++) {
		...
		if (ddp_modules_driver[module_name] != 0) {
			...
			if (module_name == DISP_MODULE_COLOR0) {
				set_color_bypass(enum DISP_MODULE_ENUM module, int bypass, void *cmdq_handle);
				set_color_bypass(DISP_MODULE_COLOR0, 1, cmdq_handle);
			}
#endif
			if (gddp_bypass_pq == 1 && dpmgr_is_PQ(module_name) == 1) {
#else
			{
#endif
			  ...
			  int reverse)
{
	...
	DISPRCD("path build cmdq on scenario %s\n", ddp_get_scenario_name(handle->scenario));
...
int dpmgr_path_flush(disp_path_handle dp_handle, int encmdq)
{
	...
	DISPRCD("path flush on scenario %s\n", ddp_get_scenario_name(handle->scenario));
...
int dpmgr_path_power_off(disp_path_handle dp_handle, enum CMDQ_SWITCH encmdq)
{
	...
	DISPRCD("path power off on scenario %s\n", ddp_get_scenario_name(handle->scenario));
	for (i = 0; i < module_num; i++) {
		module_name = modules[i];
		if (ddp_modules_driver[module_name] != 0) {
			if (ddp_modules_driver[module_name]->power_off != 0) {
				...
				ddp_modules_driver[module_name]->power_off(module_name,
					encmdq ? handle->cmdqhandle : NULL);	/* now just 0; */
			}
		}
	}
	handle->power_sate = 0;
		...
		unsigned long arg, void *cmdqhandle)
{
	...
	case DISP_IOCTL_AAL_SET_PARAM:
		{
			...
			if (is_module_in_path(DISP_MODULE_AAL, handle)) {
				if (ddp_modules_driver[DISP_MODULE_AAL]->cmd != NULL) {
					ret =
					    ddp_modules_driver[DISP_MODULE_AAL]->
	...
	case DISP_IOCTL_READ_SW_REG:
		{
			if (is_module_in_path(DISP_MODULE_COLOR0, handle))
				...
				DISPRCD("dpmgr_path_user_cmd color is not on this path\n");
	...
	default:
		{
			DISPRCD("dpmgr_path_user_cmd io not supported\n");
			break;
		}
	}
	return ret;
...
int dpmgr_enable_event(disp_path_handle dp_handle, enum DISP_PATH_EVENT event)
{
	...
	DISPRCD("enable event %s on scenario %s, irtbit 0x%x\n",
...
int dpmgr_map_event_to_irq(disp_path_handle dp_handle, enum DISP_PATH_EVENT event, enum DDP_IRQ_BIT irq_bit)
{
	...
	if (event < DISP_PATH_EVENT_NUM) {
		DISPRCD("map event %s to irq 0x%x on scenario %s\n",
...
int dpmgr_disable_event(disp_path_handle dp_handle, enum DISP_PATH_EVENT event)
{
	...
	DISPRCD("disable event %s on scenario %s\n", path_event_name(event),
		   ddp_get_scenario_name(handle->scenario));
	wq_handle = &handle->wq_list[event];
	wq_handle->init = 0;
	wq_handle->data = 0;
