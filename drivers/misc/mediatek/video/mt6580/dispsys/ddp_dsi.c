...
static void __iomem *ddp_apmixed_base __attribute__ ((unused));
#ifndef AP_PLL_CON0
...
{	\
	...
	while (INREG32(MIPITX_BASE+0xC)&0x1 != 0x1)\
		;		\
	mt_reg_sync_writel(0xFF, MIPITX_BASE+0xC);		\
	DISPDBG("MIPI write data = 0x%x, read data = 0x%x\n", write_data, INREG32(MIPITX_BASE));		\
	if (INREG32(MIPITX_BASE) == write_data)\
		DISPDBG("MIPI write success\n");		\
	else\
		DISPDBG("MIPI write fail\n");		\
}
...
static void _DSI_INTERNAL_IRQ_Handler(enum DISP_MODULE_ENUM module, unsigned int param)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (status.FRAME_DONE_INT_EN) {
			/* mmprofile_log_ex(ddp_mmp_get_events()->dsi_frame_done,
				MMPROFILE_FLAG_PULSE,
				DSI_REG[0]->DSI_LFR_STA.LFR_SKIP_CNT,
				DSI_REG[0]->DSI_LFR_STA.LFR_SKIP_STA); */
		}
	}
}
...
static enum DSI_STATUS DSI_SetMode(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq, unsigned int mode)
{
	int i = 0;
...
#if 0
static enum DSI_STATUS DSI_SetVdoFrmMode(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq, unsigned int mode)
{
	int i = 0;
...
#endif
static enum DSI_STATUS DSI_SetSwitchMode(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq, unsigned int mode)
{
	int i = 0;
...
#if 0
static enum DSI_STATUS DSI_SetBypassRack(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq,
				    unsigned int bypass)
{
	int i = 0;
...
#endif
enum DSI_STATUS DSI_DisableClk(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
#if 0
	DISPMSG("[DISP] %s\n", __func__);
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++)
...
void DSI_sw_clk_trail_cmdq(int module_idx, cmdqRecHandle cmdq)
{
	...
	DISP_REG_SET(cmdq, &DSI_PHY_REG[module_idx]->MIPITX_DSI_SW_CTRL_EN, 0x01);
...
void DSI_lane0_ULP_mode(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq, bool enter)
{
	int i = 0;
...
void DSI_clk_ULP_mode(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq, bool enter)
{
	int i = 0;
...
bool DSI_clk_HS_state(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	int i = 0;
...
void DSI_clk_HSLP_mode(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	int i = 0;
...
void DSI_clk_HS_mode(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq, bool enter)
{
	int i = 0;
...
static void DSI_WaitForNotBusy(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
		int i = 0;
...
enum DSI_STATUS DSI_SleepOut(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	int i = 0;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DSI_OUTREGBIT(cmdq, struct DSI_TIME_CON0_REG, DSI_REG[i]->DSI_TIME_CON0, UPLS_WAKEUP_PRD,
			      wake_up_prd);
	}
...
enum DSI_STATUS DSI_Wakeup(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	int i = 0;
	int ret = 0;
	int cnt = 0;
...
enum DSI_STATUS DSI_BackupRegisters(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
		int i = 0;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DISPDBG("DSI_BackupRegisters VM_CMD_EN %d TS_VFP_EN %d\n",
...
enum DSI_STATUS DSI_RestoreRegisters(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	int i = 0;
...
void DSI_Config_VDO_Timing(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq, struct LCM_DSI_PARAMS *dsi_params)
{
	int i = 0;
...
void DSI_Set_LFR(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq, unsigned int mode,
		 unsigned int type, unsigned int enable, unsigned int skip_num)
{
	/* LFR_MODE 0 disable,1 static mode ,2 dynamic mode 3,both */
	unsigned int i = 0;

	DISPDBG("module=%d,mode=%d,type=%d,enable=%d,skip_num=%d\n", module, mode, type, enable,
	       skip_num);
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
...
void DSI_LFR_UPDATE(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	unsigned int i = 0;
...
int DSI_LFR_Status_Check(void)
{
	...
	DISPDBG("LFR_SKIP_CNT 0x%x LFR_SKIP_STA 0x%x,status 0x%x\n", lfr_skip_sta.LFR_SKIP_CNT,
...
enum DSI_STATUS DSI_PS_Control(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq, struct LCM_DSI_PARAMS *dsi_params,
			  int w, int h)
{
	int i = 0;
...
enum DSI_STATUS DSI_TXRX_Control(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq,
			    struct LCM_DSI_PARAMS *dsi_params)
{
	int i = 0;
...
int MIPITX_IsEnabled(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	int i = 0;
	int ret = 0;
...
void DSI_PHY_clk_setting(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq, struct LCM_DSI_PARAMS *dsi_params)
{
#ifdef CONFIG_MTK_FPGA
#if 0
...
void DSI_PHY_TIMCONFIG(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq, struct LCM_DSI_PARAMS *dsi_params)
{
	int i = 0;
#ifdef CONFIG_MTK_FPGA
	return 0;
	...
	if (0 != dsi_params->PLL_CLOCK) {
		...
		DISPDBG("DISP/DSI DSI_PHY_TIMCONFIG, Cycle Time = %d(ns), Unit Interval = %d(ns), lane# = %d\n",
...
#define NS_TO_CYCLE(n, c)	((n) / (c))
	...
	DISPDBG("DSI_PHY_TIMCONFIG, HS_TRAIL = %d, HS_ZERO = %d, HS_PRPR = %d, LPX = %d, TA_GET = %d\n",
		 timcon0.HS_TRAIL, timcon0.HS_ZERO, timcon0.HS_PRPR, timcon0.LPX, timcon1.TA_GET);
	DISPDBG("TA_SURE = %d, TA_GO = %d, CLK_TRAIL = %d, CLK_ZERO = %d, CLK_HS_PRPR = %d\n",
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DSI_OUTREGBIT(cmdq, struct DSI_PHY_TIMCON3_REG, DSI_REG[i]->DSI_PHY_TIMECON3, CLK_HS_EXIT,
			      timcon3.CLK_HS_EXIT);
		DISPDBG("%s, 0x%08x,0x%08x,0x%08x,0x%08x\n", __func__,
...
void DSI_PHY_clk_switch(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq, int on)
{
	int i = 0;
...
enum DSI_STATUS DSI_EnableClk(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
#if 0
	DISPMSG("[DISP] %s\n", __func__);
	int i = 0;
...
void DSI_Set_VM_CMD(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
...
enum DSI_STATUS DSI_EnableVM_CMD(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	int i = 0;
		...
		do {
			...
			if (DSI_REG[d]->DSI_INTSTA.RD_RDY != 0
				...
				/* dump cmdq & rxdata */
				{
					...
					DISPDBG("Last DSI Read Why not clear irq???\n");
			...
			DISPERR
			    ("DSI Read Fail: dsi wait read ready wake up by other interrupt\n");
			...
			if (packet_type == 0x1A || packet_type == 0x1C) {	/* long read response */
				...
				DISPDBG("read return type is 0x02, re-read\n");
			else {
...
void DSI_set_cmdq_V2(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq, unsigned cmd, unsigned char count,
...
void DSI_set_cmdq_V3(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq, struct LCM_setting_table_V3 *para_tbl,
	...
	for (d = DSI_MODULE_BEGIN(module); d <= DSI_MODULE_END(module); d++) {
		do {
			...
			if (data_id == REGFLAG_ESCAPE_ID && cmd == REGFLAG_DELAY_MS_V3) {
				udelay(1000 * count);
				DISPDBG("DSI_set_cmdq_V3[%d]. Delay %d (ms)\n", index,
					count);
			...
			if (force_update) {
				/* MMProfileLog(MTKFB_MMP_Events.DSICmd, MMPROFILE_FLAG_START); */
				DSI_Start(module, cmdq);
				DSI_WaitForNotBusy(module, cmdq);
				/* MMProfileLog(MTKFB_MMP_Events.DSICmd, MMPROFILE_FLAG_END); */
			}
		  ...
		  unsigned int queue_size, unsigned char force_update)
{
	...
	/* DISPDBG("DSI_set_cmdq, module=%s, cmdq=0x%08x\n", ddp_get_module_name(module), cmdq); */
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (0 != DSI_REG[i]->DSI_MODE_CTRL.MODE) {
			...
			/* DISPDBG("[DISP] - kernel - DSI_set_cmdq. DSI_CMDQ+%04x : 0x%08x\n",
...
void _copy_dsi_params(struct LCM_DSI_PARAMS *src, struct LCM_DSI_PARAMS *dst)
{
	memcpy((struct LCM_DSI_PARAMS *) dst, (struct LCM_DSI_PARAMS *) src, sizeof(struct LCM_DSI_PARAMS));
}
		 ...
		 unsigned int width, unsigned int height)
{
	...
	if (!primary_display_is_video_mode()) {
		...
		DISPDBG("LCM is video mode, no need to send ROI!\n");
	}
			  ...
			  unsigned char force_update)
{
	...
	DSI_set_cmdq_V2(DISP_MODULE_DSI0, cmdq_hd, cmd, count, para_list, force_update);
}
			  ...
			  unsigned char force_update)
{
	...
	DSI_set_cmdq_V2(DISP_MODULE_DSI1, cmdq_hd, cmd, count, para_list, force_update);
}
			     ...
			     unsigned char *para_list, unsigned char force_update)
{
	...
	DSI_set_cmdq_V2(DISP_MODULE_DSIDUAL, cmdq_hd, cmd, count, para_list, force_update);
}
...
int ddp_dsi_init(enum DISP_MODULE_ENUM module, void *cmdq)
{
	...
	if (MIPITX_IsEnabled(module, cmdq)) {
		...
		DSI_OUTREGBIT(NULL, struct DSI_INT_ENABLE_REG, DSI_REG[0]->DSI_INTEN, SLEEPOUT_DONE, 1);
		/* DSI_OUTREGBIT(NULL, struct DSI_INT_ENABLE_REG,DSI_REG[0]->DSI_INTEN,FRAME_DONE_INT_EN,0); */
...
void _dump_dsi_params(struct LCM_DSI_PARAMS *dsi_config)
{
	if (dsi_config) {
		...
		case BURST_VDO_MODE:
			...
			break;
		default:
			DISPDBG("[DDPDSI] DSI Mode: Unknown\n");
			break;
		}

		/* DISPDBG("[DDPDSI] LANE_NUM: %d,data_format: %d,vertical_sync_active: %d\n",
...
static void DSI_PHY_CLK_LP_PerLine_config(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq,
					  struct LCM_DSI_PARAMS *dsi_params)
{
	int i;
...
{
	int i = 0;
	struct LCM_DSI_PARAMS *dsi_config = &(config->dispif_config.dsi);
...
int ddp_dsi_stop(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (_dsi_is_video_mode(module)) {
		...
		if (module == DISP_MODULE_DSIDUAL) {
			DSI_OUTREGBIT(cmdq_handle, struct DSI_COM_CTRL_REG, DSI_REG[0]->DSI_COM_CTRL,
				      DSI_DUAL_EN, 0);
			DSI_OUTREGBIT(cmdq_handle, struct DSI_COM_CTRL_REG, DSI_REG[1]->DSI_COM_CTRL,
				      DSI_DUAL_EN, 0);
			DSI_OUTREGBIT(cmdq_handle, struct DSI_START_REG, DSI_REG[0]->DSI_START, DSI_START,
				      0);
	...
	if (!s_isDsiPowerOn) {
		...
		if (is_ipoh_bootup) {
			if (module == DISP_MODULE_DSI0 || module == DISP_MODULE_DSIDUAL) {
				...
				if (ret > 0)
					DISPERR("DISP/DSI " "DSI power manager API return false\n");

			}
			s_isDsiPowerOn = true;
			DISPDBG("ipoh dsi power on return\n");
			return DSI_STATUS_OK;
		}
		DSI_PHY_clk_switch(module, NULL, true);
...
int ddp_dsi_power_off(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (s_isDsiPowerOn) {
		...
		while (1) {
			...
			if (value == 0x20)
				break;
			DISPDBG("dsi not in ulps mode, try again...\n");
		}
...
void dsi_analysis(enum DISP_MODULE_ENUM module)
{
	...
	DISPDBG("==DISP DSI ANALYSIS==\n");
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		DISPDBG("MIPITX Clock: %d\n", dsi_phy_get_clk(module));
		DISPDBG
		...
		DISPDBG("DSI%d Lane Num:%d, Ext_TE_EN:%d, Ext_TE_Edge:%d, HSTX_CKLP_EN:%d\n", i,
...
struct DDP_MODULE_DRIVER ddp_driver_dsi0 = {
	...
	.set_lcm_utils = (int (*)(enum DISP_MODULE_ENUM, struct LCM_DRIVER*)) ddp_dsi_set_lcm_utils,
...
{
	DISPDBG("[DSI](x0,y0,x1,y1)=(%d,%d,%d,%d)\n", x, y, _dsi_context[0].lcm_width,
...
int DSI_check_roi(void)
{
	...
	DISPDBG("x0=%d count=%d,read_buf[0]=%d,read_buf[1]=%d,read_buf[2]=%d,read_buf[3]=%d\n", x0,
	...
	DISPDBG("y0=%d count %d,read_buf[0]=%d,read_buf[1]=%d,read_buf[2]=%d,read_buf[3]=%d\n", y0,
	       count, read_buf[0], read_buf[1], read_buf[2], read_buf[3]);
	if ((count == 0) || (y0 != 0)) {
		DISPDBG
