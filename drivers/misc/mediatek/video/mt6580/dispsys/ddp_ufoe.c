...
int ufoe_start(enum DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	if (ufoe_enable)
...
{
	struct LCM_PARAMS *disp_if_config = &(pConfig->dispif_config);
	struct LCM_DSI_PARAMS *lcm_config = &(disp_if_config->dsi);
