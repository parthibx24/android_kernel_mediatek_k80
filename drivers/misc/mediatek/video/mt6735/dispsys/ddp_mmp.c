...
void init_ddp_mmp_events(void)
{
	if (DDP_MMP_Events.DDP == 0) {
		...
		DDP_MMP_Events.tui =
			mmprofile_register_event(DDP_MMP_Events.primary_Parent, "tui");
...
void _ddp_mmp_ovl_not_raw_under_session(unsigned int session, struct OVL_CONFIG_STRUCT *pLayer, struct mmp_metadata_bitmap_t *Bitmap)
{
...
void _ddp_mmp_ovl_raw_under_session(unsigned int session, struct OVL_CONFIG_STRUCT *pLayer, struct mmp_metadata_t *meta)
{
	if (session == 1) {
		if (ovl_get_status() == DDP_OVL1_STATUS_PRIMARY ||
		    ovl_get_status() == DDP_OVL1_STATUS_SUB_REQUESTING) { /* 8+0 */
