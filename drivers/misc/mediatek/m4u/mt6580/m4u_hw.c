...
static unsigned int gM4UTagCount[] = { 32, 32 };
...
int m4u_dump_valid_main_tlb(int m4u_id, int m4u_slave_id)
{
	...
	M4UMSG("dump valid main tlb=>\n");
	for (i = 0; i < gM4UTagCount[m4u_id]; i++) {
		m4u_get_main_tlb(m4u_id, m4u_slave_id, i, &tlb);
		if ((tlb.tag & (F_MAIN_TLB_VALID_BIT))
		    == (F_MAIN_TLB_VALID_BIT)) {
			M4ULOG_HIGH("%d:0x%x:0x%x  ", i, tlb.tag, tlb.desc);
		}
	}
	M4ULOG_HIGH("\n");
...
static int larb_clock_on(int larb)
{
	...
	break;
	case 1:
		enable_clock(MT_CG_LARB1_SMI_CKPDN, "m4u_larb1");
	break;
	default:
...
static int larb_clock_off(int larb)
{
	...
	break;
	case 1:
		disable_clock(MT_CG_LARB1_SMI_CKPDN, "m4u_larb1");
	break;
...
static int _m4u_config_port(int port, int virt, int sec, int dis, int dir)
{
	...
	/* mmprofile_log_ex(M4U_MMP_Events[M4U_MMP_CONFIG_PORT], MMProfileFlagStart, port, virt); */
...
/* debug use */
	...
	/* mmprofile_log_ex(M4U_MMP_Events[M4U_MMP_CONFIG_PORT], MMProfileFlagEnd, dis, dir); */
...
static inline void _m4u_port_clock_toggle(int m4u_index, int larb, int on)
{
	...
	/* mmprofile_log_ex(M4U_MMP_Events[M4U_MMP_TOGGLE_CG], MMProfileFlagStart, larb, on); */
	if (m4u_index == 0) {
		start = sched_clock();
		if (on) {
			smi_common_clock_on();
	...
	/* mmprofile_log_ex(M4U_MMP_Events[M4U_MMP_TOGGLE_CG], MMProfileFlagEnd, 0, 0); */
}
	...
	for (port = 0; port < M4U_PORT_NR; port++) {
		if (port_array->ports[port] & M4U_PORT_ATTR_EN) {
			...
			ret |= _m4u_config_port(port,
						!!(port_array->ports[port] & M4U_PORT_ATTR_VIRTUAL),
						!!(port_array->ports[port] & M4U_PORT_ATTR_SEC), 1, 0);
		}
	}
...
void m4u_enable_error_hang(int m4u_index, int fgenable)
{
	unsigned int m4u_base = gM4UBaseAddr[m4u_index];
...
void m4u_enable_MTLB_allshare(int m4u_index, int fgenable)
{
	unsigned int m4u_base = gM4UBaseAddr[m4u_index];

	if (1 == fgenable)
		M4U_WriteReg32(m4u_base, REG_MMU_MTLB_RESERVE_MODE_DIS, 1);
	else
		M4U_WriteReg32(m4u_base, REG_MMU_MTLB_RESERVE_MODE_DIS, 0);
}


int m4u_manual_insert_entry(int m4u_id,
			    unsigned int EntryMVA,
			    int layer, int pt_type, int secure, int Lock, unsigned int pa)
{
	unsigned int m4u_base = gM4UBaseAddr[m4u_id];


	EntryMVA &= F_PROG_VA_MASK;	/* clear bit0~11 */
	...
	if (layer) {
		EntryMVA |= F_PROG_VA_LAYER_BIT;
		if (pt_type == MMU_PT_TYPE_SMALL_PAGE)
			pa |= F_PTE_TYPE_SMALL;
		else if (pt_type == MMU_PT_TYPE_LARGE_PAGE)
			pa |= F_PTE_TYPE_LARGE;
		else
			return -1;
	} else {
		if (pt_type == MMU_PT_TYPE_SMALL_PAGE || pt_type == MMU_PT_TYPE_LARGE_PAGE)
			pa |= F_PGD_TYPE_PAGE;
		else if (pt_type == MMU_PT_TYPE_SECTION)
			pa |= F_PGD_TYPE_SECTION;
		else if (pt_type == MMU_PT_TYPE_SUPERSECTION)
			pa |= F_PGD_TYPE_SUPERSECTION;
	}
	if (pt_type == MMU_PT_TYPE_LARGE_PAGE || pt_type == MMU_PT_TYPE_SUPERSECTION)
		EntryMVA |= F_PROG_VA_SIZE16X_BIT;


	M4U_WriteReg32(m4u_base, REG_MMU_PROG_VA, EntryMVA);
	M4U_WriteReg32(m4u_base, REG_MMU_PROG_DSC, pa);
	M4U_WriteReg32(m4u_base, REG_MMU_PROG_EN, F_MMU0_PROG_EN);
...
void m4u_get_perf_counter(int m4u_index, int m4u_slave_id, M4U_PERF_COUNT *pM4U_perf_count)
{
	...
	/* Prefetch count */
	pM4U_perf_count->pfh_cnt = M4U_ReadReg32(m4u_base, REG_MMU_PF_CNT);
}
...
void m4u_print_port_status(struct seq_file *seq, int only_print_active)
{
	...
	for (port = 0; port < gM4u_port_num; port++) {
		...
		if (m4u_index == 0) {
			...
			mmu_en = m4uHw_get_field_by_mask(larb_base, SMI_LARB_MMU_EN,
					F_SMI_MMU_EN(larb_port, 1));
			sec = m4uHw_get_field_by_mask(larb_base, SMI_LARB_SEC_EN,
	...
	if (0 == m4u_id) {
		...
		for (i = 0; i < SMI_LARB_NR; i++) {
			...
			if (NULL == node)
				...
				M4U_WriteReg32(gLarbBaseAddr[i], SMI_LARB_DOMN_0, 0x33333333);
				M4U_WriteReg32(gLarbBaseAddr[i], SMI_LARB_DOMN_1, 0x33333333);
				M4U_WriteReg32(gLarbBaseAddr[i], SMI_LARB_DOMN_2, 0x33333333);
				M4U_WriteReg32(gLarbBaseAddr[i], SMI_LARB_DOMN_3, 0x33333333);
			}
		}
	}
/* ============================================= */
