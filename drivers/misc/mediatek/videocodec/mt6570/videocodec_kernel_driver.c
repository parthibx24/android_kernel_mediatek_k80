...
{
	MODULE_MFV_LOGE("[VCODEC][MMDVFS_VDEC] @@ VdecDVFS monitor %dms, decoded %d frames, total time %d, max duration %d, target lv %d",
	     MONITOR_DURATION_MS, gMMDFVFSMonitorCounts, gHWLockInterval, gHWLockMaxDuration,
	     level);
...
static VAL_INT32_T gu4HybPWRCounter;	/* no mutex, just for debug */
...
void vdec_power_on(void)
{
	...
	/* Central power on */
	MODULE_MFV_LOGE("[VCODEC] vdec_power_on + (%d) (%d) (%d)\n", gu4VdecPWRCounter, gu4HybPWRCounter,
		 gu4VDecIRQCount);
...
#endif
	MODULE_MFV_LOGD("[VCODEC] vdec_power_on -\n");
}
...
void vdec_power_off(void)
{
	...
	if (gu4VdecPWRCounter != 0) {
		...
		--gu4HybPWRCounter;
		/* Central power off */
		MODULE_MFV_LOGE("[VCODEC] vdec_power_off + (%d) (%d) (%d)\n", gu4VdecPWRCounter,
			 gu4HybPWRCounter, gu4VDecIRQCount);
...
#endif
		MODULE_MFV_LOGD("[VCODEC] vdec_power_off -\n");
	}
	spin_unlock_irqrestore(&VdecPWRLock, ulFlagsVdecPWRLock);
}
...
void venc_power_on(void)
{
	...
	MODULE_MFV_LOGE("[VCODEC] venc_power_on + (%d) (%d) (%d)\n", gu4VencPWRCounter, gu4HybPWRCounter,
		 gu4VEncIRQCount);
...
void venc_power_off(void)
{
	...
	if (gu4VencPWRCounter != 0) {
		gu4VencPWRCounter--;
		--gu4HybPWRCounter;
		MODULE_MFV_LOGE("[VCODEC] venc_power_off + (%d) (%d) (%d)\n", gu4VencPWRCounter,
			 gu4HybPWRCounter, gu4VEncIRQCount);
...
*/
	if (grVcodecHWLock.eDriverType == VAL_DRIVER_TYPE_H264_DEC ||
		...
		/* MODULE_MFV_LOGD("oal_hw_context[index].kva_u4HWIsCompleted 0x%x value=%d\n", */
		...
		if ((((volatile VAL_UINT32_T *)oal_hw_context[index].kva_u4HWIsCompleted) == NULL)
		    || (((volatile VAL_UINT32_T *)oal_hw_context[index].kva_u4HWIsTimeout) ==
			NULL)) {
		...
		*((volatile VAL_UINT32_T *)oal_hw_context[index].kva_u4HWIsTimeout) = 0;
		...
		for (i = 0; i < maxnum; i++) {
			...
			*((volatile VAL_UINT32_T *)oal_hw_context[index].kva_Oal_HW_mem_reg +
			  i * 2 + 1) =
*((volatile VAL_UINT32_T *)oal_hw_context[index].oalmem_status[i].u4ReadAddr);
			...
			if (maxnum == 3) {
				if (i == 0) {
					u4IRQStatus =
					    (*
					     ((volatile VAL_UINT32_T *)oal_hw_context[index].
					      kva_Oal_HW_mem_reg + i * 2 + 1));
					if (u4IRQStatus != 2) {
						MODULE_MFV_LOGE("[VCODEC][ERROR][ISR] IRQ status error u4IRQStatus = %d\n",
						     u4IRQStatus);
					}
				}
				...
				if (u4IRQStatus != 2) {
					MODULE_MFV_LOGE("[VCODEC][ERROR][ISR] %d, 0x%lx, %d, %d, %d, %d\n",
						 i,
						 (VAL_ULONG_T) ((volatile VAL_UINT32_T *)
								oal_hw_context[index].
		...
		if (VAL_RESULT_NO_ERROR != eValRet) {
			MODULE_MFV_LOGE("[VCODEC][ERROR] ISR set IsrEvent error\n");
			/* return; */
		}
		/* Clear interrupt */
		/* ACK interrupt */
		...
		if (VAL_RESULT_NO_ERROR != eValHWLockRet)
			MODULE_MFV_LOGE("[VCODEC][ERROR] ISR set dec_isr error\n");
...
*((volatile VAL_UINT32_T *)oal_hw_context[index].oalmem_status[i].u4ReadAddr);
		...
		eValHWLockRet = eVideoSetEvent(&HWLockEvent, sizeof(VAL_EVENT_T));
		if (VAL_RESULT_NO_ERROR != eValHWLockRet)
			MODULE_MFV_LOGE("[VCODEC][ERROR] ISR set enc_isr error\n");
	...
	case VCODEC_SET_THREAD_ID:
		{
			...
			if (ret) {
				MODULE_MFV_LOGE("[ERROR] VCODEC_SET_THREAD_ID, copy_from_user failed: %ld\n",
				     ret);
				return -EFAULT;
			}
	...
	case VCODEC_ALLOC_NON_CACHE_BUFFER:
		{
			...
			if ((0 == rTempMem.u4ReservedSize) || (0 == rTempMem.pvMemPa)) {
				MODULE_MFV_LOGE("[ERROR] dma_alloc_coherent fail in VCODEC_ALLOC_NON_CACHE_BUFFER, size=%lu\n",
				     rTempMem.u4MemSize);
				return -EFAULT;
			}
			...
			if (i4Index == -1) {
				MODULE_MFV_LOGE("[VCODEC][ERROR] Add_NonCacheMemoryList error, u4Index = -1\n");
				spin_unlock_irqrestore(&OalHWContextLock, ulFlags);
				break;
			}
			...
			if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
				...
				while (bLockedHW == VAL_FALSE) {
					...
					if (grVcodecHWLock.pvHandle ==
					    (VAL_VOID_T *) pmem_user_v2p_video((VAL_ULONG_T)
									       rHWLock.pvHandle)) {
						MODULE_MFV_LOGE("[WARNING] VCODEC_LOCKHW, one decoder instance try to lock twice, may cause lock HW timeout!! instance = 0x%lx, CurrentTID = %d\n",
					...
					if (VAL_RESULT_INVALID_ISR == eValRet) {
						MODULE_MFV_LOGE("[ERROR] VCODEC_LOCKHW, HWLockEvent TimeOut, CurrentTID = %d\n",
						     current->pid);
						if (FirstUseDecHW != 1) {
							spin_lock_irqsave(&HWLock, ulFlagsHWLock);
							if (grVcodecHWLock.pvHandle == 0)
					...
					if (grVcodecHWLock.pvHandle == 0)	{ /* No one holds dec hw lock now */
						if (rHWLock.eDriverType == VAL_DRIVER_TYPE_H264_DEC ||
							...
							MODULE_MFV_LOGD("VCODEC_LOCKHW, No process use enc HW, so current process can use HW, handle = 0x%lx\n",
							     (VAL_ULONG_T) grVcodecHWLock.pvHandle);

							spin_lock_irqsave(&OalHWContextLock,
									  ulFlags);
							oal_hw_context[u4Index].pvHandle =
							    (VAL_HANDLE_T) grVcodecHWLock.pvHandle;
							/* WaitISR need thread ID info. */
							grVcodecHWLock.u4VCodecThreadNum =
							    oal_hw_context[u4Index].
							    u4VCodecThreadNum;
							for (i = 0;
							     i < grVcodecHWLock.u4VCodecThreadNum;
							     i++) {
								grVcodecHWLock.u4VCodecThreadID[i] =
								    oal_hw_context[u4Index].
								    u4VCodecThreadID[i];
							}
							spin_unlock_irqrestore(&OalHWContextLock,
							...
							grVcodecHWLock.eDriverType =
							    rHWLock.eDriverType;
							eVideoGetTimeOfDay(&grVcodecHWLock.
									   rLockedTime,
									   sizeof(VAL_TIME_T));

							MODULE_MFV_LOGD("VCODEC_LOCKHW, LockInstance = 0x%lx CurrentTID = %d, rLockedTime(s, us) = %d, %d\n",
							     (VAL_ULONG_T) grVcodecHWLock.pvHandle,
							     current->pid,
							...
							if (VAL_RESULT_INVALID_ISR == eValRet
								...
								/* --gu4VDecIRQCount; */
								MODULE_MFV_LOGE("[WARNING] VCODEC_LOCKHW, reset power when HWLock (%d)!!\n",
								     gu4VDecIRQCount);
							}
							vdec_power_on();
							...
							if (VAL_RESULT_INVALID_ISR == eValRet
							    && FirstUseDecHW != 1) {
								MODULE_MFV_LOGE("[WARNING] VCODEC_LOCKHW, reset power when HWLock!!\n");
								vdec_power_off();
								/* disable_irq(VDEC_IRQ_ID); */
								/* --gu4VDecIRQCount; */
							}
						...
						MODULE_MFV_LOGE("VCODEC_LOCKHW Lock Instance = 0x%lx, Lock TID = %d, CurrentTID = %d, rLockedTime(%d s, %d us), rCurTime(%d s, %d us)\n",
						     (VAL_ULONG_T) grVcodecHWLock.pvHandle,
						     gu4VdecLockThreadId, current->pid,
						     grVcodecHWLock.rLockedTime.u4Sec,
						     grVcodecHWLock.rLockedTime.u4uSec,
				...
				while (bLockedHW == VAL_FALSE) {
					...
					if (grVcodecHWLock.pvHandle ==
					    (VAL_VOID_T *) pmem_user_v2p_video((VAL_ULONG_T)
									       rHWLock.pvHandle)) {
						MODULE_MFV_LOGE("[WARNING] VCODEC_LOCKHW, one encoder instance try to lock twice, may cause lock HW timeout!! instance = 0x%lx, CurrentTID = %d, type:%d\n",
						     (VAL_ULONG_T) grVcodecHWLock.pvHandle,
					...
					if (VAL_RESULT_INVALID_ISR == eValRet) {
						...
						if (FirstUseEncHW != 1) {
							...
							if (grVcodecHWLock.pvHandle == 0) {
								...
								if (rHWLock.u4TimeoutMs == 0) {
									MODULE_MFV_LOGE("[ERROR] VCODEC_LOCKHW - ID %d  fail, someone locked HW already 0x%lx, %lx, 0x%lx, type:%d\n",
									     current->pid,
									     (VAL_ULONG_T)
									     grVcodecHWLock.
									     pvHandle,
					...
					if (grVcodecHWLock.pvHandle == 0)	{
						...
						if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_ENC) {
							...
							grVcodecHWLock.pvHandle =
							    ...
							    pmem_user_v2p_video((VAL_ULONG_T)
										rHWLock.pvHandle);
							MODULE_MFV_LOGD("VCODEC_LOCKHW, No process use HW, so current process can use HW, handle = 0x%lx\n",
							     (VAL_ULONG_T) grVcodecHWLock.pvHandle);
							grVcodecHWLock.u4ThreadID = current->pid;
							grVcodecHWLock.eDriverType =
							    rHWLock.eDriverType;
						...
						MODULE_MFV_LOGD("VCODEC_LOCKHW, LockInstance = 0x%lx, CurrentInstance = 0x%lx, CurrentTID = %d, TimeInterval(ms) = %d, TimeOutValue(ms)) = %d\n",
						     (VAL_ULONG_T) grVcodecHWLock.pvHandle,
						     pmem_user_v2p_video((VAL_ULONG_T) rHWLock.
									 pvHandle), current->pid,
						     u4TimeInterval, rHWLock.u4TimeoutMs);

						MODULE_MFV_LOGD("VCODEC_LOCKHW, LockInstance = 0x%lx, CurrentInstance = 0x%lx, CurrentTID = %d, rLockedTime(s, us) = %d, %d, rCurTime(s, us) = %d, %d\n",
						     (VAL_ULONG_T) grVcodecHWLock.pvHandle,
						     pmem_user_v2p_video((VAL_ULONG_T) rHWLock.
									 pvHandle), current->pid,
						     grVcodecHWLock.rLockedTime.u4Sec,
						...
						++gLockTimeOutCount;
						if (gLockTimeOutCount > 30) {
							MODULE_MFV_LOGE("[ERROR] VCODEC_LOCKHW - ID %d  fail, someone locked HW over 30 times without timeout 0x%lx, %lx, 0x%lx, type:%d\n",
							     current->pid,
							     (VAL_ULONG_T) grVcodecHWLock.pvHandle,
							     pmem_user_v2p_video((VAL_ULONG_T)
										 rHWLock.pvHandle),
							...
							/* if (u4TimeInterval >= rHWLock.u4TimeoutMs) */
						{
							if (rHWLock.eDriverType ==
								...
								MODULE_MFV_LOGE("VCODEC_LOCKHW, Locked process(A) timeout, so release HW and let currnet process(B) to use HW\n");
								...
								ret =
								    search_HWLockSlot_ByHandle(0,
											       ...
											       pvHandle);
								spin_unlock_irqrestore
								    (&OalHWContextLock, ulFlags);

								if (ret == -1) {
									MODULE_MFV_LOGE("[ERROR] VCODEC_LOCKHW, Locked process (A) - Instance 0x%lx fail, didn't call InitHWLock\n",
									     (VAL_ULONG_T)
									     grVcodecHWLock.
									     pvHandle);
									spin_unlock_irqrestore
								...
								if (u4Index == -1) {
									MODULE_MFV_LOGE("[ERROR] VCODEC_LOCKHW, Locked process - ID %d fail, didn't call InitHWLock\n",
									     current->pid);
									spin_unlock_irqrestore
									    (&HWLock,
									     ulFlagsHWLock);
					...
					if (VAL_TRUE == bLockedHW) {
						MODULE_MFV_LOGE("VCODEC_LOCKHW, Lock ok grVcodecHWLock.pvHandle = 0x%lx, va:%lx, type:%d",
						     (VAL_ULONG_T) grVcodecHWLock.pvHandle,
						     (VAL_ULONG_T) rHWLock.pvHandle,
						     rHWLock.eDriverType);
						gLockTimeOutCount = 0;
				...
				if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_ENC) {
					...
					if (ret == -1) {
						MODULE_MFV_LOGE("VCODEC_LOCKHW - ID %d  fail, didn't call InitHWLock\n",
						     current->pid);
						return -EFAULT;
					}
				}
			...
			if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
				...
				if (grVcodecHWLock.pvHandle ==
					...
					if (VAL_TRUE == gMMDFVFSMonitorStarts
						...
						if (_monitor_duration < MONITOR_DURATION_MS) {
							_diff = VdecDvfsStep();
							MODULE_MFV_LOGD("[VCODEC][MMDVFS_VDEC] @@ UNLOCK - lock time(%d ms, %d ms), cnt=%d, _monitor_duration=%d",
							...
							_perc =
							    (VAL_UINT32_T) (100 * gHWLockInterval /
									    _monitor_duration);
							MODULE_MFV_LOGE("[VCODEC][MMDVFS_VDEC] @@ UNLOCK - reset monitor duration (%d ms), percent: %d, (DROP_PERCENTAGE = %d, RAISE_PERCENTAGE = %d)",
					...
					MODULE_MFV_LOGE("[ERROR] VCODEC_UNLOCKHW, Not owner trying to unlock dec hardware 0x%lx\n",
					     pmem_user_v2p_video((VAL_ULONG_T) rHWLock.pvHandle));
					spin_unlock_irqrestore(&HWLock, ulFlagsHWLock);
					return -EFAULT;
				}
				...
				if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_ENC) {
					...
					if (grVcodecHWLock.pvHandle ==
						...
						/* [TODO] error handling */
						MODULE_MFV_LOGE("[ERROR] VCODEC_UNLOCKHW, Not owner trying to unlock enc hardware 0x%lx, pa:%lx, va:%lx type:%d\n",
						     (VAL_ULONG_T) grVcodecHWLock.pvHandle,
						     pmem_user_v2p_video((VAL_ULONG_T) rHWLock.
									 pvHandle),
						     (VAL_ULONG_T) rHWLock.pvHandle,
	...
	case VCODEC_WAITISR:
		{
			...
			if (val_isr.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
				...
				if (val_isr.eDriverType == VAL_DRIVER_TYPE_H264_DEC
				    || val_isr.eDriverType == VAL_DRIVER_TYPE_VP8_DEC) {
					MODULE_MFV_LOGD("VCODEC_WAITISR_H264_DEC + tid = %d\n",
						 current->pid);
					spin_lock_irqsave(&OalHWContextLock, ulFlags);
					/* u4Index = search_HWLockSlot_ByHandle(0, pmem_user_v2p_video
					...
					if (u4Index == -1) {
						MODULE_MFV_LOGE("[ERROR] VCODEC_WAITISR Fail, handle = 0x%lx, tid = %d, index = -1\n",
						     pmem_user_v2p_video((unsigned long)val_isr.
									 pvHandle), current->pid);
						return -EFAULT;
					}

					MODULE_MFV_LOGD("VCODEC_WAITISR, index = %d, start wait VCODEC_WAITISR handle 0x%lx\n",
					     u4Index,
					     pmem_user_v2p_video((unsigned long)val_isr.pvHandle));

					/* spin_lock_irqsave(&HWLock, ulFlagsHWLock); */
					/* MODULE_MFV_LOGD("VCODEC_WAITISR, grVcodecHWLock.pvHandle = 0x%lx\n", */
					/* (VAL_ULONG_T)grVcodecHWLock.pvHandle); */
					/* //spin_unlock_irqrestore(&HWLock, ulFlagsHWLock); */

					spin_lock_irqsave(&OalHWContextLock, ulFlags);
					oal_hw_context[u4Index].IsrEvent.u4TimeoutMs =
					    val_isr.u4TimeoutMs;
					spin_unlock_irqrestore(&OalHWContextLock, ulFlags);
					eValRet =
					    eVideoWaitEvent(&oal_hw_context[u4Index].IsrEvent,
							    sizeof(VAL_EVENT_T));
					...
					if (VAL_RESULT_INVALID_ISR == eValRet) {
						if (DecIsrEvent.u4TimeoutMs == 1
							...
							MODULE_MFV_LOGE("[ERROR] VCODEC_WAITISR, VP8 intend to let first WAITISE fail with timeOut value = %d (%d)\n",
							     DecIsrEvent.u4TimeoutMs,
							     gu4VDecIRQCount);
						} else {
							MODULE_MFV_LOGE("[ERROR] VCODEC_WAITISR, WAIT_ISR_CMD TimeOut (%d)\n",
							     gu4VDecIRQCount);
						}

						spin_lock_irqsave(&OalHWContextLock, ulFlags);
						*((volatile VAL_UINT32_T *)oal_hw_context[u4Index].
						...
						*((volatile VAL_UINT32_T *)oal_hw_context[u4Index].
						...
						/* Unlock */
						spin_lock_irqsave(&HWLock, ulFlagsHWLock);
						/* use search by thread ID to avoid handle already freed in
						user space that will cause KE! */
						for (i = 0; i < grVcodecHWLock.u4VCodecThreadNum;
						     i++) {
							if (grVcodecHWLock.u4VCodecThreadID[i] ==
							    current->pid) {
								/* if (grVcodecHWLock.pvHandle == */
								/* (VAL_VOID_T *)pmem_user_v2p_video
								((unsigned long)val_isr.pvHandle)) //normal case */
								for (j = 0;
								...
								if (VAL_RESULT_NO_ERROR !=
								    eValHWLockRet) {
									MODULE_MFV_LOGE("[ERROR] VCODEC_WAITISR, set HWLockEvent error\n");
								}
								break;
							}
						}
					...
					eValRet =
					    eVideoWaitEvent(&DecIsrEvent, sizeof(VAL_EVENT_T));
					if (VAL_RESULT_INVALID_ISR == eValRet) {
						return -2;
					} else if (VAL_RESULT_RESTARTSYS == eValRet) {
				...
				if (val_isr.eDriverType == VAL_DRIVER_TYPE_MP4_ENC)	{ /* Hybrid */
					...
					if (VAL_RESULT_INVALID_ISR == eValRet) {
						MODULE_MFV_LOGE("[ERROR] VCODEC_WAITISR, WAIT_ISR_CMD TimeOut\n");
					...
					if (bLockedHW == VAL_FALSE) {
						MODULE_MFV_LOGE("[ERROR] VCODEC_WAITISR, DO NOT have enc HWLock, so return fail pa:%lx, va:%lx\n",
						     pmem_user_v2p_video((VAL_ULONG_T) val_isr.
									 pvHandle),
	...
	case VCODEC_INITHWLOCK:
		{
			...
			if (NULL == context) {
				spin_unlock_irqrestore(&OalHWContextLock, ulFlags);
				MODULE_MFV_LOGE("get context fail\n");
				return -EFAULT;
			}
			...
			if (hwoal_reg.u4NumOfRegister != 0) {
				...
				if (0 == context->pa_Oal_HW_mem_reg) {
					spin_unlock_irqrestore(&OalHWContextLock, ulFlags);
					MODULE_MFV_LOGE("get Oal_HW_mem_reg fail\n");
					return -EFAULT;
				}
			}
			pa_u4HWIsCompleted =
			...
			if (0 == pa_u4HWIsCompleted) {
				spin_unlock_irqrestore(&OalHWContextLock, ulFlags);
				MODULE_MFV_LOGE("get pa_u4HWIsCompleted fail\n");
				return -EFAULT;
			}
			pa_u4HWIsTimeout =
			    pmem_user_v2p_video((unsigned long)
			...
			if (0 == pa_u4HWIsTimeout) {
				spin_unlock_irqrestore(&OalHWContextLock, ulFlags);
				MODULE_MFV_LOGE("get pa_u4HWIsTimeout fail\n");
				return -EFAULT;
			}
			...
			/* ret = copy_from_user(&oal_mem_status[0], ((VAL_VCODEC_OAL_HW_REGISTER_T *)
			...
			if (hwoal_reg.u4NumOfRegister != 0) {
				...
				mutex_unlock(&NonCacheMemoryListLock);
				if (NULL == pu4TempKVA) {
					MODULE_MFV_LOGE("search pu4TempKVA fail\n");
					return -EFAULT;
				}
				spin_lock_irqsave(&OalHWContextLock, ulFlags);
				context->kva_Oal_HW_mem_reg = pu4TempKVA;
				MODULE_MFV_LOGD("[VCODEC] context->ua = 0x%lx  pa_Oal_HW_mem_reg = 0x%lx\n",
			...
			mutex_unlock(&NonCacheMemoryListLock);
			if (NULL == kva_TempReg) {
				MODULE_MFV_LOGE("search kva_TempReg fail\n");
				return -EFAULT;
			}
			spin_lock_irqsave(&OalHWContextLock, ulFlags);
			...
			MODULE_MFV_LOGD("[VCODEC] kva_TempReg = 0x%lx, kva_u4HWIsCompleted = 0x%lx, kva_u4HWIsTimeout = 0x%lx\n",
			...
			for (i = 0; i < hwoal_reg.u4NumOfRegister; i++) {
				...
				addr_pa =
				    pmem_user_v2p_video((unsigned long)oal_mem_status[i].
							u4ReadAddr);
				spin_unlock_irqrestore(&OalHWContextLock, ulFlags);
				if (0 == addr_pa) {
					MODULE_MFV_LOGE("get addr_pa fail\n");
					return -EFAULT;
				}
...
static long vcodec_unlocked_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	MODULE_MFV_LOGE("vcodec_unlocked_compat_ioctl: 0x%x\n", cmd);
	switch (cmd) {
	case VCODEC_ALLOC_NON_CACHE_BUFFER:
	case VCODEC_FREE_NON_CACHE_BUFFER:
		{
	...
	case VCODEC_INITHWLOCK:
		{
			...
			for (i = 0; i < u4NumOfRegister; i++) {
				...
				pHWStatus[i].u4ReadAddr, u, pHWStatus[i].u4ReadData); */
			}
...
static int vcodec_probe(struct platform_device *dev)
{
	...
	/* if (request_irq(MT_VDEC_IRQ_ID ,
	(irq_handler_t)video_intr_dlr, IRQF_TRIGGER_LOW, VCODEC_DEVNAME, NULL) < 0) */
	if (request_irq
	...
	/* if (request_irq(MT_VENC_IRQ_ID ,
	(irq_handler_t)video_intr_dlr2, IRQF_TRIGGER_LOW, VCODEC_DEVNAME, NULL) < 0) */
	if (request_irq
...
static int __init vcodec_driver_init(void)
{
...
#ifdef CONFIG_ARCH_MT6570
	{
		...
		if (node) {
			...
			MODULE_MFV_LOGE("[VCODEC][DeviceTree] VDEC Not supported, use hard-code value");
...

	{
		...
		MODULE_MFV_LOGE("[VCODEC][DeviceTree] KVA_VENC_BASE(0x%lx), KVA_VDEC_BASE(0x%lx), KVA_VDEC_GCON_BASE(0x%lx)",
	...
	for (i = 0; i < VCODEC_MULTIPLE_INSTANCE_NUM; i++) {
		...
		if (VAL_RESULT_NO_ERROR != eValRet)
			MODULE_MFV_LOGE("[MFV][ERROR] create isr event error\n");
	...
	if (VAL_RESULT_NO_ERROR != eValHWLockRet)
		MODULE_MFV_LOGE("[VCODEC][ERROR] create hwlock event error\n");
	...
	m4u_register_fault_callback(M4U_PORT_VENC_CDMA_VDEC_CDMA, vcodec_m4u_fault_callback,
				    (void *)&grVcodecHWLock);
	m4u_register_fault_callback(M4U_PORT_VENC_BSDMA_VDEC_POST0, vcodec_m4u_fault_callback,
				    (void *)&grVcodecHWLock);
...
static void __exit vcodec_driver_exit(void)
{
	...
	for (i = 0; i < OALMEM_STATUS_NUM; i++) {
		...
		if (VAL_RESULT_NO_ERROR != eValRet)
			MODULE_MFV_LOGE("[VCODEC][ERROR] close isr event error\n");
	}

	eValHWLockRet = eVideoCloseEvent(&HWLockEvent, sizeof(VAL_EVENT_T));
	if (VAL_RESULT_NO_ERROR != eValHWLockRet)
		MODULE_MFV_LOGE("[VCODEC][ERROR] close hwlock event error\n");
