		...
		if (oal_hw_context[i].u4VCodecThreadNum != VCODEC_THREAD_MAX_NUM) {
			for (j = 0; j < oal_hw_context[i].u4VCodecThreadNum; j++) {
				if (oal_hw_context[i].u4VCodecThreadID[j] == curr_tid) {
					pr_info("[VCODEC][search_HWLockSlot_ByTID]\n");
					pr_info("Lookup curr HW Locker is ObjId %d in index%d\n",
						 curr_tid, i);
					return i;
				}
			}
		...
		if (oal_hw_context[i].u4VCodecThreadNum != VCODEC_THREAD_MAX_NUM) {
			for (j = 0; j < oal_hw_context[i].u4VCodecThreadNum; j++) {
				if (oal_hw_context[i].u4VCodecThreadID[j] == current->pid) {
					...
					pr_info("[VCODEC][setCurr_HWLockSlot] setCurr %d Slot\n",
						 i);
					return &oal_hw_context[i];
				}
			}
							  ...
							  VAL_UINT32_T *a_prIndex)
{
	...
	for (i = 0; i < VCODEC_MULTIPLE_INSTANCE_NUM; i++) {
		if (oal_hw_context[i].u4VCodecThreadNum != VCODEC_THREAD_MAX_NUM) {
			for (j = 0; j < oal_hw_context[i].u4VCodecThreadNum; j++) {
				pr_info("[VCODEC][setCurr_HWLockSlot_Thread_ID]\n");
				pr_info("Dump curr slot %d, ThreadID[%d] = %d\n",
...
struct VAL_VCODEC_OAL_HW_CONTEXT_T *freeCurr_HWLockSlot(VAL_ULONG_T ulpa)
{
	...
	for (i = 0; i < VCODEC_MULTIPLE_INSTANCE_NUM; i++) {
		if (oal_hw_context[i].ObjId == ulpa) {
			...
			pr_info("[VCODEC] freeCurr_HWLockSlot %d Slot\n", i);
			return &oal_hw_context[i];
		}
	}
			    ...
			    VAL_UINT32_T a_u4VCodecThreadNum, VAL_UINT32_T *a_pu4VCodecThreadID)
{
	...
	pr_info("[VCODEC] Add_NonCacheMemoryList +, KVA = 0x%lx, KPA = 0x%lx, Size = 0x%lx\n",
	...
	pr_info("[VCODEC] Add_NonCacheMemoryList -\n");
}
...
void Free_NonCacheMemoryList(VAL_ULONG_T a_ulKVA, VAL_ULONG_T a_ulKPA)
{
	...
	pr_info("[VCODEC] Free_NonCacheMemoryList +, KVA = 0x%lx, KPA = 0x%lx\n", a_ulKVA,
		 a_ulKPA);

	for (u4I = 0; u4I < VCODEC_MULTIPLE_INSTANCE_NUM_x_10; u4I++) {
		if ((grNonCacheMemoryList[u4I].ulKVA == a_ulKVA)
		    && (grNonCacheMemoryList[u4I].ulKPA == a_ulKPA)) {
			pr_info("[VCODEC] Free Free_NonCacheMemoryList index = %d\n", u4I);
	...
	pr_info("[VCODEC]Free_NonCacheMemoryList -\n");
}
...
void Force_Free_NonCacheMemoryList(VAL_UINT32_T a_u4Tid)
{
	...
	pr_info("[VCODEC] Force_Free_NonCacheMemoryList +, curr_id = %d", a_u4Tid);
...
VAL_ULONG_T Search_NonCacheMemoryList_By_KPA(VAL_ULONG_T a_ulKPA)
{
	...
	pr_info("[VCODEC] Search_NonCacheMemoryList_By_KPA +, KPA = 0x%lx, ulVA_Offset = 0x%lx\n",
		 a_ulKPA, ulVA_Offset);

	for (u4I = 0; u4I < VCODEC_MULTIPLE_INSTANCE_NUM_x_10; u4I++) {
		if (grNonCacheMemoryList[u4I].ulKPA == (a_ulKPA - ulVA_Offset)) {
			pr_info("[VCODEC] Find Search_NonCacheMemoryList_By_KPA index = %d\n",
				 u4I);
			break;
		}
	}
	...
	pr_info("[VCODEC] Search_NonCacheMemoryList_By_KPA, ulVA = 0x%lx -\n",
