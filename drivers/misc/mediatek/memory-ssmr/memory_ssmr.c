	...
	if (region->is_unmapping) {
		...
		if (ret_map < 0) {
			...
			aee_kernel_warning_api(__FILE__, __LINE__,
				...
				region_page_va, region->alloc_pages << PAGE_SHIFT);
...
static bool is_valid_feature(unsigned int feat)
{
	...
	if (SSMR_INVALID_FEATURE(feat)) {
		pr_err("%s: invalid feature_type: %d\n",
			__func__, feat);
		return false;
	}
	...
	if (SSMR_INVALID_REGION(region_type)) {
		pr_err("%s: invalid region_type: %d\n",
		...
		unsigned int feat)
{
	...
	return _ssmr_offline_internal(pa, size,
		is_64bit ? UPPER_LIMIT64 : UPPER_LIMIT32, _ssmr_feats[feat].region);
}
...
static struct file_operations ssmr_cma_fops = {
	.owner =    THIS_MODULE,
...
static int memory_ssmr_show(struct seq_file *m, void *v)
{
	...
	for (; i < __MAX_NR_SSMRSUBS; i++) {
		unsigned long region_pa = (unsigned long) page_to_phys(_ssmregs[i].page);
...
static int __init memory_ssmr_init_region(char *name, u64 size, struct SSMR_Region *region,
		struct file_operations *entry_fops)
{
	...
	if (entry_fops) {
		...
		if (!procfs_entry) {
			pr_err("Failed to create procfs ssmr_region file\n");
			return -1;
		}
	}
