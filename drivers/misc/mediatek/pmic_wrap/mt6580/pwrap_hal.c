...
static inline void pwrap_dump_ap_register(void)
{
	...
	PWRAPREG("clock control 0x4:0x%x, 0x24:0x%x, 0x224:0x%x\n",
		 WRAP_RD32(topckgen_base + 0x4),
		 WRAP_RD32(topckgen_base + 0x24),
		 WRAP_RD32(topckgen_base + 0x224));
...
static irqreturn_t mt_pmic_wrap_irq(int irqno, void *dev_id)
{
	...
	if (g_wrap_wdt_irq_count == 10 || g_case_flag == 1)
		aee_kernel_warning("WRAPPER:NOTICE", "WRAP");
