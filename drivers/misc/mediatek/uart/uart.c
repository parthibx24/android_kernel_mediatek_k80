...
/*---------------------------------------------------------------------------*/
ssize_t
mtk_uart_attr_show(struct kobject *kobj, struct attribute *attr, char *buffer)
{
	struct mtuart_entry *entry = NULL;

	entry = container_of(attr, struct mtuart_entry, attr);
	return entry->show(kobj, buffer);
}

/*---------------------------------------------------------------------------*/
ssize_t
mtk_uart_attr_store(struct kobject *kobj, struct attribute *attr,
...
/*---------------------------------------------------------------------------*/
ssize_t
mtk_uart_debug_store(struct kobject *kobj, const char *buffer, size_t size)
{
#if (UART_NR < 5)
	...
	res = sscanf(buffer, "0x%x 0x%x 0x%x 0x%x 0x%x", &a, &b, &c, &d, &e);
...
{
	struct mtuart_sysobj *obj = NULL;

	obj = container_of(kobj, struct mtuart_sysobj, kobj);
...
/*---------------------------------------------------------------------------*/
ssize_t
mtk_uart_sysrq_store(struct kobject *kobj, const char *buffer, size_t size)
{
	struct mtuart_sysobj *obj = NULL;
	int a;
...
/*---------------------------------------------------------------------------*/
ssize_t
mtk_uart_vffsz_store(struct kobject *kobj, const char *buffer, size_t size)
{
#if defined(ENABLE_VFIFO)
	struct mtuart_sysobj *obj = NULL;
	int idx, sz;
...
/*---------------------------------------------------------------------------*/
ssize_t
mtk_uart_conse_store(struct kobject *kobj, const char *buffer, size_t size)
{
	struct mtuart_sysobj *obj = NULL;
	int enable;
...
/*---------------------------------------------------------------------------*/
ssize_t
mtk_uart_vff_en_store(struct kobject *kobj, const char *buffer, size_t size)
{
#if (UART_NR < 5)
	...
	return size;
#else
	int u1_tx, u1_rx, u2_tx, u2_rx, u3_tx, u3_rx;
	int u4_tx, u4_rx, u5_tx, u5_rx;
	struct mtk_uart_setting *uart_setting;
...
/*---------------------------------------------------------------------------*/
ssize_t
mtk_uart_lsr_status_store(struct kobject *kobj,
	const char *buffer, size_t size)
{
#if (UART_NR < 5)
...
static void uart_mem_dump(int nport, void *start_addr, int len)
{
	...
	if (curr_p == NULL) {
		pr_info("[UART%d-DUMP]NULL point to dump!\n", nport);
		return;
	}
	if (len == 0) {
		pr_info("[UART%d-DUMP]Not need to dump\n", nport);
		return;
	}
	...
	if (tail_num > 0) {
		...
		pr_debug("[UART%d-DUMP]%03X: %08X %08X %08X %08X\n",
		       nport, i * 16, *curr_p, *(curr_p + 1), *(curr_p + 2),
		       *(curr_p + 3));
	}
}
...
void
update_history_bulk(char is_tx, int nport, unsigned char *chars, int count)
{
	int room;
	struct uart_history_data *x_history;
...
ssize_t mtk_uart_history_show(struct kobject *kobj, char *buffer)
{
	...
	len = scnprintf(ptr, remain, "tx(%d):%d; rx(%d):%d\n",
			tx_history.index, tx_history.offset, rx_history.index,
			rx_history.offset);
	ptr += len;
	remain -= len;
...
/*---------------------------------------------------------------------------*/
ssize_t
mtk_uart_history_store(struct kobject *kobj, const char *buffer, size_t size)
{
...
void
adjust_kernel_cmd_line_setting_for_console(char *u_boot_cmd_line,
...
/*---------------------------------------------------------------------------*/
static void
mtk_uart_console_write(struct console *co, const char *s, unsigned int count)
{
	/* Notice:
...
static int __init mtk_uart_console_setup(struct console *co, char *options)
{
	...
	pr_debug("[UART]set_option port(%d)baud(%d) parity(%c) bits(%d) flow(%c) - ret(%d)\n",
...
/*---------------------------------------------------------------------------*/
static struct mtk_uart_vfifo
*mtk_uart_vfifo_alloc(struct mtk_uart *uart, int type)
{
...
/*---------------------------------------------------------------------------*/
static void
mtk_uart_vfifo_free(struct mtk_uart *uart, struct mtk_uart_vfifo *vfifo)
{
	unsigned long flags;
...
mtk_uart_vfifo_get_trig(struct mtk_uart *uart, struct mtk_uart_vfifo *vfifo)
{
	return vfifo->trig;
}
...
#endif
	vfifo = container_of(hrt, struct mtk_uart_vfifo, flush);
	dma = (struct mtk_uart_dma *)vfifo->dma;
	uart = dma->uart;
...
static irqreturn_t mtk_vfifo_irq_handler(int irq, void *dev_id)
{
	...
	if (!vfifo) {
		pr_info("mtk_vfifo_irq_handler: vfifo is NULL\n");
		return IRQ_NONE;
	}
	if (!vfifo->dma) {
		pr_info("mtk_vfifo_irq_handler: dma is NULL\n");
		return IRQ_NONE;
	}
...
/*---------------------------------------------------------------------------*/
static int
mtk_uart_dma_alloc(struct mtk_uart *uart, struct mtk_uart_dma *dma,
	int mode, struct mtk_uart_vfifo *vfifo)
{
	int ret = 0;
...
#if defined(ENABLE_VFIFO)
	if (uart->tx_mode == UART_TX_VFIFO_DMA) {
		/*1. UART_IER_ETBEI can't be */
		/* disabled or zero data appears in TX */
 ...
 */
static void
mtk_uart_set_termios(struct uart_port *port,
...
static void
mtk_uart_power_mgnt(struct uart_port *port, unsigned int state,
...
static int
mtk_uart_verify_port(struct uart_port *port, struct serial_struct *ser)
{
...
static int
mtk_uart_ioctl(struct uart_port *port, unsigned int cmd, unsigned long arg)
{
#if defined(ENABLE_DEBUG)
	struct mtk_uart *uart = (struct mtk_uart *)port;
...
/* !defined(CONFIG_MTK_LEGACY) && !defined(CONFIG_FPGA_EARLY_PORTING) */
	...
	if (err) {
		mtk_uart_vfifo_delete(uart);
		pr_info("create vff buffer fail:%d\n", err);
	}
#endif
	return err;
}
...
static int mtk_uart_pm_restore(struct device *device)
{
	...
	if (uart_freeze_enable[port_idx]) {
		mtk_uart_switch_to_tx(uart);
		ret = uart_resume_port(&mtk_uart_drv, &uart->port);
		pr_info("[%s] uart (%p) base: 0x%lx, nport %d, dma mode: %d\n",
			__func__, uart, uart->base,
			uart->nport, uart->dma_mode);
	}
...
static int mtk_uart_pm_restore_noirq(struct device *device)
{
	...
	if (!uart || !uart->setting) {
		pr_info("[%s] uart or uart->setting is null!!\n", __func__);
		return 0;
	}
	mtk_uart_fifo_set_trig(uart, uart->tx_trig, uart->rx_trig);
	...
	for (uart_idx = 0; uart_idx < UART_NR; uart_idx++) {
		...
		while (!(UART_READ32(UART_SLEEP_ACK) & UART_CLK_OFF_ACK)) {
			if (i++ >= WAIT_UART_ACK_TIMES) {
				...
				pr_info_ratelimited("[UART]CANNOT GET UART[%d] SLEEP ACK\n",
						uart_idx);
				/* dump_uart_reg(); */
				return -EBUSY;
			}
...
int request_uart_to_wakeup(void)
{
	...
	for (uart_idx = 0; uart_idx < UART_NR; uart_idx++) {
		...
		while ((UART_READ32(UART_SLEEP_ACK) & UART_CLK_OFF_ACK)) {
			if (i++ >= WAIT_UART_ACK_TIMES) {
				reg_sync_writel(val1, UART_SLEEP_REQ);
				pr_info("[UART]CANNOT GET UART[%d] WAKE ACK\n",
					uart_idx);
				/* dump_uart_reg(); */
				return -EBUSY;
			}
