...
static struct lhd_limit_data current_core;
static struct lhd_limit_data current_freq;
static struct lhd_limit_data core_set[LHD_MAX_KIR];
static struct lhd_limit_data freq_set[LHD_MAX_KIR];
...
{
	struct lhd_limit_data final_core;
	int retval = 0;
	int i;
...
EXPORT_SYMBOL(lhd_update_userlimit_cpu_freq);
...
{
	int i = 0, data;
	struct lhd_limit_data core_limit;
	...
	if (i < arg_num)
		...
		lhd_update_userlimit_cpu_core(LHD_KIR_PERF, nr_ppm_clusters, &core_limit);
...
static int perfmgr_perfserv_core_show(struct seq_file *m, void *v)
{
	seq_printf(m, "min:%d max:%d\n",
		core_set[LHD_KIR_PERF].min, core_set[LHD_KIR_PERF].max);
...
{
	int i = 0, data;
	struct lhd_limit_data freq_limit;
	unsigned int arg_num = nr_ppm_clusters * 2; /* for min and max */
	char *tok, *tmp;
	char *buf = lbc_copy_from_user_for_proc(ubuf, cnt);
	...
	if (i < arg_num)
		...
		lhd_update_userlimit_cpu_freq(LHD_KIR_PERF, nr_ppm_clusters, &freq_limit);
...
{
	seq_printf(m, "min:%d max:%d\n",
		freq_set[LHD_KIR_PERF].min, freq_set[LHD_KIR_PERF].max);
