...
/*******************************************/
int cpu_ctrl_update_userlimit_cpu_freq(int kicker, int num_cluster
...
EXPORT_SYMBOL(cpu_ctrl_update_userlimit_cpu_freq);
		...
		, const char __user *ubuf, size_t cnt, loff_t *pos)
{
	...
	if (i < arg_num) {
		...
		cpu_ctrl_update_userlimit_cpu_freq(CPU_KIR_PERF
		...
		const char __user *ubuf, size_t cnt, loff_t *pos)
{
	...
	if (i < arg_num)
		...
		cpu_ctrl_update_userlimit_cpu_freq(CPU_KIR_BOOT,
				perfmgr_clusters, freq_limit);
