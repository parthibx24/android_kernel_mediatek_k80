...
#if !defined(CONFIG_MTK_LEGACY)
/* GPIO Pin control*/
struct pinctrl *cam_pinctrl = NULL;
struct pinctrl_state *cam1_gpio_serial = NULL;
struct pinctrl_state *cam1_gpio_mipi = NULL;
struct pinctrl_state *cam_mclk1_enable = NULL;
struct pinctrl_state *cam_mclk1_disable = NULL;
struct pinctrl_state *cam_mclk2_enable = NULL;
struct pinctrl_state *cam_mclk2_disable = NULL;


int mtkcam_pin_mux_init(struct platform_device *pdev)
{
	int ret = 0;

	cam_pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(cam_pinctrl)) {
		dev_err(&pdev->dev, "Cannot find camera pinctrl!");
		ret = PTR_ERR(cam_pinctrl);
	}

	cam1_gpio_serial = pinctrl_lookup_state(cam_pinctrl, "cam1_gpio_serial");
	if (IS_ERR(cam1_gpio_serial)) {
		ret = PTR_ERR(cam1_gpio_serial);
		pr_debug("%s : pinctrl err, cam1_gpio_serial\n", __func__);
	}

	cam1_gpio_mipi = pinctrl_lookup_state(cam_pinctrl, "cam1_gpio_mipi");
	if (IS_ERR(cam1_gpio_mipi)) {
		ret = PTR_ERR(cam1_gpio_mipi);
		pr_debug("%s : pinctrl err, cam1_gpio_mipi\n", __func__);
	}


	cam_mclk1_enable = pinctrl_lookup_state(cam_pinctrl, "cam_mclk1_enable");
	if (IS_ERR(cam_mclk1_enable)) {
		ret = PTR_ERR(cam_mclk1_enable);
		pr_debug("%s : pinctrl err, cam_mclk1_enable\n", __func__);
	}

	cam_mclk1_disable = pinctrl_lookup_state(cam_pinctrl, "cam_mclk1_disable");
	if (IS_ERR(cam_mclk1_disable)) {
		ret = PTR_ERR(cam_mclk1_disable);
		pr_debug("%s : pinctrl err, cam_mclk1_disable\n", __func__);
	}

	cam_mclk2_enable = pinctrl_lookup_state(cam_pinctrl, "cam_mclk2_enable");
	if (IS_ERR(cam_mclk2_enable)) {
		ret = PTR_ERR(cam_mclk2_enable);
		pr_debug("%s : pinctrl err, cam_mclk2_enable\n", __func__);
	}

	cam_mclk2_disable = pinctrl_lookup_state(cam_pinctrl, "cam_mclk2_disable");
	if (IS_ERR(cam_mclk2_disable)) {
		ret = PTR_ERR(cam_mclk2_disable);
		pr_debug("%s : pinctrl err, cam_mclk2_disable\n", __func__);
	}
	return ret;
}


void ISP_MCLK1_GPIO_EN(bool En)
{
	if(En)
	{
		pinctrl_select_state(cam_pinctrl, cam_mclk1_enable);

	}
	else
	{
		pinctrl_select_state(cam_pinctrl, cam_mclk1_disable);

	}
}

void ISP_MCLK2_GPIO_EN(bool En)
{
	if(En)
	{
		pinctrl_select_state(cam_pinctrl, cam_mclk2_enable);
	}
	else
	{
		pinctrl_select_state(cam_pinctrl, cam_mclk2_disable);
	}
}

#else

#ifndef GPIO_CAM_CLK0
#define GPIO_CAM_CLK0          (GPIO72 | 0x80000000)
#define GPIO_CAM_CLK0_M_GPIO 	 GPIO_MODE_00
#define GPIO_CAM_CLK0_M_CAM_CLK0  GPIO_MODE_01
#endif


#ifndef GPIO_CAM_CLK1
#define GPIO_CAM_CLK1          (GPIO75 | 0x80000000)
#define GPIO_CAM_CLK1_M_GPIO 	 GPIO_MODE_00
#define GPIO_CAM_CLK1_M_CAM_CLK1  GPIO_MODE_01
#endif
...
{
	if (En) {
		mt_set_gpio_mode(GPIO_CAM_CLK0, GPIO_CAM_CLK0_M_CAM_CLK0);

	} else {
		mt_set_gpio_mode(GPIO_CAM_CLK0, GPIO_CAM_CLK0_M_GPIO);
...
{
	if (En) {
		mt_set_gpio_mode(GPIO_CAM_CLK1, GPIO_CAM_CLK1_M_CAM_CLK1);

	} else {
		mt_set_gpio_mode(GPIO_CAM_CLK1, GPIO_CAM_CLK1_M_GPIO);
...
********************************************************************************/
int _kdCISModulePowerOn(enum CAMERA_DUAL_CAMERA_SENSOR_ENUM SensorIdx,
				char *currSensorName, BOOL On, char *mode_name)
{
	int ret = -1;
...
struct platform_device *camerahw_platform_device;
...
void checkPowerBeforeClose(char *mode_name)
{
	int i = 0;
	int j = 0;
	enum CAMERA_DUAL_CAMERA_SENSOR_ENUM SensorIdx = DUAL_CAMERA_NONE_SENSOR;

	PK_DBG("cntPWROnMain:%d, cntPWROnSub:%d\n", cntPWROnMain, cntPWROnSub);

	for (i = 0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		SensorIdx = g_invokeSocketIdx[i];
		if (SensorIdx == DUAL_CAMERA_MAIN_SENSOR) {
			for (j = 0; j < cntPWROnMain; j++)
				kdCISModulePowerOn(SensorIdx, (char *)g_invokeSensorNameStr[i],
						   false, mode_name);
		} else if (SensorIdx == DUAL_CAMERA_SUB_SENSOR) {
			for (j = 0; j < cntPWROnSub; j++)
				kdCISModulePowerOn(SensorIdx, (char *)g_invokeSensorNameStr[i],
						   false, mode_name);
		}
	}
	...
	for (i = (KDIMGSENSOR_MAX_INVOKE_DRIVERS - 1); i >= KDIMGSENSOR_INVOKE_DRIVER_0; i--) {
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			if (0 != (g_CurrentSensorIdx & g_invokeSocketIdx[i])) {
				...
				/* turn on power */
				ret =
				    _kdCISModulePowerOn((enum CAMERA_DUAL_CAMERA_SENSOR_ENUM)
							g_invokeSocketIdx[i],
...
static inline int adopt_CAMERA_HW_GetResolution(void *pBuf)
{
	...
	pRes[0] = (MSDK_SENSOR_RESOLUTION_INFO_STRUCT* )kmalloc(sizeof(MSDK_SENSOR_RESOLUTION_INFO_STRUCT), GFP_KERNEL);
	if (pRes[0] == NULL) {
		PK_ERR(" ioctl allocate mem failed\n");
		return -ENOMEM;
	}
	pRes[1] = (MSDK_SENSOR_RESOLUTION_INFO_STRUCT* )kmalloc(sizeof(MSDK_SENSOR_RESOLUTION_INFO_STRUCT), GFP_KERNEL);
	if (pRes[1] == NULL) {
		kfree(pRes[0]);
		PK_ERR(" ioctl allocate mem failed\n");
		return -ENOMEM;
...
static inline int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	PK_DBG("[CAMERA_HW][Resolution] 0x%x\n", (MUINT32) pSensorGetInfo->pSensorResolution);
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	struct ACDK_SENSOR_GROUP_INFO_STRUCT *pSensorGroupInfo = NULL;
	...
	case SENSOR_FEATURE_GET_GROUP_INFO:
		...
		pSensorGroupInfo = (struct ACDK_SENSOR_GROUP_INFO_STRUCT *) pFeaturePara;
    ...
    switch (pFeatureCtrl->FeatureId)
    {
		...
		case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
			...
			*(pFeaturePara_64 + 1) = (uintptr_t)pValue;
				PK_DBG("1[CAMERA_HW] %p %p %p\n",(void*)(uintptr_t)(*(pFeaturePara_64+1)),(void*)pFeaturePara_64,(void*)(pValue));
				if (g_pSensorFunc) {
		...
		case SENSOR_FEATURE_AUTOTEST_CMD:
			...
			*(pFeaturePara_64 + 1) = (uintptr_t)pValue1;
				PK_DBG("[CAMERA_HW] %p %p %p\n",(void*)(uintptr_t)(*(pFeaturePara_64+1)),(void*)pFeaturePara_64,(void*)(pValue0));
				if (g_pSensorFunc) {
					ret = g_pSensorFunc->SensorFeatureControl(pFeatureCtrl->InvokeCamera, pFeatureCtrl->FeatureId, (unsigned char *)pFeaturePara, (unsigned int *) &FeatureParaLen);
				}
				else {
		...
		case SENSOR_FEATURE_GET_EV_AWB_REF:
			{
				...
				unsigned long long *pFeaturePara_64 = (unsigned long long *) pFeaturePara;
				MUINT32 * usr_ptr = (MUINT32 *)(uintptr_t)(*(pFeaturePara_64));
			...
			*(pFeaturePara_64) = (uintptr_t)pAeAwbRef;
				...
				if (copy_to_user((void __user *) usr_ptr, (void *)pAeAwbRef, sizeof(struct SENSOR_AE_AWB_REF_STRUCT))){
		...
		case SENSOR_FEATURE_GET_CROP_INFO:
			...
			*(pFeaturePara_64 + 1) = (uintptr_t)pCrop;
				...
				if (copy_to_user((void __user *) usr_ptr, (void *)pCrop , sizeof(struct SENSOR_WINSIZE_INFO_STRUCT))){
					PK_DBG("[CAMERA_HW]ERROR: copy_to_user fail \n");
				}
				kfree(pCrop);
		...
		case SENSOR_FEATURE_GET_VC_INFO:
			...
			*(pFeaturePara_64 + 1) = (uintptr_t)pVcInfo;
				...
				if (copy_to_user((void __user *) usr_ptr, (void *)pVcInfo , sizeof(struct SENSOR_VC_INFO_STRUCT))){
					PK_DBG("[CAMERA_HW]ERROR: copy_to_user fail \n");
				}
				kfree(pVcInfo);
			*(pFeaturePara_64 + 1) = (uintptr_t)usr_ptr;
		...
		case SENSOR_FEATURE_GET_PDAF_INFO:
			...
			*(pFeaturePara_64 + 1) = (uintptr_t)pPdInfo;
				...
				if (copy_to_user((void __user *) usr_ptr, (void *)pPdInfo , sizeof(struct SET_PD_BLOCK_INFO_T))){
					PK_DBG("[CAMERA_HW]ERROR: copy_to_user fail \n");
				}
				kfree(pPdInfo);
			*(pFeaturePara_64 + 1) = (uintptr_t)usr_ptr;
		...
		case SENSOR_FEATURE_GET_EXIF_INFO:
			...
			void *usr_ptr =  (void *)(uintptr_t) (*(pFeaturePara_64));
				...
				*(pFeaturePara_64) = (uintptr_t)pExif;
				...
				if (copy_to_user((void __user *) usr_ptr, (void *)pExif , sizeof(struct SENSOR_EXIF_INFO_STRUCT))){
					PK_DBG("[CAMERA_HW]ERROR: copy_to_user fail \n");
				}
				kfree(pExif);
			*(pFeaturePara_64) = (uintptr_t)usr_ptr;
		...
		case SENSOR_FEATURE_GET_SHUTTER_GAIN_AWB_GAIN:
			...
			*(pFeaturePara_64) = (uintptr_t)pCurAEAWB;
				...
				if (copy_to_user((void __user *) usr_ptr, (void *)pCurAEAWB , sizeof(struct SENSOR_AE_AWB_CUR_STRUCT))){
					PK_DBG("[CAMERA_HW]ERROR: copy_to_user fail \n");
				}
				kfree(pCurAEAWB);
			*(pFeaturePara_64) = (uintptr_t)usr_ptr;
			...
			*(pFeaturePara_64) = (uintptr_t)pDelayInfo;
				...
				if (copy_to_user((void __user *) usr_ptr, (void *)pDelayInfo , sizeof(struct SENSOR_DELAY_INFO_STRUCT))){
					PK_DBG("[CAMERA_HW]ERROR: copy_to_user fail \n");
				}
				kfree(pDelayInfo);
			*(pFeaturePara_64) = (uintptr_t)usr_ptr;
			...
			*(pFeaturePara_64) = (uintptr_t)pFlashInfo;
				...
				if (copy_to_user((void __user *) usr_ptr, (void *)pFlashInfo , sizeof(struct SENSOR_FLASHLIGHT_AE_INFO_STRUCT))){
					PK_DBG("[CAMERA_HW]ERROR: copy_to_user fail \n");
				}
				kfree(pFlashInfo);
			*(pFeaturePara_64) = (uintptr_t)usr_ptr;
			...
			void *usr_ptr = (void *)(uintptr_t)(*(pFeaturePara_64 + 1));
				...
				*(pFeaturePara_64 + 1) = (uintptr_t)pPdaf_data;//*(pFeaturePara_64 + 1) = (uintptr_t)pPdaf_data;
				...
				if (copy_to_user((void __user *) usr_ptr, (void *)pPdaf_data , (kal_uint32)(*(pFeaturePara_64+2)))){
					PK_DBG("[CAMERA_HW]ERROR: copy_to_user fail \n");
				}
				kfree(pPdaf_data);
			*(pFeaturePara_64 + 1) =(uintptr_t) usr_ptr;
...
#define CAM_PLL_48MHZ 2
#define CAM_PLL_52MHZ 4
...
static inline int kdSetSensorGpio(int *pBuf)
{
	...
	if (pSensorgpio->SensroInterfaceType == SENSORIF_SERIAL) {
		if (pSensorgpio->GpioEnable == 1) {
			pinctrl_select_state(cam_pinctrl, cam1_gpio_serial);

		} else {
			pinctrl_select_state(cam_pinctrl, cam1_gpio_mipi);
		}
	}
#endif
	return ret;
...
#if 0				/* parallel was not supported */
...
#endif
		{
			mt_set_gpio_mode(GPIO_CAMERA_RDP0_A_PIN, GPIO_CAMERA_RDP0_A_PIN_M_PAD_RDP0_A);	/* GPIO 35  */
			mt_set_gpio_mode(GPIO_CAMERA_RDN0_A_PIN, GPIO_CAMERA_RDN0_A_PIN_M_PAD_RDN0_A);	/* GPIO 36  */
			mt_set_gpio_mode(GPIO_CAMERA_RDP1_A_PIN, GPIO_CAMERA_RDP1_A_PIN_M_PAD_RDP1_A);	/* GPIO 37  */
			mt_set_gpio_mode(GPIO_CAMERA_RDN1_A_PIN, GPIO_CAMERA_RDN1_A_PIN_M_PAD_RDN1_A);	/* GPIO 38  */
			mt_set_gpio_mode(GPIO_CAMERA_RCP_A_PIN, GPIO_CAMERA_RCP_A_PIN_M_PAD_RCP_A);	/* GPIO 39  */
			mt_set_gpio_mode(GPIO_CAMERA_RCN_A_PIN, GPIO_CAMERA_RCN_A_PIN_M_PAD_RCN_A);	/* GPIO 40  */
		}

	} else if (pSensorgpio->SensroInterfaceType == SENSORIF_SERIAL) {
		if (pSensorgpio->GpioEnable == 1) {
			mt_set_gpio_mode(GPIO_CAMERA_RDP0_A_PIN, GPIO_CAMERA_RDP0_A_PIN_M_CMCSK);	/* GPIO 35  */
			mt_set_gpio_mode(GPIO_CAMERA_RDN0_A_PIN, GPIO_CAMERA_RDN0_A_PIN_M_CMCSD);	/* GPIO 36  */
			mt_set_gpio_mode(GPIO_CAMERA_RDP1_A_PIN, GPIO_CAMERA_RDP1_A_PIN_M_CMCSD);	/* GPIO 37  */
			mt_set_gpio_mode(GPIO_CAMERA_RDN1_A_PIN, GPIO_CAMERA_RDN1_A_PIN_M_CMCSD);	/* GPIO 38  */
			mt_set_gpio_mode(GPIO_CAMERA_RCP_A_PIN, GPIO_CAMERA_RCP_A_PIN_M_CMCSD);	/* GPIO 39  */
...
static int compat_get_acdk_sensor_featurecontrol_struct(struct COMPAT_ACDK_SENSOR_FEATURECONTROL_STRUCT
							__user *data32,
...
static int compat_put_acdk_sensor_featurecontrol_struct(struct COMPAT_ACDK_SENSOR_FEATURECONTROL_STRUCT
							__user *data32,
							struct ACDK_SENSOR_FEATURECONTROL_STRUCT __user *
							data)
{
...
static int compat_get_acdk_sensor_resolution_info_struct(struct COMPAT_ACDK_SENSOR_PRESOLUTION_STRUCT
							 __user *data32,
...
static int compat_put_acdk_sensor_resolution_info_struct(struct COMPAT_ACDK_SENSOR_PRESOLUTION_STRUCT
							 __user *data32,
							 struct ACDK_SENSOR_RESOLUTION_INFO_STRUCT __user *
							 data)
{
	int err;
...
{
	struct device *sensor_device2 = NULL;
	UINT32 major;
...
EXPORT_SYMBOL(ISP_MCLK1_GPIO_EN);
EXPORT_SYMBOL(ISP_MCLK2_GPIO_EN);
