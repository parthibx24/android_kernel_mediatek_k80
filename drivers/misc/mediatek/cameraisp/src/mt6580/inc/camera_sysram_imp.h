...
#define LOG_MSG(fmt, arg...)    pr_debug(LOG_TAG \
"[%s]" fmt "\r\n", __func__, ##arg)
#define LOG_WRN(fmt, arg...)    pr_warn(LOG_TAG \
"[%s]WRN(%5d):" fmt "\r\n", __func__, __LINE__, ##arg)
#define LOG_ERR(fmt, arg...)    pr_err(LOG_TAG \
"[%s]ERR(%5d):" fmt "\r\n", __func__, __LINE__, ##arg)
#define LOG_DMP(fmt, arg...)    pr_err( \
LOG_TAG "" fmt, ##arg)
/* -------------------------------------------- */
...
/*  */
/* (78408)//(74496)
 * Always allocate max SYSRAM size because
 * there is no other user.
 * 78408: Max size used when format is RGB565.
 */
#define SYSRAM_USER_SIZE_VIDO   (SYSRAM_BASE_SIZE)
...
static struct SYSRAM_MEM_POOL_STRUCT
SysramMemPoolInfo[SYSRAM_MEM_BANK_AMOUNT] = {
	[SYSRAM_MEM_BANK_0] = {
