...
/* #define ISP_WR32(addr, data)
 * mt65xx_reg_sync_writel(data, addr)
...
/* 0x6100,the same with the value in
 * isp_reg.h and page-aligned
 */
#define ISP_REG_RANGE           (0x10000)
/* seninf driver */
/* the same with the value in
 * seninf_drv.cpp(chip-dependent)
 */
#define SENINF_BASE_ADDR     0x15008000
/* 0x800,the same with the value in
 * seninf_reg.h and page-aligned
 */
#define SENINF_REG_RANGE    (0x4000)
/* the same with the value in
 * seninf_drv.cpp(chip-dependent)
 * #define IMGSYS_CG_CLR0_ADDR  0x15000000
 */
/* 0x100,the same with the value in
 * seninf_drv.cpp and page-aligned
 * #define MMSYS_RANGE           (0x1000)
 */
/* the same with the value in
 * seninf_drv.cpp(chip-dependent)
 */
#define PLL_BASE_ADDR        0x10000000
/* 0x200,the same with the value in
 * seninf_drv.cpp and page-aligned
 */
 ...
 * seninf_drv.cpp and page-aligned
 */
#define MIPIRX_CONFIG_RANGE (0x100)
/* the same with the value in
 * seninf_drv.cpp(chip-dependent)
 */
#define MIPIRX_ANALOG_ADDR   0x10011000
#define MIPIRX_ANALOG_RANGE (0x4000)
/* the same with the value in
 * seninf_drv.cpp(chip-dependent)
 */
#define GPIO_BASE_ADDR       0x10017000
#define GPIO_RANGE          (0x1000)
#define EFUSE_BASE_ADDR      0x10206000
/* 0x400,the same with the value in
 * seninf_drv.cpp and page-aligned
 */
#define EFUSE_RANGE          (0x1000)
/* security concern */
...
#define ISP_MIPI_ANA_ADDR               0x10011000
#endif

#ifndef ISP_REG_ADDR_EN1
 ...
 * Vent@20121025: Remove ISP_IRQ_INTX_STATUS_BPC_ERR_ST.
 * From TH Wu's explanation,
 * this bit is not used as an error state anymore.
 */
...
static signed int *g_pTbl_RTBuf;
...
#if (LOG_CONSTRAINT_ADJ == 1)
static unsigned int g_log_def_constraint = 100;
#endif
...
unsigned int g_EnableClkCnt;
unsigned int g_TempAddr;

/* static ISP_DEQUE_BUF_INFO_STRUCT g_deque_buf = {0,{}};
 * Marked to remove build warning.WARNING
 */

unsigned long g_Flash_SpinLock;

#ifndef _rtbc_use_cq0c_
static unsigned int g_rtbcAAA;
static unsigned int g_EnqBuf;
static unsigned int g_DeqBuf;
static signed int g_rtbc_enq_dma = _rt_dma_max_;
...
static unsigned int g_sof_count;
static unsigned int g_start_time;
static unsigned int g_avg_frame_time;

/* allan */
static signed int g_sof_pass1done;

static dev_t g_IspDevNo;
static struct cdev *g_pIspCharDrv;
static struct class *g_pIspClass;

static signed int g_bPass1_On_In_Resume_TG1;

#define DEFAULT_PA 0x3773

static unsigned int g_oldImgoAddr = DEFAULT_PA;
static unsigned int g_newImgoAddr = DEFAULT_PA;
...
/* test flag */
#define ISP_KERNEL_MOTIFY_SINGLE_TEST
#ifdef ISP_KERNEL_MOTIFY_SINGLE_TEST
/*** Linux signal test ***/
...
/* this is bit of a trickery: SI_QUEUE is
...
/* t = find_task_by_pid_type(PIDTYPE_PID, g_pid);
...
#endif				/* ISP_KERNEL_MOTIFY_SINGLE_TEST */
 ...
 *
 *************************************************/
static inline unsigned int ISP_GetIRQState(unsigned int type, unsigned int stus)
{
	unsigned int ret;
	unsigned long flags;
 ...
 * get kernel time
 *************************************************/
static void ISP_GetTime(unsigned int *pSec, unsigned int *pUSec)
{
	ktime_t Time;
...
signed int ISP_DumpReg(void)
{
	...
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x008,
	ISP_RD32((void *)(ISP_ADDR + 0x008)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x00C,
	ISP_RD32((void *)(ISP_ADDR + 0x00C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x010,
	ISP_RD32((void *)(ISP_ADDR + 0x010)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x018,
	ISP_RD32((void *)(ISP_ADDR + 0x018)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x01C,
	...
	ISP_RD32((void *)(ISP_ADDR + 0x028)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x02C,
	ISP_RD32((void *)(ISP_ADDR + 0x02C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x030,
	ISP_RD32((void *)(ISP_ADDR + 0x030)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x034,
	...
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x074,
	ISP_RD32((void *)(ISP_ADDR + 0x074)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x078,
	...
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x0AC,
	ISP_RD32((void *)(ISP_ADDR + 0x0AC)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x0B0,
	ISP_RD32((void *)(ISP_ADDR + 0x0B0)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x0B4,
	ISP_RD32((void *)(ISP_ADDR + 0x0B4)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x0B8,
	ISP_RD32((void *)(ISP_ADDR + 0x0B8)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x0BC,
	ISP_RD32((void *)(ISP_ADDR + 0x0BC)));
	/* cq0 cur base */
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x0E8,
	ISP_RD32((void *)(ISP_ADDR + 0x0E8)));
	/* fbc imgo */
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x0F4,
	ISP_RD32((void *)(ISP_ADDR + 0x0F4)));
	/* fbc img2o */
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x0F8,
	ISP_RD32((void *)(ISP_ADDR + 0x0F8)));
	/* mdp crop x */
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x110,
	ISP_RD32((void *)(ISP_ADDR + 0x110)));
	/* mdp crop y */
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x114,
	ISP_RD32((void *)(ISP_ADDR + 0x114)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x138,
	ISP_RD32((void *)(ISP_ADDR + 0x138)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x13C,
	ISP_RD32((void *)(ISP_ADDR + 0x13C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x144,
	ISP_RD32((void *)(ISP_ADDR + 0x144)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x148,
	...
	ISP_RD32((void *)(ISP_ADDR + 0x20C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x22C,
	ISP_RD32((void *)(ISP_ADDR + 0x22C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x230,
	ISP_RD32((void *)(ISP_ADDR + 0x230)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x234,
	ISP_RD32((void *)(ISP_ADDR + 0x234)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x238,
	ISP_RD32((void *)(ISP_ADDR + 0x238)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x23C,
	ISP_RD32((void *)(ISP_ADDR + 0x23C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x240,
	ISP_RD32((void *)(ISP_ADDR + 0x240)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x244,
	ISP_RD32((void *)(ISP_ADDR + 0x244)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x258,
	ISP_RD32((void *)(ISP_ADDR + 0x258)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x260,
	ISP_RD32((void *)(ISP_ADDR + 0x260)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x26C,
	ISP_RD32((void *)(ISP_ADDR + 0x26C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x270,
	ISP_RD32((void *)(ISP_ADDR + 0x270)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x27C,
	ISP_RD32((void *)(ISP_ADDR + 0x27C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x298,
	ISP_RD32((void *)(ISP_ADDR + 0x298)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x2B4,
	ISP_RD32((void *)(ISP_ADDR + 0x2B4)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x2D0,
	ISP_RD32((void *)(ISP_ADDR + 0x2D0)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x2D4,
	ISP_RD32((void *)(ISP_ADDR + 0x2D4)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x2F0,
	ISP_RD32((void *)(ISP_ADDR + 0x2F0)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x2F4,
	ISP_RD32((void *)(ISP_ADDR + 0x2F4)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x300,
	ISP_RD32((void *)(ISP_ADDR + 0x300)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x304,
	ISP_RD32((void *)(ISP_ADDR + 0x304)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x308,
	ISP_RD32((void *)(ISP_ADDR + 0x308)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x30C,
	ISP_RD32((void *)(ISP_ADDR + 0x30C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x310,
	ISP_RD32((void *)(ISP_ADDR + 0x310)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x314,
	ISP_RD32((void *)(ISP_ADDR + 0x314)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x318,
	ISP_RD32((void *)(ISP_ADDR + 0x318)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x31C,
	ISP_RD32((void *)(ISP_ADDR + 0x31C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x320,
	ISP_RD32((void *)(ISP_ADDR + 0x320)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x324,
	ISP_RD32((void *)(ISP_ADDR + 0x324)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x328,
	ISP_RD32((void *)(ISP_ADDR + 0x328)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x32C,
	ISP_RD32((void *)(ISP_ADDR + 0x32C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x330,
	ISP_RD32((void *)(ISP_ADDR + 0x330)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x334,
	ISP_RD32((void *)(ISP_ADDR + 0x334)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x338,
	ISP_RD32((void *)(ISP_ADDR + 0x338)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x33C,
	ISP_RD32((void *)(ISP_ADDR + 0x33C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x350,
	ISP_RD32((void *)(ISP_ADDR + 0x350)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x37C,
	ISP_RD32((void *)(ISP_ADDR + 0x37C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x398,
	ISP_RD32((void *)(ISP_ADDR + 0x398)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x410,
	ISP_RD32((void *)(ISP_ADDR + 0x410)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x414,
	ISP_RD32((void *)(ISP_ADDR + 0x414)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x418,
	ISP_RD32((void *)(ISP_ADDR + 0x418)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x41C,
	ISP_RD32((void *)(ISP_ADDR + 0x41C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x420,
	ISP_RD32((void *)(ISP_ADDR + 0x420)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x448,
	ISP_RD32((void *)(ISP_ADDR + 0x448)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x4B0,
	ISP_RD32((void *)(ISP_ADDR + 0x4B0)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x4B4,
	ISP_RD32((void *)(ISP_ADDR + 0x4B4)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x4B8,
	ISP_RD32((void *)(ISP_ADDR + 0x4B8)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x4BC,
	ISP_RD32((void *)(ISP_ADDR + 0x4BC)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x4E8,
	ISP_RD32((void *)(ISP_ADDR + 0x4E8)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x534,
	ISP_RD32((void *)(ISP_ADDR + 0x534)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x538,
	ISP_RD32((void *)(ISP_ADDR + 0x538)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x53C,
	ISP_RD32((void *)(ISP_ADDR + 0x53C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x580,
	ISP_RD32((void *)(ISP_ADDR + 0x580)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x584,
	ISP_RD32((void *)(ISP_ADDR + 0x584)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x770,
	ISP_RD32((void *)(ISP_ADDR + 0x770)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x774,
	ISP_RD32((void *)(ISP_ADDR + 0x774)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x778,
	ISP_RD32((void *)(ISP_ADDR + 0x778)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x77C,
	ISP_RD32((void *)(ISP_ADDR + 0x77C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x800,
	ISP_RD32((void *)(ISP_ADDR + 0x800)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x880,
	ISP_RD32((void *)(ISP_ADDR + 0x880)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x884,
	ISP_RD32((void *)(ISP_ADDR + 0x884)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x888,
	ISP_RD32((void *)(ISP_ADDR + 0x888)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x8A0,
	ISP_RD32((void *)(ISP_ADDR + 0x8A0)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x920,
	ISP_RD32((void *)(ISP_ADDR + 0x920)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x924,
	ISP_RD32((void *)(ISP_ADDR + 0x924)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x928,
	ISP_RD32((void *)(ISP_ADDR + 0x928)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x92C,
	ISP_RD32((void *)(ISP_ADDR + 0x92C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x930,
	ISP_RD32((void *)(ISP_ADDR + 0x930)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x934,
	ISP_RD32((void *)(ISP_ADDR + 0x934)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x938,
	ISP_RD32((void *)(ISP_ADDR + 0x938)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x93C,
	ISP_RD32((void *)(ISP_ADDR + 0x93C)));
	/* unp offset */
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x948,
	ISP_RD32((void *)(ISP_ADDR + 0x948)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x960,
	ISP_RD32((void *)(ISP_ADDR + 0x960)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x9C4,
	ISP_RD32((void *)(ISP_ADDR + 0x9C4)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x9E4,
	ISP_RD32((void *)(ISP_ADDR + 0x9E4)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x9E8,
	ISP_RD32((void *)(ISP_ADDR + 0x9E8)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0x9EC,
	ISP_RD32((void *)(ISP_ADDR + 0x9EC)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xA00,
	ISP_RD32((void *)(ISP_ADDR + 0xA00)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xA04,
	ISP_RD32((void *)(ISP_ADDR + 0xA04)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xA08,
	ISP_RD32((void *)(ISP_ADDR + 0xA08)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xA0C,
	ISP_RD32((void *)(ISP_ADDR + 0xA0C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xA10,
	ISP_RD32((void *)(ISP_ADDR + 0xA10)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xA14,
	ISP_RD32((void *)(ISP_ADDR + 0xA14)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xA20,
	ISP_RD32((void *)(ISP_ADDR + 0xA20)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xAA0,
	ISP_RD32((void *)(ISP_ADDR + 0xAA0)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xACC,
	ISP_RD32((void *)(ISP_ADDR + 0xACC)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB00,
	ISP_RD32((void *)(ISP_ADDR + 0xB00)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB04,
	ISP_RD32((void *)(ISP_ADDR + 0xB04)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB08,
	ISP_RD32((void *)(ISP_ADDR + 0xB08)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB0C,
	ISP_RD32((void *)(ISP_ADDR + 0xB0C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB10,
	ISP_RD32((void *)(ISP_ADDR + 0xB10)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB14,
	ISP_RD32((void *)(ISP_ADDR + 0xB14)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB18,
	ISP_RD32((void *)(ISP_ADDR + 0xB18)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB1C,
	ISP_RD32((void *)(ISP_ADDR + 0xB1C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB20,
	ISP_RD32((void *)(ISP_ADDR + 0xB20)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB44,
	ISP_RD32((void *)(ISP_ADDR + 0xB44)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB48,
	ISP_RD32((void *)(ISP_ADDR + 0xB48)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB4C,
	ISP_RD32((void *)(ISP_ADDR + 0xB4C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB50,
	ISP_RD32((void *)(ISP_ADDR + 0xB50)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB54,
	ISP_RD32((void *)(ISP_ADDR + 0xB54)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB58,
	ISP_RD32((void *)(ISP_ADDR + 0xB58)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB5C,
	ISP_RD32((void *)(ISP_ADDR + 0xB5C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xB60,
	ISP_RD32((void *)(ISP_ADDR + 0xB60)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xBA0,
	ISP_RD32((void *)(ISP_ADDR + 0xBA0)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xBA4,
	ISP_RD32((void *)(ISP_ADDR + 0xBA4)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xBA8,
	ISP_RD32((void *)(ISP_ADDR + 0xBA8)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xBAC,
	ISP_RD32((void *)(ISP_ADDR + 0xBAC)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xBB0,
	ISP_RD32((void *)(ISP_ADDR + 0xBB0)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xBB4,
	ISP_RD32((void *)(ISP_ADDR + 0xBB4)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xBB8,
	ISP_RD32((void *)(ISP_ADDR + 0xBB8)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xBBC,
	ISP_RD32((void *)(ISP_ADDR + 0xBBC)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xBC0,
	ISP_RD32((void *)(ISP_ADDR + 0xBC0)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xC20,
	ISP_RD32((void *)(ISP_ADDR + 0xC20)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xCC0,
	ISP_RD32((void *)(ISP_ADDR + 0xCC0)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xCE4,
	ISP_RD32((void *)(ISP_ADDR + 0xCE4)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xCE8,
	ISP_RD32((void *)(ISP_ADDR + 0xCE8)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xCEC,
	ISP_RD32((void *)(ISP_ADDR + 0xCEC)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xCF0,
	ISP_RD32((void *)(ISP_ADDR + 0xCF0)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xCF4,
	ISP_RD32((void *)(ISP_ADDR + 0xCF4)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xCF8,
	ISP_RD32((void *)(ISP_ADDR + 0xCF8)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xCFC,
	ISP_RD32((void *)(ISP_ADDR + 0xCFC)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xD24,
	ISP_RD32((void *)(ISP_ADDR + 0xD24)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xD28,
	ISP_RD32((void *)(ISP_ADDR + 0xD28)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xD2C,
	ISP_RD32((void *)(ISP_ADDR + 0xD2c)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xD40,
	ISP_RD32((void *)(ISP_ADDR + 0xD40)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xD64,
	ISP_RD32((void *)(ISP_ADDR + 0xD64)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xD68,
	ISP_RD32((void *)(ISP_ADDR + 0xD68)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xD6C,
	ISP_RD32((void *)(ISP_ADDR + 0xD6c)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xD70,
	ISP_RD32((void *)(ISP_ADDR + 0xD70)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xD74,
	ISP_RD32((void *)(ISP_ADDR + 0xD74)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xD78,
	ISP_RD32((void *)(ISP_ADDR + 0xD78)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xD7C,
	ISP_RD32((void *)(ISP_ADDR + 0xD7C)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xDA4,
	ISP_RD32((void *)(ISP_ADDR + 0xDA4)));
	LOG_INF("0x%08X %08X", ISP_TPIPE_ADDR + 0xDA8,
 ...
 */
	...
	LOG_INF("0x%08X %08X ", ISP_ADDR + 0x160, ISP_RD32(ISP_ADDR + 0x160));
	LOG_INF("0x%08X %08X ", ISP_ADDR + 0x164, ISP_RD32(ISP_ADDR + 0x164));
 ...
 * Bit 23 : rdy
 *
 * Rdy should be 1    at idle or end of tile,
 * if not 0, \ABܥi\AF\E0\ACOmdp \A8S\A6^rdy
...
static int ISPDbgPortDump(int params)
{
	...
	LOG_DBG("IMGI STATUS:>");
	ISP_WR32((void *)ISP_REG_ADDR_CTL_DBG_SET,
	ISP_REG_ADDR_CTL_DBG_SET_IMGI_STS);
	LOG_DBG("(0x%08x 0x%08x)", ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
		ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
	ISP_WR32((void *)ISP_REG_ADDR_CTL_DBG_SET,
	ISP_REG_ADDR_CTL_DBG_SET_IMGI_SYNC);
	LOG_DBG("(0x%08x 0x%08x)", ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
		ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
	ISP_WR32((void *)ISP_REG_ADDR_CTL_DBG_SET,
	ISP_REG_ADDR_CTL_DBG_SET_IMGI_NO_SYNC);
	LOG_DBG("(0x%08x 0x%08x)", ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
		ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
	LOG_DBG(" <");
	...
	ISP_WR32((void *)ISP_REG_ADDR_CTL_DBG_SET,
	ISP_REG_ADDR_CTL_DBG_SET_CFA_STS);
	LOG_DBG("(0x%08x 0x%08x)", ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
		ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
	ISP_WR32((void *)ISP_REG_ADDR_CTL_DBG_SET,
	ISP_REG_ADDR_CTL_DBG_SET_CFA_SYNC);
	LOG_DBG("(0x%08x 0x%08x)", ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
		ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
	ISP_WR32((void *)ISP_REG_ADDR_CTL_DBG_SET,
	ISP_REG_ADDR_CTL_DBG_SET_CFA_NO_SYNC);
	LOG_DBG("(0x%08x 0x%08x)", ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
		ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
	LOG_DBG(" <");
	...
	ISP_WR32((void *)ISP_REG_ADDR_CTL_DBG_SET,
	ISP_REG_ADDR_CTL_DBG_SET_YUV_STS);
	LOG_DBG("(0x%08x 0x%08x)", ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
		ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
	ISP_WR32((void *)ISP_REG_ADDR_CTL_DBG_SET,
	ISP_REG_ADDR_CTL_DBG_SET_YUV_SYNC);
	LOG_DBG("(0x%08x 0x%08x)", ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
		ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
	ISP_WR32((void *)ISP_REG_ADDR_CTL_DBG_SET,
	ISP_REG_ADDR_CTL_DBG_SET_YUV_NO_SYNC);
	LOG_DBG("(0x%08x 0x%08x)", ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
		ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
	LOG_DBG(" <");
	...
	ISP_WR32((void *)ISP_REG_ADDR_CTL_DBG_SET,
	ISP_REG_ADDR_CTL_DBG_SET_OUT_STS);
	LOG_DBG("(0x%08x 0x%08x)", ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
		ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
	ISP_WR32((void *)ISP_REG_ADDR_CTL_DBG_SET,
	ISP_REG_ADDR_CTL_DBG_SET_OUT_SYNC);
	LOG_DBG("(0x%08x 0x%08x)", ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
		ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
	ISP_WR32((void *)ISP_REG_ADDR_CTL_DBG_SET,
	ISP_REG_ADDR_CTL_DBG_SET_OUT_NO_SYNC);
	LOG_DBG("(0x%08x 0x%08x)", ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
		ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
	LOG_DBG(" <");
	...
	ISP_WR32((void *)ISP_REG_ADDR_CTL_DBG_SET,
	ISP_REG_ADDR_CTL_DBG_SET_CQ_STS);
	LOG_DBG("(0x%08x 0x%08x)", ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET),
		ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
	LOG_DBG(" <");
	...
	LOG_DBG(
	"0x%08X %08X",
	ISP_TPIPE_ADDR + 0x204,
	ISP_RD32((void *)(ISP_ADDR + 0x204)));
	LOG_DBG(" <");

	LOG_DBG("IMGI :>");
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR +
	0x230, ISP_RD32((void *)(ISP_ADDR + 0x230)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR +
	0x234, ISP_RD32((void *)(ISP_ADDR + 0x234)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR +
	0x238, ISP_RD32((void *)(ISP_ADDR + 0x238)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR +
	0x23C, ISP_RD32((void *)(ISP_ADDR + 0x23C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR +
	0x240, ISP_RD32((void *)(ISP_ADDR + 0x240)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR +
	0x248, ISP_RD32((void *)(ISP_ADDR + 0x248)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR +
	0x24C, ISP_RD32((void *)(ISP_ADDR + 0x24C)));
	LOG_DBG(" <");
...
int ISPRegDump(int params, int params1)
{
	...
	LOG_DBG("start MT");
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x000,
	ISP_RD32((void *)(ISP_ADDR + 0x000)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x004,
	ISP_RD32((void *)(ISP_ADDR + 0x004)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x008,
	ISP_RD32((void *)(ISP_ADDR + 0x008)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x00C,
	ISP_RD32((void *)(ISP_ADDR + 0x00C)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x010,
	ISP_RD32((void *)(ISP_ADDR + 0x010)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x018,
	...
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x01C,
	ISP_RD32((void *)(ISP_ADDR + 0x01C)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x050,
	ISP_RD32((void *)(ISP_ADDR + 0x050)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x054,
	...
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x078,
	ISP_RD32((void *)(ISP_ADDR + 0x078)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x110,
	ISP_RD32((void *)(ISP_ADDR + 0x110)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x114,
	ISP_RD32((void *)(ISP_ADDR + 0x114)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x22C,
	ISP_RD32((void *)(ISP_ADDR + 0x22C)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x240,
	ISP_RD32((void *)(ISP_ADDR + 0x240)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x27C,
	ISP_RD32((void *)(ISP_ADDR + 0x27C)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x2B4,
	ISP_RD32((void *)(ISP_ADDR + 0x2B4)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x308,
	ISP_RD32((void *)(ISP_ADDR + 0x308)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x30C,
	ISP_RD32((void *)(ISP_ADDR + 0x30C)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x310,
	ISP_RD32((void *)(ISP_ADDR + 0x310)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x31C,
	ISP_RD32((void *)(ISP_ADDR + 0x31C)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x328,
	ISP_RD32((void *)(ISP_ADDR + 0x328)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x32C,
	ISP_RD32((void *)(ISP_ADDR + 0x32C)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x330,
	ISP_RD32((void *)(ISP_ADDR + 0x330)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x33C,
	ISP_RD32((void *)(ISP_ADDR + 0x33C)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x3A8,
	ISP_RD32((void *)(ISP_ADDR + 0x3A8)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x534,
	ISP_RD32((void *)(ISP_ADDR + 0x534)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x538,
	ISP_RD32((void *)(ISP_ADDR + 0x538)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x53C,
	ISP_RD32((void *)(ISP_ADDR + 0x53C)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x800,
	ISP_RD32((void *)(ISP_ADDR + 0x800)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x8A0,
	ISP_RD32((void *)(ISP_ADDR + 0x8A0)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x9C4,
	ISP_RD32((void *)(ISP_ADDR + 0x9C4)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x9E4,
	ISP_RD32((void *)(ISP_ADDR + 0x9E4)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x9E8,
	ISP_RD32((void *)(ISP_ADDR + 0x9E8)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0x9EC,
	ISP_RD32((void *)(ISP_ADDR + 0x9EC)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0xA20,
	ISP_RD32((void *)(ISP_ADDR + 0xA20)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0xACC,
	ISP_RD32((void *)(ISP_ADDR + 0xACC)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0xB00,
	ISP_RD32((void *)(ISP_ADDR + 0xB00)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0xB04,
	ISP_RD32((void *)(ISP_ADDR + 0xB04)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0xB08,
	ISP_RD32((void *)(ISP_ADDR + 0xB08)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0xB0C,
	ISP_RD32((void *)(ISP_ADDR + 0xB0C)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0xB10,
	ISP_RD32((void *)(ISP_ADDR + 0xB10)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0xB14,
	ISP_RD32((void *)(ISP_ADDR + 0xB14)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0xB18,
	ISP_RD32((void *)(ISP_ADDR + 0xB18)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0xB1C,
	ISP_RD32((void *)(ISP_ADDR + 0xB1C)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0xB20,
	ISP_RD32((void *)(ISP_ADDR + 0xB20)));
	LOG_DBG(
	"[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
	ISP_TPIPE_ADDR + 0xF50,
...
static int ISPResetPass2(int params)
{
	signed int i = 0;
	signed int Ret = 0;
	signed int m32RegEn2 = 0;
	signed int m32RegCropX = 0;
...
/* LOG_DBG("CQ STATUS: bef"); */
	...
	LOG_DBG(
	"ISP_REG_ADDR_CTL_EN2>: 0x%08x ",
	ISP_RD32((void *)ISP_REG_ADDR_CTL_EN2));
	m32RegEn2 = ISP_RD32((void *)ISP_REG_ADDR_CTL_EN2);
	ISP_WR32((void *)ISP_REG_ADDR_CTL_EN2, m32RegEn2 &
	(~ISP_REG_ADDR_CTL_EN2_UV_CRSA_EN_BIT));
/* LOG_DBG("ISP_REG_ADDR_CTL_EN2>>: 0x%08x ",
 * ISP_RD32((void *)ISP_REG_ADDR_CTL_EN2));
 */

	LOG_DBG(
	"ISP_REG_ADDR_CTL_CROP_X>: 0x%08x ",
	ISP_RD32((void *)ISP_REG_ADDR_CTL_CROP_X));
...
/* LOG_DBG("ISP_REG_ADDR_CTL_CROP_X>>: 0x%08x ",
 * ISP_RD32((void *)ISP_REG_ADDR_CTL_CROP_X));
 */
	...
	ISP_REG_ADDR_CTL_DBG_SET_CQ_STS);
	for (i = 0; i < 5000; i++) {
		if ((ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT)
			& 0x11) == 0x11)
			break;
	}
	LOG_DBG("i - %d ", i);
 ...
 *
 *************************************************/
static void ISP_EnableClock(bool En, unsigned int LogLevel)
{
	...
	if (En) {			/* enable clock. */
		/* new for MT, confirm with Abrams */
		enable_clock(MT_CG_DISP0_SMI_COMMON, "CAMERA");
		enable_clock(MT_CG_CAM_SMI_CKPDN, "CAMERA");
		enable_clock(MT_CG_CAM_CAM_CKPDN, "CAMERA");
		enable_clock(MT_CG_SEN_TG_CKPDN, "CAMERA");
		enable_clock(MT_CG_SEN_CAM_CKPDN, "CAMERA");
		enable_clock(MT_CG_LARB1_SMI_CKPDN, "CAMERA");
		g_EnableClkCnt++;
/* LOG_DBG("Camera clock enbled.
 * g_EnableClkCnt(%d)", g_EnableClkCnt);
 */

/*LOG_DBG(
 * "(0x%08x 0x%08x)", ISP_RD32((void *)
 * ISP_REG_ADDR_CTL_DBG_SET),
 * ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT));
 */
		if (ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET)
			!= ISP_REG_ADDR_CTL_DBG_SET_CQ_STS) {
/* LOG_DBG("CQ STATUS RETURN:>"); */
			ISP_WR32((void *)ISP_REG_ADDR_CTL_DBG_SET,
			ISP_REG_ADDR_CTL_DBG_SET_CQ_STS);
			if (LogLevel > 0) {
				...
				ISP_REG_ADDR_CTL_DBG_SET),
				ISP_RD32((void *)
				ISP_REG_ADDR_CTL_DBG_PORT));
			}
/* LOG_DBG(" <"); */
		}
	} else {		/* disable clock. */

		disable_clock(MT_CG_CAM_SMI_CKPDN, "CAMERA");
		disable_clock(MT_CG_CAM_CAM_CKPDN, "CAMERA");
		disable_clock(MT_CG_SEN_TG_CKPDN, "CAMERA");
		disable_clock(MT_CG_SEN_CAM_CKPDN, "CAMERA");
		disable_clock(MT_CG_LARB1_SMI_CKPDN, "CAMERA");
		/* new for MT, confirm with Abrams */
...
/* js_test, remove later once IMGSYS is working.
 * Now it's NOT working on FPGA.
 */
...
/* logically = 0, coverity
 * (!Reg) & ISP_REG_SW_CTL_SW_RST_STATUS
 */
	ISP_WR32((void *)ISP_REG_ADDR_SW_CTL,
	ISP_REG_SW_CTL_SW_RST_TRIG |
	ISP_REG_SW_CTL_HW_RST);	/* 0x5 */
	ISP_WR32((void *)ISP_REG_ADDR_SW_CTL,
	ISP_REG_SW_CTL_HW_RST);	/* 0x4 */
...
static void ISP_BufWrite_Init(void)
{
    ...
    //
		...
		g_IspInfo.BufInfo.Write[i].Status =
		ISP_BUF_STATUS_EMPTY;
...
static void ISP_BufWrite_Dump(void)
{
	...
	for (i = 0; i < ISP_BUF_WRITE_AMOUNT; i++) {
		LOG_DBG(
		"i(%d),Status(%d),Size(%d)",
		i, g_IspInfo.BufInfo.Write[i].Status,
...
static bool ISP_BufWrite_Add(unsigned int Size, struct ISP_REG_STRUCT *pData)
{
	...
	for (i = 0; i < ISP_BUF_WRITE_AMOUNT; i++) {
		if (g_IspInfo.BufInfo.Write[i].Status == ISP_BUF_STATUS_HOLD) {
			if ((g_IspInfo.BufInfo.Write[i].Size + Size)
				> ISP_BUF_SIZE_WRITE) {
				LOG_ERR(
				"i(%d), BufWriteSize(%d) + Size(%d) > %d",
				i, g_IspInfo.BufInfo.Write[i].Size,
				Size, ISP_BUF_SIZE_WRITE);
				return MFALSE;
			}
 ...
 *
 *************************************************/
static bool ISP_BufWrite_Get(unsigned int *pIndex,
unsigned int *pSize, unsigned char **ppData)
{
	unsigned int i;
...
static bool ISP_BufWrite_Clear(unsigned int Index)
{
	...
	if (g_IspInfo.BufInfo.Write[Index].Status == ISP_BUF_STATUS_READY) {
		if (g_IspInfo.DebugMask & ISP_DBG_BUF_WRITE)
			...
			"Index(%d), Size(%d)",
			Index,
			g_IspInfo.BufInfo.Write[Index].Size);
 ...
 *
 *************************************************/
void ISP_ScheduleWork_SENINF(struct work_struct *data)
{
	if (g_IspInfo.DebugMask & ISP_DBG_SCHEDULE_WORK)
		LOG_DBG("+");
...
void ISP_Tasklet_SENINF(unsigned long Param)
{
	...
	for (i = 0; i < 20; i++)
		LOG_DBG(
		"(0x%08x,0x%08x)\n",
		g_seninfDebug[i].regVal_1,
		g_seninfDebug[i].regVal_2);
}
...
static signed int ISP_WriteReg(struct ISP_REG_IO_STRUCT *pRegIo)
{
	...
	if (g_IspInfo.DebugMask & ISP_DBG_WRITE_REG) {
/* LOG_DBG("Data(0x%08X), Count(%d)",
 * (unsigned int)(pRegIo->Data),
 ...
 */
	...
	if (atomic_read(&(g_IspInfo.HoldInfo.HoldEnable))) {
		if (ISP_BufWrite_Add((pRegIo->Count) *
			...
			TimeVd = ISP_JiffiesToMs(jiffies)
			- g_IspInfo.TimeLog.Vd;
			TimeExpdone = ISP_JiffiesToMs(jiffies)
			- g_IspInfo.TimeLog.Expdone;
			TimeTasklet = ISP_JiffiesToMs(jiffies)
			- g_IspInfo.TimeLog.TaskletExpdone;

			LOG_ERR(
			"HoldTime(%d),VD(%d ms),Expdone(%d ms),Tasklet(%d ms)",
			g_IspInfo.HoldInfo.Time,
			TimeVd, TimeExpdone, TimeTasklet);
			ISP_BufWrite_Dump();
			ISP_DumpReg();
...
{
	LOG_DBG(
	"hold_reg(%d), BufAmount(%d)",
	atomic_read(&(g_IspInfo.
	HoldInfo.HoldEnable)),
...
static signed int ISP_EnableHoldReg(bool En)
{
	...
	if (!spin_trylock_bh(&(g_IspInfo.SpinLockHold))) {
		...
		g_IspInfo.WaitQueueHead, (IsLock =
		spin_trylock_bh(&(g_IspInfo.SpinLockHold))),
		ISP_MsToJiffies(500));
 ...
 */
/* camera1. single thread, we rest
 * p1p2 after all the object are destroyed
 */
					ISP_Reset();
					LOG_DBG(
					"ISP_REF_CNT_DEC_AND_RESET_IF_LAST_ONE. Do ISP_Reset");
				}
/* we do not care ISP_REF_CNT_DEC_AND
 * RESET_P1_IF_LAST_ONE and
...
/* due to we could only rest all in this series */
			...
			if (copy_to_user((void __user *)
				ref_cnt_ctrl.data_ptr, &imem_ref_cnt,
			     sizeof(signed int)) != 0) {
...
static signed int ISP_RTBC_ENQUE(signed int dma)
{
	...
	for (i = 0; i < ISP_RT_BUF_SIZE; i++) {
		if (g_pstRTBuf->ring_buf[rt_dma].data[i].
			base_pAddr == g_rt_buf_info.base_pAddr) {
			buffer_exist = 1;
			break;
		}
	...
	if (buffer_exist) {
		if (g_pstRTBuf->ring_buf[rt_dma].data[i].
			...
			g_pstRTBuf->ring_buf[rt_dma].data[i].
			bFilled = ISP_RTBC_BUF_EMPTY;
		...
		if (g_pstRTBuf->ring_buf[rt_dma].
			...
			/* first time add */
			index = g_pstRTBuf->ring_buf[rt_dma].
			total_count % ISP_RT_BUF_SIZE;

			g_pstRTBuf->ring_buf[rt_dma].data[index].
			memID = g_rt_buf_info.memID;
			g_pstRTBuf->ring_buf[rt_dma].data[index].
			size = g_rt_buf_info.size;
			g_pstRTBuf->ring_buf[rt_dma].data[index].base_vAddr =
			    g_rt_buf_info.base_vAddr;
			g_pstRTBuf->ring_buf[rt_dma].data[index].base_pAddr =
			    g_rt_buf_info.base_pAddr;
			g_pstRTBuf->ring_buf[rt_dma].
			data[index].bFilled = ISP_RTBC_BUF_EMPTY;
			...
			/* { */
			LOG_DBG(
			"[rtbc][ENQUE]:dma(%d),index(%d),PA(0x%x),empty(%d),total(%d)",
			rt_dma, index, g_pstRTBuf->
			ring_buf[rt_dma].data[index].base_pAddr,
			g_pstRTBuf->ring_buf[rt_dma].empty_count,
			g_pstRTBuf->ring_buf[rt_dma].total_count);
			/* } */
		}
	...
	if (g_pstRTBuf->ring_buf[rt_dma].empty_count == 1) {
		...
		rt_dma, g_pstRTBuf->ring_buf[rt_dma].
		data[index].base_pAddr,
 ...
 * &(g_IspInfo.SpinLockRTBC),g_Flash_SpinLock);
 */
...
static signed int ISP_RTBC_DEQUE(signed int dma)
{
	...
	for (i = 0; i < g_pstRTBuf->ring_buf[rt_dma].total_count; i++) {
		...
		if (g_pstRTBuf->ring_buf[rt_dma].data[index].bFilled
			== ISP_RTBC_BUF_FILLED) {
			g_pstRTBuf->ring_buf[rt_dma].data[index].
	...
	if (g_deque_buf.count == 0) {
		...
		for (i = 0; i <= g_pstRTBuf->
		ring_buf[rt_dma].total_count - 1; i++) {
			LOG_DBG(
...
static long ISP_Buf_CTRL_FUNC(unsigned long Param)
{
	...
	if (copy_from_user(&rt_buf_ctrl, (void __user *)
		...
		if (g_IspInfo.DebugMask & ISP_DBG_INT) {
			LOG_DBG(
			"[rtbc]ctrl(0x%x),buf_id(0x%x),data_ptr(0x%p),ex_data_ptr(0x%p)",
			rt_buf_ctrl.ctrl,
			rt_buf_ctrl.buf_id,
		...
		if ((rt_buf_ctrl.ctrl == ISP_RT_BUF_CTRL_ENQUE) ||
			...
			imgo_fbc.Reg_val = ISP_RD32((void *)
			ISP_REG_ADDR_IMGO_FBC);
			img2o_fbc.Reg_val = ISP_RD32((void *)
 ...
 */
		...
		if ((reg_val & 0x01) || (reg_val2 & 0x01)) {
			...
			if (rt_buf_ctrl.ex_data_ptr != 0) {
				...
				g_pstRTBuf->
				ring_buf[rt_dma].data[i].memID =
				g_ex_rt_buf_info.memID;
				g_pstRTBuf->
				ring_buf[rt_dma].data[i].size =
				...
				if ((curr_pa == g_rt_buf_info.
				base_pAddr) && (imgo_fbc.
				Bits.FB_NUM != 0) &&
				((imgo_fbc.Bits.FB_NUM ==
				imgo_fbc.Bits.FBC_CNT) ||
				(imgo_fbc.Bits.FBC_CNT ==
				(imgo_fbc.Bits.FB_NUM - 1)))) {
					ISP_WR32((void *)
					ISP_REG_ADDR_IMGO_BASE_ADDR,
					g_pstRTBuf->ring_buf[rt_dma].
					data[i].base_pAddr);
	...
	/* FBC_CNT-- */
					...
					g_IspInfo.WaitQueueHead,
				  (g_EnqBuf == 0),
					ISP_MsToJiffies(Timeout));

					if (Timeout == 0) {
						LOG_ERR(
						"[rtbc][ENQUE]:timeout(%d)",
						g_EnqBuf);
						Ret = -EFAULT;
					}
		...
		case ISP_RT_BUF_CTRL_DEQUE:
			...
			if ((reg_val & 0x01) || (reg_val2 & 0x01)) {
				...
				g_deque_buf.count = 1;
				/* RCNT = [1,2,3,...] */
				iBuf = ((_imgo_ == rt_dma) ? imgo_fbc.Bits.RCNT
				: img2o_fbc.Bits.RCNT) - 1;
				...
				for (i = 0; i < g_deque_buf.count; i++) {
					...
					g_deque_buf.data[i].base_vAddr =
					g_pstRTBuf->ring_buf[rt_dma].
					data[iBuf + i].base_vAddr;
					g_deque_buf.data[i].base_pAddr =
					g_pstRTBuf->ring_buf[rt_dma].
					data[iBuf + i].base_pAddr;
					g_deque_buf.data[i].timeStampS =
					g_pstRTBuf->ring_buf[rt_dma].
					data[iBuf + i].timeStampS;
					g_deque_buf.data[i].timeStampUs =
					g_pstRTBuf->ring_buf[rt_dma].
					data[iBuf + i].timeStampUs;
				...
				if (g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
					LOG_DBG(
					"[rtbc][DEQUE]+:wait deque done.(0x%x)(0x%x)",
					reg_val, reg_val2);
				}
				...
				if (Timeout == 0) {
					LOG_ERR(
					"[rtbc][DEQUE]:timeout(%d)",
					g_DeqBuf);
					LOG_ERR(
					"ISP_IRQ_TYPE_INT:IrqStatus(0x%08X)",
					g_IspInfo.IrqInfo.
					Status[ISP_IRQ_TYPE_INT]);
					ISP_DumpReg();
					Ret = -EFAULT;
				}
...
#if defined(_rtbc_use_cq0c_)
			bWaitBufRdy = ((_imgo_ ==
			rt_dma) ? (imgo_fbc.Bits.FBC_CNT) :
			(img2o_fbc.Bits.FBC_CNT)) ? 0 : 1;
#else
			for (i = 0; i <= g_pstRTBuf->
			ring_buf[rt_dma].total_count; i++) {
		...
		case ISP_RT_BUF_CTRL_GET_SIZE:
			...
			if (g_IspInfo.DebugMask & ISP_DBG_INT)
				LOG_DBG(
				"[rtbc][GET_SIZE]:rt_dma(%d)/size(%d)",
				rt_dma, size);

			if (copy_to_user((void __user *)rt_buf_ctrl.
				pExtend, &size, sizeof(unsigned int))
			    != 0) {
		...
		case ISP_RT_BUF_CTRL_CLEAR:
			...
			for (i = 0; i < ISP_RT_BUF_SIZE; i++) {
				if (g_pstRTBuf->ring_buf[
					rt_dma].data[i].base_pAddr ==
				    g_rt_buf_info.base_pAddr) {
					buffer_exist = 1;
					break;
				}
 ...
 *
 *************************************************/
static signed int ISP_SOF_Buf_Get(unsigned long long sec, unsigned long usec)
{
#if defined(_rtbc_use_cq0c_)
	...
	if (g_pstRTBuf->dropCnt == 0) {
		...
		if (g_TempAddr == curr_pa) {
			LOG_DBG(
			"[WARNING]g_TempAddr:Last(0x%08X) == Cur(0x%08X)",
			g_TempAddr,
			curr_pa);
			/* ISP_DumpReg(); */
		}
		 ...
		 * WCNT is untrustful.
		 */
		if (g_pstRTBuf->ring_buf[_imgo_].
			total_count > ISP_RT_CQ0C_BUF_SIZE) {
			LOG_ERR(
			"[rtbc]buf cnt(%d)",
			g_pstRTBuf->ring_buf[_imgo_].total_count);
			g_pstRTBuf->ring_buf[_imgo_].total_count =
			ISP_RT_CQ0C_BUF_SIZE;
		}

		if (curr_pa != g_pstRTBuf->ring_buf[_imgo_].
			data[curr_imgo].base_pAddr) {
			for (i = 0; i < g_pstRTBuf->
			ring_buf[_imgo_].total_count; i++) {
				if (curr_pa == g_pstRTBuf->
					ring_buf[_imgo_].data[i].base_pAddr) {
					if (g_IspInfo.DebugMask &
						ISP_DBG_BUF_CTRL)
						LOG_DBG(
		...
		g_pstRTBuf->ring_buf[_imgo_].
		data[curr_imgo].timeStampS = sec;
		g_pstRTBuf->ring_buf[_imgo_].
		data[curr_imgo].timeStampUs = usec;
		g_pstRTBuf->ring_buf[_img2o_].
		data[curr_img2o].timeStampS = sec;
		g_pstRTBuf->ring_buf[_img2o_].
		data[curr_img2o].timeStampUs = usec;

		if (g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
			LOG_DBG(
			"[rtbc]TStamp(%d.%06d),curr(%d),pa(0x%x/0x%x),o2o(0x%x/0x%x),fbc_o(0x%08x),fbc_2o(0x%08x),cq0c(0x%x)",
			g_pstRTBuf->ring_buf[_imgo_].
			data[curr_imgo].timeStampS,
			g_pstRTBuf->ring_buf[_imgo_].
			data[curr_imgo].timeStampUs,
			curr_imgo,
			g_pstRTBuf->ring_buf[_imgo_].
			data[curr_imgo].base_pAddr,
			g_pstRTBuf->ring_buf[_img2o_].
			...
			ISP_RD32((void *)ISP_REG_ADDR_IMGO_BASE_ADDR),
			ISP_RD32((void *)ISP_REG_ADDR_IMG2O_BASE_ADDR),
			(unsigned int)(imgo_fbc.Reg_val),
			(unsigned int)(img2o_fbc.Reg_val),
	...
	if (1000000 < ((sec * 1000000 + usec) - g_start_time)) {
		if (g_sof_count != 0) {
			g_avg_frame_time = (sec * 1000000 + usec)
			- g_start_time;
			g_avg_frame_time = (unsigned int)
			(g_avg_frame_time / g_sof_count);
		}
		g_sof_count = 0;
	} else {
		g_sof_count++;
	...
	for (i = 0; i <= 1; i++) {
		...
		if (g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
			LOG_DBG(
			"[rtbc][SOF]:dma(%d),start(%d),total(%d),empty(%d) ",
			i_dma,
			...
			for (i = 0; i <= g_pstRTBuf->
			ring_buf[i_dma].total_count - 1; i++) {
				LOG_DBG(
				"[rtbc][SOF]Buf List:%d/%d/0x%x/0x%x/0x%x/%d/  ",
				i,
		...
		if (g_pstRTBuf->ring_buf[i_dma].empty_count == 0) {
			/* once if buffer put into queue between
			 * SOF and ISP_DONE.
			 */
			...
			if (g_pstRTBuf->ring_buf[i_dma].empty_count >= 2) {
				...
				pAddr = g_pstRTBuf->ring_buf[i_dma].
				data[next].base_pAddr;
				...
				if (g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
					...
					i_dma,
					g_pstRTBuf->ring_buf[i_dma].start,
					g_pstRTBuf->ring_buf[i_dma].
					empty_count, next, pAddr);
				}
			} else {
				if (g_IspInfo.DebugMask & ISP_DBG_INT) {
					LOG_DBG(
	...
	/* spin_unlock_irqrestore(&(g_IspInfo.SpinLockRTBC),
	 * g_Flash_SpinLock);
	 */
#endif
 ...
 *************************************************/
#ifndef _rtbc_use_cq0c_
static signed int ISP_DONE_Buf_Time(unsigned long long sec, unsigned long usec)
{
	signed int i, i_dma;
	unsigned int curr;
	...
	for (i = 0; i <= 1; i++) {
		i_dma = (i == 0) ? _imgo_ : _img2o_;
		reg_fbc = (_imgo_ == i_dma) ?
		ISP_REG_ADDR_IMGO_FBC : ISP_REG_ADDR_IMG2O_FBC;
		...
		while (1) {
			...
			if (g_pstRTBuf->ring_buf[i_dma].data[curr].bFilled
				== ISP_RTBC_BUF_EMPTY) {
				g_pstRTBuf->ring_buf[i_dma].data[curr].bFilled =
				    ISP_RTBC_BUF_FILLED;
...
static signed int ISP_WaitIrq(struct ISP_WAIT_IRQ_STRUCT WaitIrq)
{
	...
	unsigned int i;
	unsigned long flags;
	unsigned int startTimeS = 0, startTimeUS = 0;
	unsigned int endTimeS = 0, endTimeUS = 0;
	...
	if (g_IspInfo.DebugMask & ISP_DBG_INT) {
		LOG_DBG(
		"Clear(%d),Type(%d),Status(0x%08X),Timeout(%d)",
 ...
 */
		...
		if ((g_IspInfo.IrqInfo.Status[WaitIrq.Type] &
			WaitIrq.Status) == WaitIrq.Status
		    || Timeout == 0) {
			break;
		}
	...
	if (Timeout == 0) {
		...
		LOG_ERR(
		"start(%u.%06u),end(%u.%06u)",
		startTimeS, startTimeUS,
		endTimeS, endTimeUS);
		LOG_ERR(
		"Clear(%d),Type(%d),IrqStatus(0x%08X),WaitStatus(0x%08X),Timeout(%d)",
		WaitIrq.Clear, WaitIrq.Type,
		g_IspInfo.IrqInfo.Status[WaitIrq.Type],
		WaitIrq.Status, WaitIrq.Timeout);
	...
	if (g_IspInfo.DebugMask & ISP_DBG_INT) {
		for (i = 0; i < ISP_IRQ_TYPE_AMOUNT; i++)
			LOG_DBG(
			"Type(%d), IrqStatus(0x%08X)",
			i, g_IspInfo.IrqInfo.Status[i]);
	}
	...
	if (WaitIrq.Status == ISP_IRQ_INT_STATUS_PASS2_DON_ST ||
		...
		if (ISP_RD32((void *)(CAMINF_BASE + 0x4160))
			!= ISP_REG_ADDR_CTL_DBG_SET_CQ_STS) {
			LOG_WRN(
			"0x4160 != 0x6000 !!!(0x%8x)",
		...
		case ISP_IRQ_TYPE_INT:
			if ((CQ_status & 0x0000000F) != 0x001) {
				LOG_ERR(
				"CQ1 not idle dbg(0x%08x 0x%08x)",
				ISP_RD32((void *)(CAMINF_BASE + 0x4160)),
				CQ_status);
			}
			break;
		case ISP_IRQ_TYPE_INTB:
			if ((CQ_status & 0x000000F0) != 0x010) {
				LOG_ERR(
				"CQ2 not idle dbg(0x%08x 0x%08x)",
				ISP_RD32((void *)(CAMINF_BASE + 0x4160)),
				CQ_status);
			}
			break;
		case ISP_IRQ_TYPE_INTC:
			if ((CQ_status & 0x00000F00) != 0x100) {
				LOG_ERR(
				"CQ3 not idle dbg(0x%08x 0x%08x)",
				ISP_RD32((void *)(CAMINF_BASE + 0x4160)),
				CQ_status);
			}
			break;
 ...
 *
 *************************************************/
static __tcmfunc irqreturn_t ISP_Irq(signed int Irq, void *DeviceId)
{
	/* LOG_DBG("- E."); */
	unsigned int i;
	...
	/* sec and usec */
	time_frmb_usec = do_div(time_frmb_sec, 1000000);
	...
	spin_lock((spinlock_t *)(&SpinLockCamHaVer));
	if (!CAM_HAL_VER_IS3) {
		spin_unlock((spinlock_t *)(&SpinLockCamHaVer));
		...
		for (i = 0; i < ISP_IRQ_TYPE_AMOUNT; i++) {
			if (g_IspInfo.IrqInfo.ErrMask[i] & IrqStatus[i]) {
				/* ISP_IRQ_INTX_STATUS_IMGO_ERR_ST:
				 * on-the-fly imgo error, not really overrun
				 */
...
/*if ( IrqStatus[ISP_IRQ_TYPE_INTX]
 * &ISP_IRQ_INTX_STATUS_DMA_ERR_ST ) {
 *
 * #if defined(_debug_dma_err_)
 * LOG_ERR("[rtbc]StatusX(0x%08X),
 * DMA_ERR",IrqStatus[ISP_IRQ_TYPE_INTX]);
 * //IMGO may overrun before setting smi regs
 * //allan
 * DMAErrHandler();
 * LOG_DBG("ERR (8018 0x%08x)(8024 0x%08x)
 * (802C 0x%08x)(4440 0x%08x) (810C 0x%08x)",
 * ISP_RD32(0xF5008018),
 * ISP_RD32(0xF5008024),
 * ISP_RD32(0xF500802C),
 * ISP_RD32(0xF5004440),
 ...
 */
		 ...
		 */
		/* LOG_ERR("[rtbc]Status(0x%08X),
		 * TG_ERR",IrqStatus[ISP_IRQ_TYPE_INT]);
		 */
		/* } */
		/*  */
		/* if (IrqStatus_fbc_int) { */
		/* LOG_ERR("[rtbc]dropframe st(0x%08X),co(0x%08X),
		 * c2o(0x%08X)",IrqStatus_fbc_int,
		 * ISP_RD32(ISP_REG_ADDR_IMGO_FBC),
		 * ISP_RD32(ISP_REG_ADDR_IMG2O_FBC));
		 */
		/* } */
		...
		if (IrqStatus[ISP_IRQ_TYPE_INT] &
			...
			if (g_newImgoAddr != DEFAULT_PA &&
			    ...
			    ((imgo_fbc.Bits.FB_NUM == imgo_fbc.Bits.FBC_CNT)
			     ...
			     (imgo_fbc.Bits.FB_NUM - 1)))) {
				LOG_DBG(
				"[rtbc][TG1_DON]o,tg_pa(0x%x),old(0x%x)",
				tg_pa,
				g_oldImgoAddr);
				LOG_DBG(
				"[rtbc][TG1_DON]o,NUM(0x%x),CNT(0x%x)",
				imgo_fbc.Bits.FB_NUM,
				imgo_fbc.Bits.FBC_CNT);
				LOG_DBG("[rtbc][TG1_DON]o,new(0x%x)",
				g_newImgoAddr);
				ISP_WR32((void *)ISP_REG_ADDR_IMGO_BASE_ADDR,
				g_newImgoAddr);
				g_oldImgoAddr = DEFAULT_PA;
			...
			if (g_newImg2oAddr != DEFAULT_PA &&
			    ...
			    ((img2o_fbc.Bits.FB_NUM == img2o_fbc.Bits.FBC_CNT)
			     || (img2o_fbc.Bits.FBC_CNT ==
			     (img2o_fbc.Bits.FB_NUM - 1)))) {
				LOG_DBG(
				"[rtbc][TG1_DON]2o,tg_pa(0x%x),old(0x%x)",
				tg_pa, g_oldImg2oAddr);
				LOG_DBG("[rtbc][TG1_DON]2o,NUM(0x%x),CNT(0x%x)",
					img2o_fbc.Bits.FB_NUM,
					img2o_fbc.Bits.FBC_CNT);
				LOG_DBG("[rtbc][TG1_DON]2o,new(0x%x)",
				g_newImg2oAddr);
				ISP_WR32((void *)ISP_REG_ADDR_IMG2O_BASE_ADDR,
				g_newImg2oAddr);
				g_oldImg2oAddr = DEFAULT_PA;
		...
		if (IrqStatus[ISP_IRQ_TYPE_INT] &
			...
			if ((sof_pa != 0) && (sof_pa == g_oldImgoAddr)
				...
				(imgo_fbc.Bits.FB_NUM - 1)))) {
				LOG_DBG(
				"[rtbc][SOF]o,sof_pa(0x%x),old(0x%x)",
				sof_pa,
				g_oldImgoAddr);
				LOG_DBG("[rtbc][SOF]o,NUM(0x%x),CNT(0x%x)",
				imgo_fbc.Bits.FB_NUM,
				imgo_fbc.Bits.FBC_CNT);
				LOG_DBG(
				"[rtbc][SOF]o,new(0x%x)",
			...
			if ((sof_pa != 0) && (sof_pa == g_oldImg2oAddr)
				...
				(img2o_fbc.Bits.FB_NUM - 1)))) {
				LOG_DBG(
				"[rtbc][SOF]2o,sof_pa(0x%x),old(0x%x)",
				sof_pa,
				g_oldImg2oAddr);
				LOG_DBG("[rtbc][SOF]2o,NUM(0x%x),CNT(0x%x)",
				img2o_fbc.Bits.FB_NUM,
					img2o_fbc.Bits.FBC_CNT);
				LOG_DBG("[rtbc][SOF]2o,new(0x%x/0x%x)",
				sof_pa, g_newImg2oAddr);
			...
			LOG_DBG("P1_SOF_%d(0x%08lx,0x%08lx,0x%08x,0x%08x)",
				...
				0x00FF0000) >> 16),
				imgo_fbc.Reg_val, img2o_fbc.Reg_val,
				imgo_pa, img2o_pa);
		...
		for (j = 0; j < ISP_IRQ_TYPE_AMOUNT_FRMB; j++) {
			for (i = 0; i < IRQ_USER_NUM_MAX; i++) {
				...
				IspInfo_FrmB.IrqInfo.Status[i][j] |=
				    (IrqStatus[j] &
				    IspInfo_FrmB.IrqInfo.Mask[j]);

				/* 2. update signal occurring time and
				 * passed by signal count
				 */
				if (IspInfo_FrmB.IrqInfo.MarkedFlag[i][j] &
	...
	/* Tasklet. It is uninterrupted,
	 * so there can NOT be "Sleep" in tasklet function.
	 */
	if (IrqStatus[ISP_IRQ_TYPE_INT] &
		(ISP_IRQ_INT_STATUS_EXPDON1_ST)) {
...
static long ISP_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	case ISP_READ_IRQ:
		{
			if (copy_from_user
				...
				spin_lock_irqsave(
				&(g_IspInfo.SpinLockIrq), flags);

				ISP_IRQ_TYPE_ENUM
				IrqStatus[ISP_IRQ_TYPE_AMOUNT];

				IrqStatus[ISP_IRQ_TYPE_INT] =
				    (ISP_RD32((void *)
				    ISP_REG_ADDR_INT_STATUS) &
				     (g_IspInfo.IrqInfo.Mask[
				     ISP_IRQ_TYPE_INT] |
				     g_IspInfo.IrqInfo.
				     ErrMask[ISP_IRQ_TYPE_INT]));
				IrqStatus[ISP_IRQ_TYPE_DMA] =
				    (ISP_RD32((void *)
				    ISP_REG_ADDR_DMA_INT) &
				     (g_IspInfo.IrqInfo.Mask[
				     ISP_IRQ_TYPE_DMA] |
				     g_IspInfo.IrqInfo.
				     ErrMask[ISP_IRQ_TYPE_DMA]));
				...
				ReadIrq.Status =
				g_IspInfo.IrqInfo.Status[ReadIrq.Type];
	...
	case ISP_CLEAR_IRQ:
		{
			if (copy_from_user
				...
				LOG_DBG(
				"ISP_CLEAR_IRQ:Type(%d),Status(0x%08X),IrqStatus(0x%08X)",
	...
	case ISP_DEBUG_FLAG:
		{
			if (copy_from_user(&DebugFlag[0], (void __user *)Param,
				sizeof(unsigned int)) == 0) {
				spin_lock_irqsave(
				&(g_IspInfo.SpinLockIrq), flags);
	...
	case ISP_DEBUG_FLAG_FRMB:
		{
			if (copy_from_user(DebugFlag, (void __user *)Param,
				...
				spin_unlock_irqrestore(
				&(IspInfo_FrmB.SpinLockIrq[lock_key]),
						       flags);
	...
	case ISP_SENSOR_FREQ_CTRL:
		{
			unsigned int senFreq = 0;

			if (copy_from_user(&senFreq, (void __user *)Param,
				sizeof(unsigned int)) == 0) {
				LOG_DBG("senFreq=%u", senFreq);
#if 1
/* from abrams, mt6580 is
 * 17:15:rg_cam_mux_sel
 */
				clkmux_sel(MT_CLKMUX_CAM_MUX_SEL,
				senFreq, "CAMERA_SENSOR");
				/* clkmux_sel(MT_CLKMUX_SCAM_MUX_SEL,
				 * senFreq, "CAMERA_SENSOR");
				 */
#else
				clkmux_sel(MT_MUX_CAMTG, senFreq,
				"CAMERA_SENSOR");
#endif
			} else {
	...
	case ISP_SET_USER_PID:
		{
			if (copy_from_user(&pid, (void __user *)Param,
				sizeof(unsigned int)) == 0) {
				spin_lock(&(g_IspInfo.SpinLockIsp));
				...
				LOG_DBG(
				"[ISP_KERNEL_MOTIFY_SINGLE_TEST]:0x08%x ",
				pid);
				spin_unlock(&(g_IspInfo.SpinLockIsp));
			} else {
				LOG_ERR("copy_from_user failed");
	...
	case ISP_BUFFER_CTRL:
		spin_lock_irqsave((spinlock_t *)(&SpinLockCamHaVer), flags);
		if (!CAM_HAL_VER_IS3) {
			spin_unlock_irqrestore((spinlock_t *)(
			&SpinLockCamHaVer), flags);
	...
	case ISP_SET_CAM_VERSION:
		{
			...
			if (copy_from_user(&bIspVer,
				(void __user *)Param, sizeof(bool)) == 0) {
	...
	case ISP_WAIT_IRQ_FRMB:
		if (copy_from_user
			...
			if ((Irq_FrmB.UserInfo.UserKey >= IRQ_USER_NUM_MAX)
				 ...
				 * WaitIrq_FrmB.UserInfo.UserKey,
				 IRQ_USER_NUM_MAX);
				 */
				/* force to use default user key if
			...
			if ((Irq_FrmB.UserInfo.Type >= ISP_IRQ_TYPE_AMOUNT_FRMB)
				...
				LOG_ERR(
				"invalid type(%d), max(%d)",
				Irq_FrmB.UserInfo.Type,
	...
	case ISP_GET_CUR_SOF_FRMB:
		if (copy_from_user(&DebugFlag[0], (void __user *)Param,
			...
			switch (DebugFlag[0]) {
			case _IRQ:
				/* TG sof update at expo_done,
				 *causes potential timing issue
				 */
				/* DebugFlag[1] = ((ISP_RD32
				 * (ISP_REG_ADDR_TG_INTER_ST)
				 * &0x00FF0000)>>16);
				 */
	...
	case ISP_GET_DMA_ERR_FRMB:
		if (copy_to_user((void __user *)Param, &g_DmaErr_p1[0],
			sizeof(unsigned int) * nDMA_ERR) != 0) {
			LOG_ERR("get dma_err fail\n");
		}
		break;
	case ISP_GET_INT_ERR_FRMB:
		if (copy_to_user((void __user *)Param, (void *)g_ISPIntErr,
	...
	case ISP_FLUSH_IRQ_REQUEST:
		if (copy_from_user
			 ...
			 * || (Irq_FrmB.UserInfo.UserKey<1))
			 */
			/* UserKey(0) is Isp drv,
			 *flush ISP EnqueThread SOF signal
			 */
	...
	case ISP_GET_DROP_FRAME_FRMB:
		if (copy_from_user(&DebugFlag[0], (void __user *)
			Param, sizeof(unsigned int)) != 0) {
			LOG_ERR("get irq from user fail");
			Ret = -EFAULT;
		} else {
			switch (DebugFlag[0]) {
			case _IRQ:
				spin_lock_irqsave(
				&(IspInfo_FrmB.SpinLockIrq[_IRQ]), flags);
...
static int compat_get_isp_buf_ctrl_frmb_struct_data(
compat_ISP_BUFFER_CTRL_STRUCT_FRMB __user *
data32,
...
static int compat_put_isp_buf_ctrl_frmb_struct_data(
compat_ISP_BUFFER_CTRL_STRUCT_FRMB __user *
data32,
ISP_BUFFER_CTRL_STRUCT_FRMB __user *data)
{
...
static int compat_get_isp_register_irq_user_key_data(
compat_ISP_REGISTER_USERKEY_STRUCT_FRMB __user
*data32,
...
static int compat_put_isp_register_irq_user_key_data(
compat_ISP_REGISTER_USERKEY_STRUCT_FRMB __user
*data32,
ISP_REGISTER_USERKEY_STRUCT_FRMB __user *data)
{
...
#if 0
/* #define COMPAT_ISP_READ_REGISTER   _IOWR(
 * ISP_MAGIC, ISP_CMD_READ_REG, compat_ISP_REG_IO_STRUCT)
 * #define COMPAT_ISP_WRITE_REGISTER  _IOWR(
 * ISP_MAGIC, ISP_CMD_WRITE_REG, compat_ISP_REG_IO_STRUCT)
 * #define COMPAT_ISP_BUFFER_CTRL     _IOWR(
 * ISP_MAGIC, ISP_CMD_RT_BUF_CTRL, compat_ISP_BUFFER_CTRL_STRUCT)
 * #define COMPAT_ISP_REF_CNT_CTRL    _IOWR(
 * ISP_MAGIC, ISP_CMD_REF_CNT, compat_ISP_REF_CNT_CTRL_STRUCT)
 */
#endif
static long ISP_ioctl_compat(struct file *filp,
	...
	case COMPAT_ISP_READ_REGISTER:
		{
			...
			if (err) {
				LOG_INF(
				"compat_get_isp_read_register_data error!!!\n");
				return err;
			}
			ret =
			...
			if (err) {
				LOG_INF(
				"compat_put_isp_read_register_data error!!!\n");
				return err;
			}
			return ret;
		}
	...
	case COMPAT_ISP_BUFFER_CTRL:
		{
			if (CAM_HAL_VER_IS3) {
				...
				*data32;
				...
				err = compat_get_isp_buf_ctrl_frmb_struct_data(
				data32, data);
				if (err) {
					LOG_INF(
				...
				ret =
				    ...
				    ISP_BUFFER_CTRL,
						(unsigned long)data);
				err = compat_put_isp_buf_ctrl_frmb_struct_data(
				data32, data);
				if (err) {
					LOG_INF(
				...
				err = compat_get_isp_buf_ctrl_struct_data(
				data32, data);
				if (err) {
					LOG_INF(
					"compat_get_isp_buf_ctrl_struct_data error!!!\n");
				...
				ret =
				    ...
				    ISP_BUFFER_CTRL,
						(unsigned long)data);
				err = compat_put_isp_buf_ctrl_struct_data(
				data32, data);
				if (err) {
					LOG_INF(
					"compat_put_isp_buf_ctrl_struct_data error!!!\n");
	...
	case COMPAT_ISP_REF_CNT_CTRL:
		{
			...
			err = compat_get_isp_ref_cnt_ctrl_struct_data(
			data32, data);
			if (err) {
				LOG_INF(
			...
			ret =
			    filp->f_op->unlocked_ioctl(filp,
			    ISP_REF_CNT_CTRL, (unsigned long)data);
			err = compat_put_isp_ref_cnt_ctrl_struct_data(
			data32, data);
			if (err) {
				LOG_INF(
	...
	case COMPAT_ISP_REGISTER_IRQ_USER_KEY:
		{
			...
			err = compat_get_isp_register_irq_user_key_data(
			data32, data);
			if (err) {
				LOG_INF(
				"compat_get_isp_register_irq_user_key_data error!!!\n");
				return err;
			}
			ret =
			    filp->f_op->unlocked_ioctl(filp,
			    ISP_REGISTER_IRQ_USER_KEY,
					(unsigned long)data);
			err = compat_put_isp_register_irq_user_key_data(
			data32, data);
			if (err) {
				LOG_INF(
				"compat_put_isp_register_irq_user_key_data error!!!\n");
				return err;
			}
			return ret;
		}
...
static signed int ISP_open(struct inode *pInode, struct file *pFile)
{
	...
	spin_unlock_irqrestore((spinlock_t *)(&SpinLockCamHaVer), flags);

	g_IspInfo.BufInfo.Read.pData =
...
static signed int ISP_release(struct inode *pInode, struct file *pFile)
{
	...
	/* cam 3 variable un-initialization */
	ISP_release_FrmB();
	/*  */
EXIT:
...
static signed int mmap_kmem(struct file *filp, struct vm_area_struct *vma)
{
	...
	/* check length - do not allow larger mappings than
	...
	LOG_INF(
	"Vma->vm_pgoff(0x%x),Vma->vm_start(0x%x),Vma->vm_end(0x%x),length(0x%x)",
	...
	if (length > ISP_RTBUF_REG_RANGE) {
		LOG_ERR(
		"mmap range error! : length(0x%x),ISP_RTBUF_REG_RANGE(0x%x)!",
		(unsigned int)(length), ISP_RTBUF_REG_RANGE);
		return -EAGAIN;
	}
	ret = remap_pfn_range(vma,
				   vma->vm_start,
				   virt_to_phys((void *)g_pTbl_RTBuf)
				   >> PAGE_SHIFT,
...
static signed int ISP_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	...
	if (pVma->vm_pgoff == RT_BUF_TBL_NPAGES) {
		...
		(unsigned int)(pVma->vm_pgoff << PAGE_SHIFT),
		...
		case IMGSYS_BASE_ADDR:	/* imgsys */
			if (length > ISP_REG_RANGE) {
				LOG_ERR(
				"mmap range error : length(0x%x),ISP_REG_RANGE(0x%x)!",
				(unsigned int)(length),
				ISP_REG_RANGE);
				return -EAGAIN;
			}
		...
		case SENINF_BASE_ADDR:
			if (length > SENINF_REG_RANGE) {
				LOG_ERR(
				"mmap range error : length(0x%x),SENINF_REG_RANGE(0x%x)!",
				(unsigned int)(length),
				SENINF_REG_RANGE);
				return -EAGAIN;
			}
		...
		case PLL_BASE_ADDR:
			if (length > PLL_RANGE) {
				LOG_ERR(
				"mmap range error : length(0x%x),PLL_RANGE(0x%x)!",
		...
		case GPIO_BASE_ADDR:
			if (length > GPIO_RANGE) {
				LOG_ERR(
				"mmap range error : length(0x%x),GPIO_RANGE(0x%x)!",
		...
		case EFUSE_BASE_ADDR:
			if (length > EFUSE_RANGE) {
				LOG_ERR(
				"mmap range error : length(0x%x),EFUSE_RANGE(0x%x)!",
 ...
 *
 *************************************************/
static signed int ISP_probe_FrmB(void)
{
	signed int Ret = 0;
#if 0
	...
	unsigned char n;
	/*  */
	init_waitqueue_head((wait_queue_head_t *)(&IspInfo_FrmB.WaitQueueHead));
	/*  */
	/* INIT_WORK(&IspInfo_FrmB.ScheduleWorkVD,
	 *ISP_ScheduleWork_VD);
	 */
	/* INIT_WORK(&IspInfo_FrmB.ScheduleWorkEXPDONE,
	 *ISP_ScheduleWork_EXPDONE);
	 */
	...
	/*  */
	IspInfo_FrmB.IrqInfo.Mask[ISP_IRQ_TYPE_INT] =
	ISP_REG_MASK_INT_STATUS;
	IspInfo_FrmB.IrqInfo.Mask[ISP_IRQ_TYPE_DMA] =
	ISP_REG_MASK_DMA_INT;
	IspInfo_FrmB.IrqInfo.Mask[ISP_IRQ_TYPE_INTB] =
	ISP_REG_MASK_INTB_STATUS;
	IspInfo_FrmB.IrqInfo.Mask[ISP_IRQ_TYPE_DMAB] =
	ISP_REG_MASK_DMAB_INT;
	IspInfo_FrmB.IrqInfo.Mask[ISP_IRQ_TYPE_INTC] =
	ISP_REG_MASK_INTC_STATUS;
	IspInfo_FrmB.IrqInfo.Mask[ISP_IRQ_TYPE_DMAC] =
	ISP_REG_MASK_DMAC_INT;
	IspInfo_FrmB.IrqInfo.Mask[ISP_IRQ_TYPE_INTX] =
	ISP_REG_MASK_INTX_STATUS;
	IspInfo_FrmB.IrqInfo.Mask[ISP_IRQ_TYPE_DMAX] =
	ISP_REG_MASK_DMAX_INT;

	IspInfo_FrmB.IrqInfo.ErrMask[ISP_IRQ_TYPE_INT] =
	ISP_REG_MASK_INT_STATUS_ERR;
	IspInfo_FrmB.IrqInfo.ErrMask[ISP_IRQ_TYPE_DMA] =
	ISP_REG_MASK_DMA_INT_ERR;
	IspInfo_FrmB.IrqInfo.ErrMask[ISP_IRQ_TYPE_INTB] =
	ISP_REG_MASK_INTB_STATUS_ERR;
	/* IspInfo_FrmB.IrqInfo.ErrMask[ISP_IRQ_TYPE_DMAB] =
	 *ISP_REG_MASK_DMAB_INT_ERR;
	 */
	IspInfo_FrmB.IrqInfo.ErrMask[ISP_IRQ_TYPE_INTC] =
	ISP_REG_MASK_INTC_STATUS_ERR;
	/* IspInfo_FrmB.IrqInfo.ErrMask[ISP_IRQ_TYPE_DMAC] =
	 *ISP_REG_MASK_DMAC_INT_ERR;
	 */
	IspInfo_FrmB.IrqInfo.ErrMask[ISP_IRQ_TYPE_INTX] =
	ISP_REG_MASK_INTX_STATUS_ERR_FRMB;
	IspInfo_FrmB.IrqInfo.ErrMask[ISP_IRQ_TYPE_DMAX] =
	ISP_REG_MASK_DMAX_INT_ERR;
 ...
 *************************************************/
static signed int ISP_remove_FrmB(void)
{
#if 0
...
static signed int ISP_probe(struct platform_device *pDev)
{
	...
	for (i = 0; i < ISP_CAM_BASEADDR_NUM; i++) {
		...
		pDev->dev.of_node->name, gISPSYS_Reg[i], gISPSYS_Reg_FrmB[i]);
	}

	/* get IRQ ID and request IRQ */
	for (i = 0; i < ISP_CAM_IRQ_IDX_NUM; i++) {
		cam_isp_dev->irq[i] = irq_of_parse_and_map(
		pDev->dev.of_node, i);
	...
	/* NEED_TUNING_BY_CHIP. 1: Only one IORESOURCE_MEM type resource in
	 *kernel\mt_devs.c\mt_resource_isp[].
	 */
	for (i = 0; i < 1; i++)	{
	...
	spin_lock_init((spinlock_t *)(&SpinLockCamHaVer));
	...
	g_IspInfo.IrqInfo.ErrMask[ISP_IRQ_TYPE_INT] =
	ISP_REG_MASK_INT_STATUS_ERR;
	g_IspInfo.IrqInfo.ErrMask[ISP_IRQ_TYPE_DMA] =
	ISP_REG_MASK_DMA_INT_ERR;
	g_IspInfo.IrqInfo.ErrMask[ISP_IRQ_TYPE_INTB] =
	ISP_REG_MASK_INTB_STATUS_ERR;
	/* g_IspInfo.IrqInfo.ErrMask[ISP_IRQ_TYPE_DMAB] =
	 *ISP_REG_MASK_DMAB_INT_ERR;
	 */
	g_IspInfo.IrqInfo.ErrMask[ISP_IRQ_TYPE_INTC] =
	ISP_REG_MASK_INTC_STATUS_ERR;
	/* g_IspInfo.IrqInfo.ErrMask[ISP_IRQ_TYPE_DMAC] =
	 *ISP_REG_MASK_DMAC_INT_ERR;
	 */
	g_IspInfo.IrqInfo.ErrMask[ISP_IRQ_TYPE_INTX] =
	ISP_REG_MASK_INTX_STATUS_ERR;
	g_IspInfo.IrqInfo.ErrMask[ISP_IRQ_TYPE_DMAX] =
	ISP_REG_MASK_DMAX_INT_ERR;
...
static signed int ISP_suspend(struct platform_device *pDev, pm_message_t Mesg)
{
	...
	unsigned int regTG1Val = ISP_RD32((void *)(ISP_ADDR + 0x414));

	LOG_DBG(
	"g_bPass1_On_In_Resume_TG1(%d),regTG1Val(0x%08x)",
	g_bPass1_On_In_Resume_TG1,
	regTG1Val);
...
static signed int ISP_resume(struct platform_device *pDev)
{
	...
	LOG_DBG(
	"g_bPass1_On_In_Resume_TG1(%d),regTG1Val(0x%x)",
	g_bPass1_On_In_Resume_TG1,
	regTG1Val);

	if (g_bPass1_On_In_Resume_TG1) {
		g_bPass1_On_In_Resume_TG1 = 0;
		ISP_WR32((void *)(ISP_ADDR + 0x414),
 ...
 *************************************************/
#ifdef CONFIG_PM
signed int ISP_pm_suspend(struct device *device)
{
...
signed int ISP_pm_resume(struct device *device)
{
	struct platform_device *pdev = to_platform_device(device);
...
signed int ISP_pm_restore_noirq(struct device *device)
{
	pr_debug("calling %s()\n", __func__);
#if 0				/* fix me later */
...
static signed int ISP_DumpRegToProc(char *pPage,
				char **ppStart,
				off_t off, signed int Count,
				signed int *pEof, void *pData)
#else
...
#endif
{
#if FSOP_USE_OLD_TYLE
	char RegBuf[64];
	unsigned int CopyBufSize = (Count < (sizeof(RegBuf) - 1)) ?
	...
	if (sscanf(RegBuf, "%x %x", &Addr, &Data) == 2) {
		...
		LOG_ERR(
		"Write => Addr: 0x%08X, Write Data: 0x%08X. Read Data: 0x%08X.",
		(unsigned int)(ISP_ADDR_CAMINF + Addr), Data,
		ioread32((ISP_ADDR_CAMINF + Addr)));
	...
	LOG_DBG("Count(%d)", (signed int) Count);
	return Count;
#else
	return 0;
#endif
...
#endif
{
	...
	ioread32((ISP_ADDR_CAMINF + proc_regOfst)));
...
#endif
{
	...
	if (sscanf(RegBuf, "%x %x", &Addr, &Data) == 2) {
		...
		/*TODO: Must fixed by Device tree */
		LOG_ERR(
		"Write => Addr: 0x%08X, Write Data: 0x%08X. Read Data: 0x%08X.",
		...
		proc_regOfst = Addr;
		LOG_ERR(
		"Read => Addr: 0x%08X, Read Data: 0x%08X.",
		GPIO_BASE + Addr,
...
int port, unsigned int mva, void *data)
{
	...
	"[TF_%d]0x%08X %08X",
	port,
	(unsigned int)(ISP_TPIPE_ADDR + 0x000C),
	...
	"[TF_%d]0x%08X %08X",
	port,
	(unsigned int)(ISP_TPIPE_ADDR + 0x0074),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x0074));
	LOG_DBG(
	"[TF_%d]0x%08X %08X",
	port,
	(unsigned int)(ISP_TPIPE_ADDR + 0x0078),
	(unsigned int)ISP_RD32(ISP_ADDR + 0x0078));
	LOG_DBG(
	"[TF_%d]0x%08X %08X",
	port,
	(unsigned int)(ISP_TPIPE_ADDR + 0x007c),
	...
	case M4U_PORT_IMGO:
		LOG_DBG(
		"[TF_IMGO]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x0300),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x0300));
		LOG_DBG(
		"[TF_IMGO]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x0304),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x0304));
		LOG_DBG(
		"[TF_IMGO]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x0308),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x0308));
		LOG_DBG(
		"[TF_IMGO]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x030C),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x030C));
		LOG_DBG(
		"[TF_IMGO]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x0310),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x0310));
		LOG_DBG(
		"[TF_IMGO]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x031C),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x031C));
		break;
	case M4U_PORT_IMG2O:
		LOG_DBG(
		"[TF_IMG2O]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x0320),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x0320));
		LOG_DBG(
		"[TF_IMG2O]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x0324),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x0324));
		LOG_DBG(
		"[TF_IMG2O]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x0328),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x0328));
		LOG_DBG(
		"[TF_IMG2O]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x032C),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x032C));
		LOG_DBG(
		"[TF_IMG2O]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x0330),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x0330));
		LOG_DBG(
		"[TF_IMG2O]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x033C),
	...
	case M4U_PORT_LSCI:
		...
		LOG_DBG(
		"[TF_LSCI]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x00E8),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x00E8));
		LOG_DBG(
		"[TF_LSCI]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x00EC),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x00EC));
		LOG_DBG(
		"[TF_LSCI]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x00F0),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x00F0));
		LOG_DBG(
		"[TF_LSCI]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x026C),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x026C));
		LOG_DBG(
		"[TF_LSCI]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x0270),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x0270));
		LOG_DBG(
		"[TF_LSCI]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x0274),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x0274));
		LOG_DBG(
		"[TF_LSCI]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x0278),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x0278));
		LOG_DBG(
		"[TF_LSCI]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x027c),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x027c));
		break;
	case M4U_PORT_IMGI:
		LOG_DBG(
		"[TF_IMGI]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x0050),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x0050));
		LOG_DBG(
		"[TF_IMGI]0x%08X %08X",
		(unsigned int)(ISP_TPIPE_ADDR + 0x0054),
...
static signed int __init ISP_Init(void)
{
	...
	g_pBuf_kmalloc = kmalloc((RT_BUF_TBL_NPAGES + 2)
	* PAGE_SIZE, GFP_KERNEL);
	...
	for (i = 0; i < RT_BUF_TBL_NPAGES * PAGE_SIZE; i += PAGE_SIZE)
		...
		(((unsigned long)g_pTbl_RTBuf) + i));


	/* Register M4U callback dump */
	LOG_DBG("register M4U callback dump");
	m4u_register_fault_callback(M4U_PORT_IMGO,
	ISP_M4U_TranslationFault_callback, NULL);
	m4u_register_fault_callback(M4U_PORT_IMG2O,
	ISP_M4U_TranslationFault_callback, NULL);
	m4u_register_fault_callback(M4U_PORT_LSCI,
	ISP_M4U_TranslationFault_callback, NULL);
	m4u_register_fault_callback(M4U_PORT_IMGI,
	ISP_M4U_TranslationFault_callback, NULL);
	m4u_register_fault_callback(M4U_PORT_ESFKO,
	ISP_M4U_TranslationFault_callback, NULL);
	m4u_register_fault_callback(M4U_PORT_AAO,
	ISP_M4U_TranslationFault_callback, NULL);
	ISP_Init_FrmB();
