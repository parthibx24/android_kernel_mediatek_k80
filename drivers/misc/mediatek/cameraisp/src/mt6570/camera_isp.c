...
********************************************************************************/
#ifndef ISP_WR32
#define ISP_WR32(addr, data)    iowrite32(data, (void *)addr)	/* For other projects. */
...
#define MIPIRX_ANALOG_ADDR   0x10011000	/* the same with the value in seninf_drv.cpp(chip-dependent) */
#define MIPIRX_ANALOG_RANGE (0x4000)
#define GPIO_BASE_ADDR       0x10017000	/* the same with the value in seninf_drv.cpp(chip-dependent) */
...
volatile spinlock_t SpinLockCamHaVer;
...
static __tcmfunc irqreturn_t ISP_Irq(MINT32 Irq, MVOID *DeviceId)
{
	...
	volatile MUINT32 IrqStatus_FBC = 0;
	...
	if (!CAM_HAL_VER_IS3) {
		...
		/* 4. wake up users */
		wake_up_interruptible((wait_queue_head_t *)(&IspInfo_FrmB.WaitQueueHead));	/* could move to ISP_Irq_FrmB */
	}
...
static long ISP_ioctl(struct file *pFile, MUINT32 Cmd, unsigned long Param)
{
...
#if 1				/* from abrams, rainier is 17:15:rg_cam_mux_sel */
				clkmux_sel(MT_CLKMUX_CAM_MUX_SEL, senFreq, "CAMERA_SENSOR");
				/* clkmux_sel(MT_CLKMUX_SCAM_MUX_SEL, senFreq, "CAMERA_SENSOR"); */
#else
	...
	case ISP_WAIT_IRQ_FRMB:
		if (copy_from_user
			...
			if ((Irq_FrmB.UserInfo.UserKey >= IRQ_USER_NUM_MAX)
				...
				Irq_FrmB.UserInfo.UserKey = 0;	/* force to use default user key if the passed key is invalid */
			}
	...
	case ISP_GET_INT_ERR_FRMB:
		if (copy_to_user((void __user *)Param, (void *)g_ISPIntErr, sizeof(MUINT32) * _IRQ_MAX) !=
		    0) {
...
static MINT32 ISP_DumpRegToProc(char *pPage,
				char **ppStart,
				off_t off, MINT32 Count, MINT32 *pEof, MVOID *pData)
#else
...
#endif
{
	...
	if (sscanf(RegBuf, "%x %x", &Addr, &Data) == 2) {
		...
		LOG_ERR("Read => Addr: 0x%08X, Read Data: 0x%08X.",
			(unsigned int)(ISP_ADDR_CAMINF + Addr), ioread32((ISP_ADDR_CAMINF + Addr)));
	}
