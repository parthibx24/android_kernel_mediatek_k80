...
{
	MUINT32 ret;
	unsigned long flags;/*MUINT32 flags;*/
	/*  */
...
static int sof_pass1done[2] = { 0, 0 };	/* record lost p1_done or not, 1 for lost p1_done. 0 for normal , 2 for last working buffer. */
...
static long ISP_Buf_CTRL_FUNC_FRMB(unsigned long Param)
{
	...
	if (copy_from_user(&rt_buf_ctrl, (void __user *)Param, sizeof(ISP_BUFFER_CTRL_STRUCT_FRMB))
		...
		case ISP_RT_BUF_CTRL_CLEAR_FRMB:
			...
			case _imgo_:
			case _img2o_:
				memset((void *)&g_DmaErr_p1[0], 0, sizeof(MUINT32) * nDMA_ERR_P1);
				memset((void *)IspInfo_FrmB.IrqInfo.LastestSigTime_usec[ISP_IRQ_TYPE_INT],
				       0, sizeof(MUINT32) * 32);
				memset((void *)IspInfo_FrmB.IrqInfo.LastestSigTime_sec[ISP_IRQ_TYPE_INT], 0,
				       sizeof(MUINT32) * 32);
				memset((void *)IspInfo_FrmB.IrqInfo.Eismeta[ISP_IRQ_TYPE_INT], 0,
...
static MINT32 ISP_ED_BufQue_CTRL_FUNC_FRMB(ISP_ED_BUFQUE_STRUCT_FRMB param)
{
	...
	case ISP_ED_BUFQUE_CTRL_WAIT_DEQUE:	/* a dequeue thread is waiting to do dequeue */
		...
		if (idx == -1) {
			...
			restTime = wait_event_interruptible_timeout(*((wait_queue_head_t *)(&WaitQueueHead_EDBuf_WaitDeque)),
	...
	case ISP_ED_BUFQUE_CTRL_WAIT_FRAME:	/* wait for a specific buffer */
		...
		if (P2_EDBUF_MgrList[idx].dequedNum == P2_Support_BurstQNum) {
			...
			restTime = wait_event_interruptible_timeout(*((wait_queue_head_t *)(&WaitQueueHead_EDBuf_WaitFrame)),
		...
		if (MTRUE == bCopyFromUser) {
			...
			if (FirstUnusedIrqUserKey >= IRQ_USER_NUM_MAX || FirstUnusedIrqUserKey < 0) {
				...
				for (i = 1; i < FirstUnusedIrqUserKey; i++) {	/* index 0 is for all the users that do not register irq first */
					if (strcmp((char *)IrqUserKey_UserInfo[i].userName, m_UserName) ==
					    0) {
...
static MINT32 ISP_FLUSH_IRQ(ISP_WAIT_IRQ_STRUCT_FRMB irqinfo)
{
	...
	wake_up_interruptible((wait_queue_head_t *)(&IspInfo_FrmB.WaitQueueHead));
...
static MINT32 ISP_WaitIrq_FrmB(ISP_WAIT_IRQ_STRUCT_FRMB *WaitIrq)
{
	...
	Timeout = wait_event_interruptible_timeout(*((wait_queue_head_t *)(&IspInfo_FrmB.WaitQueueHead)),
...
static MINT32 __init ISP_Init_FrmB(void)
{
	...
	LOG_DBG("- E.");
	/*  */
	pstRTBuf_FrmB = NULL; /* for kernel coding rule: ERROR:INITIALISED_STATIC: do not initialise statics to 0 or NULL */
	/*  */
