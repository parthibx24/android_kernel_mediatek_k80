...
static int cldma_queue_broadcast_state(struct ccci_modem *md, unsigned int state, DIRECTION dir, int index)
{
	struct port_proxy *proxy_p;

	proxy_p = md->port_proxy_obj;
	port_proxy_queue_status_notice(proxy_p, index, dir, state);
	return 0;
}
		...
		if (new_skb) {
			...
			req->data_buffer_ptr_saved =
			    dma_map_single(&md->plat_dev->dev, new_skb->data, skb_data_size(new_skb),
								DMA_FROM_DEVICE);
			if (dma_mapping_error(&md->plat_dev->dev, req->data_buffer_ptr_saved)) {
				CCCI_ERROR_LOG(md->index, TAG, "error dma mapping\n");
...
#endif
#endif
	__pm_wakeup_event(&md_ctrl->trm_wake_lock, 10 * HZ);
...
static irqreturn_t md_cd_ccif_isr(int irq, void *data)
{
	...
	if (md_ctrl->channel_id & (1<<AP_MD_PEER_WAKEUP))
		__pm_wakeup_event(&md_ctrl->peer_wake_lock, HZ);
...
static int md_cd_broadcast_state(struct ccci_modem *md, enum MD_STATE state)
{
...
static int ccci_modem_probe(struct platform_device *plat_dev)
{
	...
	wakeup_source_init(&md_ctrl->trm_wake_lock, md_ctrl->trm_wakelock_name);
	snprintf(md_ctrl->peer_wakelock_name, sizeof(md_ctrl->peer_wakelock_name), "md%d_cldma_peer", md_id + 1);
	wakeup_source_init(&md_ctrl->peer_wake_lock, md_ctrl->peer_wakelock_name);
