...
struct ccci_port_ops {
	...
	void (*md_state_notice)(struct ccci_port *port, enum MD_STATE state);
	void (*queue_state_notify)(struct ccci_port *port, int dir, int qno,
				   unsigned int qstate);
	void (*dump_info)(struct ccci_port *port, unsigned int flag);
};
...
void port_proxy_md_status_notice(struct port_proxy *proxy_p, DIRECTION dir,
							int filter_ch_no, int filter_queue_idx, enum MD_STATE state);
...
void port_proxy_queue_status_notice(struct port_proxy *proxy_p, int qno,
				    DIRECTION dir, unsigned int state);
/****************************************************************************************************************/
...
{
	__pm_wakeup_event(&proxy_p->wakelock, sec * HZ);
}
