...
struct ccci_modem_ops {
	...
	int (*broadcast_state)(struct ccci_modem *md, enum MD_STATE state);
...
struct ccci_modem {
	...
	volatile enum MD_STATE md_state;	/* check comments below, put it here for cache benefit */
...
void ccci_md_status_notice(struct ccci_modem *md, DIRECTION dir, int filter_ch_no,
								int filter_queue_idx, enum MD_STATE state);
...
enum MD_STATE_FOR_USER get_md_state_for_user(struct ccci_modem *md);
...
static inline enum MD_STATE ccci_md_get_state(struct ccci_modem *md)
{
	return md->md_state;
}
...
static inline int ccci_md_broadcast_state(struct ccci_modem *md, enum MD_STATE state)
{
