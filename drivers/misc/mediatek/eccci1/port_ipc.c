...
static int port_ipc_ack_recv_skb(struct ccci_port *port, struct sk_buff *skb)
{
	...
	ccci_free_skb(skb);
	__pm_wakeup_event(&port->rx_wakelock, HZ / 2);
	return 0;
}
...
void port_ipc_md_state_notice(struct ccci_port *port, enum MD_STATE state)
{
	struct ccci_ipc_ctrl *ipc_ctrl = (struct ccci_ipc_ctrl *)port->private_data;
...
static int port_ipc_kernel_write(struct ccci_modem *md, struct ipc_ilm *in_ilm)
{
	u32 task_id;
