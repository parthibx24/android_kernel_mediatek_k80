	...
	while (i > 0) {
		...
		case MTKSTP_SYNC:	/* b'10 */
			...
			if ((*p_data & 0x80) == 0x80) {
				...
				if (i >= 4) {
					...
					PINT8 type_name = "<UNKNOWN>";

					type_name = stp_type_to_dbg_string(type);
					STP_DBG_FUNC("STP Rx Header: [%02x %02x %02x %02x]\n",
							*p_data, *(p_data + 1), *(p_data + 2), *(p_data + 3));
					STP_DBG_FUNC("type=%s, len=%d, seq=%d, ack=%d\n",
							type_name, ((*(p_data + 1) & 0x0f) << 8) +
							*(p_data + 2), (*p_data & 0x38) >> 3, *p_data & 0x07);
#endif
				} else
		...
		case MTKSTP_FW_MSG:
			...
			if (stp_core_ctx.parser.type == STP_TASK_INDX &&
				...
				/* (*sys_timer_start)(stp_core_ctx.tx_timer, mtkstp_tx_timeout,
				 * (MTK_WCN_TIMER_CB)stp_tx_timeout_handler, NULL);
				 */
				STP_TRACE_FUNC("--\n");
				return 0;
			}
...
/* stp_dump_data(buffer, "rx queue", length); */
	...
	if (STP_NOT_ENABLE(stp_core_ctx) && WMT_TASK_INDX == STP_PENDING_TYPE(stp_core_ctx)) {
		/* (*sys_dbg_print)("mtk_wcn_stp_parser_data, buff = %x %x %x %x %x %x %x %x %x %x",
