...
INT_32 priv_driver_cmds(IN struct net_device *prNetDev, IN PCHAR pcCommand, IN INT_32 i4TotalLen)
{
	...
	if (i4CmdFound == 0) {
		...
		} else if (strncasecmp(pcCommand, CMD_RM_IT, strlen(CMD_RM_IT)) == 0) {
			kalIoctl(prGlueInfo, wlanoidRadioMeasurementIT, (PVOID)(pcCommand+6),
					 i4TotalLen, FALSE, FALSE, FALSE, &i4BytesWritten);
		} else if (strncasecmp(pcCommand, CMD_DUMP_UAPSD, strlen(CMD_DUMP_UAPSD)) == 0)
