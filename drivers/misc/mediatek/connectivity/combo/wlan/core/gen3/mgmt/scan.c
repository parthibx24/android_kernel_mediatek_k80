...
static BOOLEAN needReplaceCandidate(P_ADAPTER_T prAdapter, P_BSS_DESC_T prCandBss, P_BSS_DESC_T prCurrBss,
...
try_again:
	LINK_FOR_EACH_ENTRY(prBssDesc, prEssLink, rLinkEntryEss, BSS_DESC_T) {
		...
		DBGLOG(SCN, INFO,
		"%pM Score,Total %d:BW[%d],CI[%d],DE[%d],PR[%d],SM[%d],SC[%d],SR[%d,%d],ST[%d],BD[%d],Connected %d\n",
		...
		if (!prCandBssDesc ||
			needReplaceCandidate(prAdapter, prCandBssDesc, prBssDesc, u2CandBssScore, u2ScoreTotal)) {
