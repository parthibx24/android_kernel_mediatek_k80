...
P_QUE_T qmDetermineStaTxQueue(
	IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo, IN UINT_8 ucActiveTs, OUT PUINT_8 pucTC)
{
	...
	do {
		...
		if (prStaRec->fgIsQoS) {
			...
			if ((prBssInfo->arACQueParms[eAci].ucIsACMSet) &&
				!(ucActiveTs & BIT(eAci)) && (eAci != WMM_AC_BK_INDEX)) {
...
P_MSDU_INFO_T qmEnqueueTxPackets(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfoListHead)
{
	...
	do {
		...
		if (!fgDropPacket) {
			...
			default:
				prTxQue = qmDetermineStaTxQueue(prAdapter, prCurrentMsduInfo, ucActivedTspec, &ucTC);
				if (!prTxQue) {
				   ...
				   IN UINT_8 ucTC, IN UINT_32 u4CurrentQuota, IN UINT_32 u4TotalQuota)
{
	...
	UINT_8 ucAc;
	BOOLEAN fgAcmFlowCtrl = FALSE;
	static const UINT_8 aucTc2Ac[] = {ACI_BK, ACI_BE, ACI_VI, ACI_VO};
#endif
...
#if CFG_SUPPORT_SOFT_ACM
			if (ucTC <= TC3_INDEX && prStaRec->afgAcmRequired[aucTc2Ac[ucTC]]) {
				ucAc = aucTc2Ac[ucTC];
				DBGLOG(QM, TRACE, "AC %d Pending Pkts %u\n", ucAc, prCurrQueue->u4NumElem);
				/* Quick check remain medium time and pending packets */
				if (QUEUE_IS_EMPTY(prCurrQueue) || !wmmAcmCanDequeue(prAdapter, ucAc, 0))
					goto skip_dequeue;
				fgAcmFlowCtrl = TRUE;
			} else
				fgAcmFlowCtrl = FALSE;
...
#if CFG_SUPPORT_SOFT_ACM
				if (fgAcmFlowCtrl) {
					UINT_32 u4PktTxTime = 0;

					u4PktTxTime = wmmCalculatePktUsedTime(prBssInfo, prStaRec,
						prDequeuedPkt->u2FrameLength - ETH_HLEN);
					if (!wmmAcmCanDequeue(prAdapter, ucAc, u4PktTxTime))
						break;
				}
#endif
				/* Available to be Tx */
	...
	do {
		...
		if (HAL_RX_STATUS_GET_HEADER_TRAN(prRxStatus) == TRUE) { /* (!HIF_RX_HDR_GET_80211_FLAG(prHifRxHdr)){ */
			...
			if (prCurrSwRfb->prStaRec == NULL && RXM_IS_DATA_FRAME(u2FrameCtrl)) {
				...
				DBGLOG(QM, INFO,
				       "S/W Header Trans: FC=0x%04x GVLD=0x%x StaRecIdx=%d WlanIdx=%d PktLen=%d\n",
...
VOID qmInsertNoNeedWaitPkt(IN P_SW_RFB_T prSwRfb, IN ENUM_NO_NEED_WATIT_DROP_REASON_T eDropReason)
{
	...
	DBGLOG_LIMITED(QM, INFO, "qmInsertNoNeedWaitPkt SSN:[%u], DropReason:(%d)\n", prSwRfb->u2SSN, eDropReason);
...
VOID mqmParseAssocRspWmmIe(IN PUINT_8 pucIE, IN P_STA_RECORD_T prStaRec)
{
	...
	if ((WMM_IE_OUI_TYPE(pucIE) == VENDOR_OUI_TYPE_WMM) && (!kalMemCmp(WMM_IE_OUI(pucIE), aucWfaOui, 3))) {
		P_IE_WMM_PARAM_T prWmmParam = (P_IE_WMM_PARAM_T) pucIE;
		ENUM_ACI_T eAci;
		...
		case VENDOR_OUI_SUBTYPE_WMM_PARAM:
			if (IE_LEN(pucIE) != 24)
				break;	/* WMM Info IE with a wrong length */
			prStaRec->fgIsQoS = TRUE;
			prStaRec->fgIsUapsdSupported = !!(prWmmParam->ucQosInfo & WMM_QOS_INFO_UAPSD);
			for (eAci = ACI_BE; eAci < ACI_NUM; eAci++)
				prStaRec->afgAcmRequired[eAci] =
					!!(prWmmParam->arAcParam[eAci].ucAciAifsn & WMM_ACIAIFSN_ACM);
