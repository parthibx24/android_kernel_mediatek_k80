...
{
	UINT_8 ucTid;
	PVOID *pprTxDTemplate = NULL;

	if (!prStaRec)
		return;
	DBGLOG(TX, INFO, "Data Packets in Aci %d will route to %s\n", ucAci, fgToMcu ? "MCU":"LMAC");
	pprTxDTemplate = &prStaRec->aprTxDescTemplate[0];
	for (ucTid = 0; ucTid < TX_DESC_TID_NUM; ucTid++) {
		if (aucTid2ACI[ucTid] != ucAci)
			continue;
		HAL_MAC_TX_DESC_SET_PORT_INDEX((P_HW_MAC_TX_DESC_T)pprTxDTemplate[ucTid],
			fgToMcu ? PORT_INDEX_MCU:PORT_INDEX_LMAC);
	}
}

/* if some msdus are waiting tx done status, but now roaming done, then need to change
** wlan index of these msdus to match tx done status event
** In multi-thread solution, we also need to check if pending tx packets in hif_thread tx queue.
*/
VOID nicTxHandleRoamingDone(P_ADAPTER_T prAdapter, P_STA_RECORD_T prOldStaRec, P_STA_RECORD_T prNewStaRec)
{
	...
	UINT_8 ucOldWlanIndex = prOldStaRec->ucWlanIndex;
	UINT_8 ucNewWlanIndex = prNewStaRec->ucWlanIndex;

	KAL_SPIN_LOCK_DECLARATION();

	KAL_ACQUIRE_SPIN_LOCK(prAdapter, SPIN_LOCK_TXING_MGMT_LIST);
	prMsduInfo = (P_MSDU_INFO_T)QUEUE_GET_HEAD(&prAdapter->rTxCtrl.rTxMgmtTxingQueue);
	while (prMsduInfo) {
	...
	/* I think any time we disconnect with previous AP, rTxP0Queue and rTxP1Queue should be
	** empty. because we have stopped dequeue when initial to connect the new roaming AP.
	** It is enough time for hif_thread to send out these packets. But anyway, let's prepare code
	** for that case to avoid scheduler corner case.
	*/
#if CFG_SUPPORT_MULTITHREAD
	KAL_ACQUIRE_SPIN_LOCK(prAdapter, SPIN_LOCK_TX_PORT_QUE);
	prMsduInfo = (P_MSDU_INFO_T)QUEUE_GET_HEAD(&prAdapter->rTxP0Queue);
	while (prMsduInfo) {
		if (prMsduInfo->ucWlanIndex == ucOldWlanIndex)
			prMsduInfo->ucWlanIndex = ucNewWlanIndex;
		prMsduInfo = (P_MSDU_INFO_T)QUEUE_GET_NEXT_ENTRY(&prMsduInfo->rQueEntry);
	}
	prMsduInfo = (P_MSDU_INFO_T)QUEUE_GET_HEAD(&prAdapter->rTxP1Queue);
	while (prMsduInfo) {
		if (prMsduInfo->ucWlanIndex == ucOldWlanIndex)
			prMsduInfo->ucWlanIndex = ucNewWlanIndex;
		prMsduInfo = (P_MSDU_INFO_T)QUEUE_GET_NEXT_ENTRY(&prMsduInfo->rQueEntry);
