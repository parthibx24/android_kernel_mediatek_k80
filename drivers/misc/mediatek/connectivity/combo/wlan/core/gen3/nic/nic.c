			...
			IN PARAM_POWER_MODE ePowerMode, IN enum POWER_SAVE_CALLER_T ucCaller)
{
	...
	DBGLOG(NIC, INFO, "unicCheckPowerSaveSend u4OldValue=0x%04x, ucCaller=%d\n", u4OldValue, ucCaller);
	...
	DBGLOG(NIC, INFO, "unicCheckPowerSaveSend u4PowerSaveFlag[%d]=0x%04x\n", ucBssIndex,
		...
		IN BOOLEAN fgEnCmdEvent)
{
	...
	DBGLOG(NIC, INFO, "nicConfigPowerSaveProfile u4OldValue=0x%04x\n", u4OldValue);
	 /*set send command flag*/
	if (u4OldValue & PS_SYNC_WITH_FW) {
		prAdapter->rWlanInfo.u4PowerSaveFlag[ucBssIndex] &= ~PS_SYNC_WITH_FW;
		DBGLOG(NIC, INFO, "nicConfigPowerSaveProfile u4PowerSaveFlag[%d]=0x%04x\n", ucBssIndex,
			prAdapter->rWlanInfo.u4PowerSaveFlag[ucBssIndex]);
