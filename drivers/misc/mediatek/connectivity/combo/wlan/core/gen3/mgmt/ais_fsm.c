...
VOID aisGetAndSetScanChannel(IN P_ADAPTER_T prAdapter)
{
	...
	ucChannelp = prAdapter->prGlueInfo->aucChannelNum;
		...
		case AIS_STATE_LOOKING_FOR:
			...
			if ((prAisFsmInfo->eCurrentState == AIS_STATE_ONLINE_SCAN)
				...
				if ((pGlinfo->u4LastFullScanTime == 0) ||
					...
					kalMemSet(pGlinfo->aucChannelNum, 0, FULL_SCAN_MAX_CHANNEL_NUM);
					if (pGlinfo->pucFullScan2PartialChannel != NULL) {
						kalMemSet(pGlinfo->pucFullScan2PartialChannel, 0,
	...
	if (!u4ReqScan) {
		...
		*/
		if (prAdapter->prAisBssInfo && prAdapter->prAisBssInfo->prStaRecOfAP &&
			prAdapter->prAisBssInfo->prStaRecOfAP->fgSupportBTM) {
			...
			wnmSendBTMQueryFrame(prAdapter, prAdapter->prAisBssInfo->prStaRecOfAP);
		}
	}
