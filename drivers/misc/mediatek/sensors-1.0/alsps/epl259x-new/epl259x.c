  ...
  if(enable_hs)
  {
    ...
    epl_sensor_I2C_Write(client, 0x1b, buf);
...
#endif /*PS_GES*/
      {
        ...
        {
          buf = epl_sensor.ps.ps_intb_nonlos | epl_sensor.ps.integration_time | epl_sensor.ps.gain;
          epl_sensor_I2C_Write(client, 0x03, buf);

          buf = epl_sensor.ps.ps_rs | epl_sensor.ps.adc | epl_sensor.ps.cycle;
          epl_sensor_I2C_Write(client, 0x04, buf);

          buf = epl_sensor.ps.ps_std | epl_sensor.ps.ir_on_control | epl_sensor.ps.ir_mode | epl_sensor.ps.ir_drive;
          epl_sensor_I2C_Write(client, 0x05, buf);

          buf = epl_sensor.als.als_intb_nonlos | epl_sensor.als.integration_time | epl_sensor.als.gain;
          epl_sensor_I2C_Write(client, 0x01, buf);

          buf = epl_sensor.als.als_rs | epl_sensor.als.adc | epl_sensor.als.cycle;
          epl_sensor_I2C_Write(client, 0x02, buf);

          buf = epl_sensor.als.als_std | epl_sensor.als.interrupt_channel_select | epl_sensor.als.persist | epl_sensor.als.interrupt_type;
          epl_sensor_I2C_Write(client, 0x07, buf);
        }
        else
        {
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
  ...
  epl_sensor.als.report_type = CMC_BIT_TABLE; //CMC_BIT_RAW; //CMC_BIT_DYN_INT;
  ...
  switch(epl_sensor.als.report_type)
  {
    ...
    case CMC_BIT_DYN_INT:
      ...
      if(change_flag == true)
      {
        ...
        //epl_sensor_I2C_Write(obj->client, 0x01, epl_sensor.als.als_intb_nonlos | epl_sensor.als.integration_time | epl_sensor.als.gain); //setting with epl_sensor_fast_update()
...
#if MTK_LTE
    if((err = als_data_report(als, SENSOR_STATUS_ACCURACY_MEDIUM)))
    {
  ...
  if(epl_sensor.als.report_type == CMC_BIT_INTR_LEVEL || epl_sensor.als.report_type == CMC_BIT_DYN_INT)
  {
    ...
    if(epl_sensor.als.report_type == CMC_BIT_DYN_INT)
    {
      if(epl_sensor.revno != 0x8188)  //3638
      {
        ...
        thd_add = als_thd_add / als_rs_value[gRawData.raw_bytes[0]>>5];
        mutex_unlock(&sensor_mutex);
        APS_LOG("[%s]: thd_add=%d \r\n", __func__, thd_add);


        if(epl_sensor.als.high_threshold <= (0xffff-thd_add))
          epl_sensor.als.high_threshold = epl_sensor.als.high_threshold + thd_add;
...
void epl_sensor_fast_update(struct i2c_client *client)
{
  ...
  epl_sensor_I2C_Write(client, 0x11, EPL_POWER_OFF | EPL_RESETN_RESET);
  if(epl_sensor.als.polling_mode == 0)
    epl_sensor_I2C_Write(client, 0x07, epl_sensor.als.als_std | epl_sensor.als.interrupt_channel_select | epl_sensor.als.persist | EPL_INTTY_DISABLE);
#if ALS_DYN_INTT
  if(epl_sensor.revno != 0x8188)  //3638
  {
    if(dynamic_intt_idx == 0)
    ...
    {
      epl_sensor_I2C_Write(client, 0x02, EPL_RS_0 | epl_sensor.als.adc | EPL_CYCLE_1);
    }
  }
  else
#endif
  ...
  epl_sensor_I2C_Write(client, 0x01, epl_sensor.als.als_intb_nonlos | epl_sensor.als.integration_time | epl_sensor.als.gain);
  ...
  if(epl_sensor.als.polling_mode == 0)
  {
    ...
    epl_sensor_I2C_Write(client, 0x07, epl_sensor.als.als_std | epl_sensor.als.interrupt_channel_select | epl_sensor.als.persist | epl_sensor.als.interrupt_type);
  }
  ...
  if(epl_sensor.revno != 0x8188)  //3638
  {
    ...
    {
      epl_sensor_I2C_Write(client, 0x02, EPL_RS_0 | epl_sensor.als.adc | epl_sensor.als.cycle);
    }
  }
  else
#endif
...
int epl_sensor_setup_eint(struct i2c_client *client)
{
  ...
  if (obj->irq_node)
  {
    ...
    if (request_irq(obj->irq, epl_sensor_eint_func, IRQF_TRIGGER_LOW, "ALS-eint", NULL)) {
...
static ssize_t epl_sensor_store_integration(struct device_driver *ddri, const char *buf, size_t count)
{
  ...
  switch (epl_sensor.mode)
  {
    case EPL_MODE_PS:
      ...
      epl_sensor_I2C_Write(obj->client, 0x03, epl_sensor.ps.ps_intb_nonlos | epl_sensor.ps.integration_time | epl_sensor.ps.gain);
    ...
    case EPL_MODE_ALS: //als
...
#endif
      {
        ...
        epl_sensor_I2C_Write(obj->client, 0x01, epl_sensor.als.als_intb_nonlos | epl_sensor.als.integration_time | epl_sensor.als.gain);
...
static ssize_t epl_sensor_store_gain(struct device_driver *ddri, const char *buf, size_t count)
{
  ...
  switch (epl_sensor.mode)
  {
    case EPL_MODE_PS:
      ...
      epl_sensor_I2C_Write(epld->client, 0x03, epl_sensor.ps.ps_intb_nonlos | epl_sensor.ps.integration_time | epl_sensor.ps.gain);
      break;

    case EPL_MODE_ALS: //als
      epl_sensor.als.gain = value;
      epl_sensor_I2C_Write(epld->client, 0x01, epl_sensor.als.als_intb_nonlos | epl_sensor.als.integration_time | epl_sensor.als.gain);
      break;
...
static ssize_t epl_sensor_store_cycle(struct device_driver *ddri, const char *buf, size_t count)
{
  ...
  switch (epl_sensor.mode)
  {
    case EPL_MODE_PS:
      ...
      epl_sensor_I2C_Write(obj->client, 0x04, epl_sensor.ps.ps_rs | epl_sensor.ps.adc | epl_sensor.ps.cycle);
      epl_sensor_I2C_Read(obj->client, 0x04, 1);
      APS_LOG("%s: 0x04 = 0x%x (0x%x)\n", __FUNCTION__, epl_sensor.ps.ps_rs | epl_sensor.ps.adc | epl_sensor.ps.cycle, gRawData.raw_bytes[0]);
      break;

    case EPL_MODE_ALS: //als
      epl_sensor.als.cycle = (value & 0x7);
      epl_sensor_I2C_Write(obj->client, 0x02, epl_sensor.als.als_rs | epl_sensor.als.adc | epl_sensor.als.cycle);
      epl_sensor_I2C_Read(obj->client, 0x02, 1);
      APS_LOG("%s: 0x02 = 0x%x (0x%x)\n", __FUNCTION__, epl_sensor.als.als_rs | epl_sensor.als.adc | epl_sensor.als.cycle, gRawData.raw_bytes[0]);
      break;
  }
  epl_sensor_update_mode(obj->client);
...
static ssize_t epl_sensor_store_ps_polling_mode(struct device_driver *ddri, const char *buf, size_t count)
{
  ...
  epl_sensor_I2C_Write(epld->client, 0x07, epl_sensor.als.als_std | epl_sensor.als.interrupt_channel_select | epl_sensor.als.persist | epl_sensor.als.interrupt_type);
...
static ssize_t epl_sensor_show_ps(struct device_driver *ddri, char *buf)
{
  ...
  ssize_t len = 0;
  bool enable_ps;
  enable_ps = test_bit(CMC_BIT_PS, &epld->enable) && atomic_read(&epld->ps_suspend)==0;
#if !PS_DYN_K
...
#endif
  len += snprintf(buf + len, PAGE_SIZE - len, "%d", (epl_sensor.ps.data.data >> 4));
  return len;
}
...
static DRIVER_ATTR(ps,                          S_IWUSR | S_IRUGO, epl_sensor_show_ps,              NULL                                        );
...
static int als_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	...
	APS_LOG("epl259x als set delay = (%d) ok.\n", value);
...
static int __init epl_sensor_init(void)
{
  ...
  ret = get_alsps_dts_func(node, hw_info);
