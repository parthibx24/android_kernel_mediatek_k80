/* BMA22E motion sensor driver
 *
 * Author: Xiaoli.li <Xiaoli.li@mediatek.com>
 *
...
static int bma222_factory_enable_sensor(bool enabledisable, int64_t sample_periods_ms)
{
	int err;
	...
	err = gsensor_set_batch(0, sample_periods_ms * 1000000, 0);
	if (err) {
...
static int bma222_factory_get_data(int32_t data[3], int *status)
{
	return gsensor_get_data(&data[0], &data[1], &data[2], status);

}

static int bma222_factory_get_raw_data(int32_t data[3])
{
	char strbuf[BMA222_BUFSIZE] = { 0 };

	BMA222_ReadRawData(bma222_i2c_client, strbuf);
	GSE_LOG("support bma222_factory_get_raw_data!\n");
	return 0;
}
static int bma222_factory_enable_calibration(void)
{
	return 0;
}
static int bma222_factory_clear_cali(void)
{
	int err = 0;

	err = BMA222_ResetCalibration(bma222_i2c_client);
	if (err) {
		GSE_ERR("bma222_ResetCalibration failed!\n");
		return -1;
	}
	return 0;
}

static int bma222_factory_set_cali(int32_t data[3])
{
	int err = 0;
	int cali[3] = { 0 };
...
static struct accel_factory_fops bma222_factory_fops = {
	.enable_sensor = bma222_factory_enable_sensor,
	.get_data = bma222_factory_get_data,
	.get_raw_data = bma222_factory_get_raw_data,
	.enable_calibration = bma222_factory_enable_calibration,
	.clear_cali = bma222_factory_clear_cali,
	.set_cali = bma222_factory_set_cali,
	.get_cali = bma222_factory_get_cali,
	.do_self_test = bma222_factory_do_self_test,
};

static struct accel_factory_public bma222_factory_device = {
	.gain = 1,
	.sensitivity = 1,
	.fops = &bma222_factory_fops,
};
...
static int bma222_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* factory */
	err = accel_factory_device_register(&bma222_factory_device);
	if (err) {
...
static int bma222_i2c_remove(struct i2c_client *client)
{
	...
	accel_factory_device_deregister(&bma222_factory_device);
