...
static struct tipc_msg_buf *_alloc_msg_buf(size_t sz)
{
	...
	mb->buf_va = buf_shm->kaddr;
	mb->buf_sz = sz;
	mb->shm_handle = buf_shm;
...
{
#ifdef TEEI_SHM_POOL
	teei_shm_kfree((struct teei_shm *)(mb->shm_handle));
#else
...
static int tipc_virtio_probe(struct virtio_device *vdev)
{
	...
	TEEI_BOOT_FOOTPRINT("TEEI Tipc Probe End");
...
static void tipc_virtio_remove(struct virtio_device *vdev)
{
	...
	TEEI_BOOT_FOOTPRINT("TEEI Tipc Remove Start");
	...
	TEEI_BOOT_FOOTPRINT("TEEI Tipc Remove End");
}
