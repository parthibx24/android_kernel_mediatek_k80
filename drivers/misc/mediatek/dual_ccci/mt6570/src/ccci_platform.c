...
int get_dts_settings(int md_id)
{
	...
	CCCI_MSG_INF(md_id, "plt", "md%d get ccif reg:0x%x, md ccif reg:0x%x, ccif_irq:%d, wdt_irq:%d\n",
			     (md_id + 1), (int)s_ccif_hw_info.reg_base, (int)s_ccif_hw_info.md_reg_base,
			     (int)s_ccif_hw_info.irq_id, (int)md_wdt_irq_id[md_id]);
