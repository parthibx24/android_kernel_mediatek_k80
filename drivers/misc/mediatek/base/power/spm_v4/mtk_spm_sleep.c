...
static void spm_trigger_wfi_for_sleep(struct pwr_ctrl *pwrctrl)
{
	if (is_cpu_pdn(pwrctrl->pcm_flags))
		...
		mt_secure_call(MTK_SIP_KERNEL_SPM_ARGS, SPM_ARGS_SUSPEND, 0, 0, 0);
		mt_secure_call(MTK_SIP_KERNEL_SPM_LEGACY_SLEEP, 0, 0, 0, 0);
		mt_secure_call(MTK_SIP_KERNEL_SPM_ARGS, SPM_ARGS_SUSPEND_FINISH, 0, 0, 0);
	}
		...
		struct pwr_ctrl *pwrctrl)
{
	...
	mt_secure_call(MTK_SIP_KERNEL_SPM_SUSPEND_ARGS, pwrctrl->pcm_flags,
			pwrctrl->pcm_flags1, pwrctrl->timer_val, 0);
}
...
int spm_set_sleep_wakesrc(u32 wakesrc, bool enable, bool replace)
{
	...
	mt_secure_call(MTK_SIP_KERNEL_SPM_PWR_CTRL_ARGS, SPM_PWR_CTRL_SUSPEND,
			PWR_WAKE_SRC, __spm_suspend.pwrctrl->wake_src, 0);
...
u32 spm_get_sleep_wakesrc(void)
{
	return mt_secure_call(MTK_SIP_KERNEL_SPM_GET_PWR_CTRL_ARGS,
			SPM_PWR_CTRL_SUSPEND, PWR_WAKE_SRC, 0, 0);
}
...
unsigned int spm_go_to_sleep(u32 spm_flags, u32 spm_data)
{
	...
	mt_secure_call(MTK_SIP_KERNEL_SPM_ARGS, SPM_ARGS_PCM_WDT, 1, 30, 0);
...
RESTORE_IRQ:
	...
	mt_secure_call(MTK_SIP_KERNEL_SPM_ARGS, SPM_ARGS_PCM_WDT, 0, 0, 0);
