...
static void spm_dvfsfw_init(int curr_opp)
{
	...
	mt_secure_call(MTK_SIP_KERNEL_SPM_VCOREFS_ARGS, VCOREFS_SMC_CMD_0, curr_opp, dram_issue, 0);
...
int spm_vcorefs_pwarp_cmd(void)
{
	...
	for (opp = 0; opp < NUM_OPP; opp++)
		mt_secure_call(MTK_SIP_KERNEL_SPM_VCOREFS_ARGS, VCOREFS_SMC_CMD_2, opp, get_vcore_ptp_volt(opp), 0);
...
void spm_go_to_vcorefs(int spm_flags)
{
	...
	mt_secure_call(MTK_SIP_KERNEL_SPM_VCOREFS_ARGS, VCOREFS_SMC_CMD_1, spm_flags, 0, 0);
...
void spm_request_dvfs_opp(int id, enum dvfs_opp opp)
{
	...
	case 0: /* ZQTX */
		...
		mt_secure_call(MTK_SIP_KERNEL_SPM_VCOREFS_ARGS, VCOREFS_SMC_CMD_2, id, emi_req[opp], 0);
	...
	case 1: /* CMMGR */
		...
		if (__spm_get_dram_type() == SPMFW_LP3_1CH)
			mt_secure_call(MTK_SIP_KERNEL_SPM_VCOREFS_ARGS, VCOREFS_SMC_CMD_2, 0, emi_req_lp3[opp], 0);
		else
			mt_secure_call(MTK_SIP_KERNEL_SPM_VCOREFS_ARGS, VCOREFS_SMC_CMD_2, 0, emi_req[opp], 0);
		break;
	default:
		break;
	}
