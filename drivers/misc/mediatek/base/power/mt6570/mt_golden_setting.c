...
static int golden_test_proc_show(struct seq_file *m, void *v)
{
	...
	if (0 == _golden.nr_golden_setting) {
		...
		seq_puts(m, "echo disable > /proc/golden/golden_test\n");
		seq_puts(m, "2.   insert golden setting (tool mode): ");
		seq_puts(m, "echo 0x10000000 (addr) 0bxxxx_xxxx_xxxx_xxxx_0001_0100_1001_0100 (mask & golden value) > /proc/golden/golden_test\n");
		seq_puts(m, "(2.) insert golden setting (hex mode):  ");
		seq_puts(m, "echo 0x10000000 (addr) 0xFFFF (mask) 0x1494 (golden value) > /proc/golden/golden_test\n");
		seq_puts(m, "(2.) insert golden setting (dec mode):  ");
		seq_puts(m, "echo 268435456 (addr) 65535 (mask) 5268 (golden value) > /proc/golden/golden_test\n");
		seq_puts(m, "3.   set filter:                        ");
		seq_puts(m, "echo filter func_name [line_num] > /proc/golden/golden_test\n");
		seq_puts(m, "(3.) disable filter:                    ");
		seq_puts(m, "echo filter > /proc/golden/golden_test\n");
		seq_puts(m, "4.   enable snapshot:                   ");
		seq_puts(m, "echo enable > /proc/golden/golden_test\n");
		seq_puts(m, "5.   set compare mode:                  ");
		seq_puts(m, "echo compare > /proc/golden/golden_test\n");
		seq_puts(m, "(5.) set apply mode:                    ");
		seq_puts(m, "echo apply > /proc/golden/golden_test\n");
		seq_puts(m, "(5.) set color mode:                    ");
		seq_puts(m, "echo color > /proc/golden/golden_test\n");
		seq_puts(m, "(5.) set diff mode:                     ");
		seq_puts(m, "echo color > /proc/golden/golden_test\n");
		seq_puts(m, "(5.) disable compare/apply/color mode:  ");
		seq_puts(m, "echo normal > /proc/golden/golden_test\n");
		seq_puts(m, "6.   set register value (normal mode):  ");
		seq_puts(m, "echo set 0x10000000 (addr) 0x13201494 (reg val) > /proc/golden/golden_test\n");
		seq_puts(m, "(6.) set register value (mask mode):    ");
		seq_puts(m, "echo set 0x10000000 (addr) 0xffff (mask) 0x13201494 (reg val) > /proc/golden/golden_test\n");
		seq_puts(m, "(6.) set register value (bit mode):     ");
		seq_puts(m, "echo set 0x10000000 (addr) 0 (bit num) 1 (reg val) > /proc/golden/golden_test\n");
	} else {
