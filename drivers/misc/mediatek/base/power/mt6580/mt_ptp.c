...
#define VMAX_VAL		PTP_VOLT_TO_PMIC_VAL(130625)
#define VMIN_VAL		PTP_VOLT_TO_PMIC_VAL(115000)
...
static struct ptp_det ptp_detectors[NR_PTP_DET] = {
	...
	[PTP_DET_MCUSYS] = {
			    ...
			    .VBOOT = PTP_VOLT_TO_PMIC_VAL(130625),
			    },
};
...
static void ptp_init_det(struct ptp_det *det, struct ptp_devinfo *devinfo)
{
	...
	det->VMAX = VMAX_VAL;
	det->VMIN = VMIN_VAL;
	det->VBOOT = PTP_VOLT_TO_PMIC_VAL(130625);
...
static inline void handle_init_err_isr(struct ptp_det *det)
{
	...
	ptp_isr_info("====================================================\n");
	ptp_isr_info("PTP init err: PTPEN(0x%08X) = 0x%08X, PTPINTSTS(0x%08X) = 0x%08X\n",
		     (unsigned int)PTP_PTPEN, ptp_read(PTP_PTPEN),
		     (unsigned int)PTP_PTPINTSTS, ptp_read(PTP_PTPINTSTS));
	ptp_isr_info("PTP_SMSTATE0 (0x%08X) = 0x%08X\n",
		     (unsigned int)PTP_SMSTATE0, ptp_read(PTP_SMSTATE0));
	ptp_isr_info("PTP_SMSTATE1 (0x%08X) = 0x%08X\n",
		     (unsigned int)PTP_SMSTATE1, ptp_read(PTP_SMSTATE1));
	ptp_isr_info("====================================================\n");
...
static inline void handle_mon_mode_isr(struct ptp_det *det)
{
	...
	if ((temp > 0x4b) && (temp < 0xd3)) {
		ptp_isr_info(
		"thermal sensor init has not been completed.(temp = 0x%08X)\n", temp);
		goto out;
	}
...
static inline void handle_mon_err_isr(struct ptp_det *det)
{
	...
	ptp_isr_info("====================================================\n");
	ptp_isr_info("PTP mon err: PTPEN(0x%08X) = 0x%08X, PTPINTSTS(0x%08X) = 0x%08X\n",
		     (unsigned int)PTP_PTPEN, ptp_read(PTP_PTPEN),
		     (unsigned int)PTP_PTPINTSTS, ptp_read(PTP_PTPINTSTS));
	ptp_isr_info("PTP_SMSTATE0 (0x%08X) = 0x%08X\n",
		     (unsigned int)PTP_SMSTATE0, ptp_read(PTP_SMSTATE0));
	ptp_isr_info("PTP_SMSTATE1 (0x%08X) = 0x%08X\n",
		     (unsigned int)PTP_SMSTATE1, ptp_read(PTP_SMSTATE1));
	ptp_isr_info("PTP_TEMP (0x%08X) = 0x%08X\n", (unsigned int)PTP_TEMP, ptp_read(PTP_TEMP));
	ptp_isr_info("PTP_TEMPMSR0 (0x%08X) = 0x%08X\n",
		     (unsigned int)PTP_TEMPMSR0, ptp_read(PTP_TEMPMSR0));
	ptp_isr_info("PTP_TEMPMSR1 (0x%08X) = 0x%08X\n",
		     (unsigned int)PTP_TEMPMSR1, ptp_read(PTP_TEMPMSR1));
	ptp_isr_info("PTP_TEMPMSR2 (0x%08X) = 0x%08X\n",
		     (unsigned int)PTP_TEMPMSR2, ptp_read(PTP_TEMPMSR2));
	ptp_isr_info("PTP_TEMPMONCTL0 (0x%08X) = 0x%08X\n",
		     (unsigned int)PTP_TEMPMONCTL0, ptp_read(PTP_TEMPMONCTL0));
	ptp_isr_info("PTP_TEMPMSRCTL1 (0x%08X) = 0x%08X\n",
		     (unsigned int)PTP_TEMPMSRCTL1, ptp_read(PTP_TEMPMSRCTL1));
	ptp_isr_info("====================================================\n");
