...
void accdet_detect(void)
{
	...
	pr_debug("[Accdet]accdet_detect\n");

	accdet_status = PLUG_OUT;
	ret = queue_work(accdet_workqueue, &accdet_work);
	if (!ret)
		pr_debug("[Accdet]accdet_detect:accdet_work return:%d!\n", ret);
}
...
void accdet_state_reset(void)
{
	...
	pr_debug("[Accdet]accdet_state_reset\n");
...
static bool accdet_timeout_ns(u64 start_time_ns, u64 timeout_time_ns)
{
	...
	if (cur_time < start_time_ns) {
		pr_debug("@@@@Timer overflow, start%lld cur timer%lld\n",
			start_time_ns, cur_time);
		start_time_ns = cur_time;
		timeout_time_ns = 400 * 1000;/* 400us */
		pr_debug("@@@@reset timer, start%lld setting%lld\n",
	...
	if (timeout_time_ns <= elapse_time) {
		/* timeout */
		pr_debug("@@@@ACCDET IRQ clear Timeout\n");
		return false;
	}
	return true;
}
...
static inline void headset_plug_out(void)
{
	...
	if (cur_key != 0) {
		...
		pr_debug("[accdet]headset_plug_out send key = %d release\n",
			cur_key);
		cur_key = 0;
	}
	pr_debug("[accdet]set state in cable_type = NO_DEVICE\n");
 ...
 */
	...
	while (pmic_pwrap_read(ACCDET_IRQ_STS) & IRQ_STATUS_BIT) {
		pr_debug("[Accdet]check_cable_type: Clear interrupt on-going\n");
		msleep(20);
	}
	...
	/*disable ACCDET unit*/
	pr_debug("accdet: disable_accdet\n");
...
static void disable_micbias(unsigned long a)
{
	...
	if (!ret)
		pr_debug("[Accdet]disable_micbias:accdet_work return:%d!\n", ret);
}
...
static void disable_micbias_callback(struct work_struct *work)
{
	...
	if (cable_type == HEADSET_NO_MIC) {
		...
		pr_debug("[Accdet] cable_pin_recognition = %d\n",
		...
		/*pr_debug("[Accdet] FSA8049 disable!\n");*/
#endif
		disable_accdet();
		pr_debug("[Accdet] more than 5s MICBIAS : Disabled\n");
	}
#ifdef CONFIG_ACCDET_PIN_RECOGNIZATION
	else if (cable_type == HEADSET_MIC) {
		pmic_pwrap_write(ACCDET_PWM_WIDTH, cust_headset_settings->pwm_width);
		pmic_pwrap_write(ACCDET_PWM_THRESH, cust_headset_settings->pwm_thresh);
		pr_debug("[Accdet]pin recog after 5s recover micbias polling!\n");
	}
#endif
}
...
static void accdet_eint_work_callback(struct work_struct *work)
{
	...
	if (cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		pr_debug("[Accdet] FSA8049 enable!\n");
		...
		pr_debug("[Accdet]pin recog start, micbias always on!\n");
#endif
		/*set PWM IDLE  on*/
		pmic_pwrap_write(ACCDET_STATE_SWCTRL,
			(pmic_pwrap_read(ACCDET_STATE_SWCTRL) | ACCDET_SWCTRL_IDLE_EN));
...
/*EINT_PIN_PLUG_OUT*/
/*Disable ACCDET*/
		pr_debug("[Accdet]EINT func:plug-out,cur_eint_state = %d\n",
			cur_eint_state);
		mutex_lock(&accdet_eint_irq_sync_mutex);
 ...
 */
	...
	pr_debug("[Accdet]accdet_eint_func after cur_eint_state=%d\n",
		cur_eint_state);
...
{
	/* 0.24V ~ */
	/* pr_debug("[accdet] come in key_check!!\n"); */
	if ((b < accdet_dts_data.three_key.down_key) &&
		...
		return MD_KEY;
	pr_debug("[accdet] leave key_check!!\n");
	return NO_KEY;
}
#else
static int key_check(int b)
...
static void send_accdet_status_event(int cable_type, int status)
{
	...
	case HEADSET_NO_MIC:
		...
		pr_debug("[accdet]Headphone(3-pole)%s\n",
	...
	case HEADSET_MIC:
		...
		input_report_switch(kpd_accdet_dev, SW_JACK_PHYSICAL_INSERT, status);
		input_sync(kpd_accdet_dev);
		pr_debug("[accdet]MICROPHONE(4-pole) %s\n",
			status?"PlugIn":"PlugOut");
		break;
	default:
		pr_debug("[accdet][send_accdet_status_Inputevent]Invalid cableType\n");
	}
}
...
static void multi_key_detection(int current_status)
{
	...
	if (current_status == 0) {
		...
		if (cali_voltage < 0) {
			pr_debug("[Accdet]read auxadc cali_vol:%d adjust to 0\n",
				cali_voltage);
			cali_voltage = 0;
		}
		pr_debug("[Accdet]adc cali_voltage = %d mV\n", cali_voltage);
#ifdef CONFIG_FOUR_KEY_HEADSET
		if (cali_voltage < 100) {
...
/* skip the case that user press the key very very quickly and repeatedly */
	...
	if (((pmic_pwrap_read(ACCDET_IRQ_STS) & IRQ_STATUS_BIT) != IRQ_STATUS_BIT) || eint_accdet_sync_flag) {
		...
		pr_debug("[Accdet]plug out side effect key press, do not report key = %d\n",
			cur_key);
		cur_key = NO_KEY;
	}
	if (current_status)
...
static void accdet_workqueue_func(void)
{
	...
	ret = queue_work(accdet_workqueue, &accdet_work);
	if (!ret)
		pr_debug("[Accdet]accdet_work return:%d!\n", ret);
}
...
static inline void clear_accdet_interrupt(void)
{
	...
	pr_debug("[Accdet]clear_accdet_interrupt: ACCDET_IRQ_STS = 0x%x\n",
		pmic_pwrap_read(ACCDET_IRQ_STS));
}
...
#define PIN_ADC_CHANNEL 5
	...
	pr_debug("[Accdet]accdet interrupt happen:[%s]current AB = %d\n",
	...
	case PLUG_OUT:
		...
		if (current_status == 0) {
			...
			pmic_pwrap_write(ACCDET_PWM_WIDTH, cust_headset_settings->pwm_width);
			pmic_pwrap_write(ACCDET_PWM_THRESH, cust_headset_settings->pwm_width);
			pr_debug("[Accdet]PIN recognition micbias always on!\n");
			pr_debug("[Accdet]before adc read, pin_adc_value = %d mv!\n",
				pin_adc_value);
			msleep(500);
			current_status = ((pmic_pwrap_read(ACCDET_STATE_RG) & 0xc0) >> 6);
			if (current_status == 0 && show_icon_delay != 0) {
				...
				pr_debug("[Accdet]pin_adc_value = %d mv!\n", pin_adc_value);
				accdet_auxadc_switch(0);
				...
				/* mt_set_gpio_out(GPIO_CAMERA_2_CMRST_PIN, GPIO_OUT_ONE); */
					...
					if (1 == eint_accdet_sync_flag) {
						...
						pr_debug("[Accdet]Headset has plugged out\n");
					}
					mutex_unlock(&accdet_eint_irq_sync_mutex);
				} else {
					mutex_lock(&accdet_eint_irq_sync_mutex);
			...
			pr_debug("[Accdet]PLUG_OUT state not change!\n");
#ifdef CONFIG_ACCDET_EINT
			pr_debug("[Accdet]do not send plug out event in plug out\n");
#else
			mutex_lock(&accdet_eint_irq_sync_mutex);
			if (1 == eint_accdet_sync_flag) {
				accdet_status = PLUG_OUT;
			...
			pr_debug("[Accdet]PLUG_OUT can't change to this state!\n");
		}
		break;
		...
		if (current_status == 0) {
			...
			if (1 == eint_accdet_sync_flag) {
				while ((pmic_pwrap_read(ACCDET_IRQ_STS) & IRQ_STATUS_BIT)
					...
					pr_debug("[Accdet]check_cable_type: MIC BIAS clear IRQ on-going1\n");
			...
			if (button_status) {
				...
				if (1 == eint_accdet_sync_flag)
					...
					pr_debug("[Accdet]multi_key_detection: Headset has plugged out\n");
				mutex_unlock(&accdet_eint_irq_sync_mutex);
				accdet_auxadc_switch(0);
			...
			if (1 == eint_accdet_sync_flag) {
				accdet_status = MIC_BIAS;
				cable_type = HEADSET_MIC;
				pr_debug("[Accdet]MIC_BIAS state not change!\n");
			} else {
				pr_debug("[Accdet]Headset has plugged out\n");
			}
			mutex_unlock(&accdet_eint_irq_sync_mutex);
		} else if (current_status == 3) {
			pr_debug("[Accdet]do not send plug ou in micbiast\n");
			mutex_lock(&accdet_eint_irq_sync_mutex);
			if (1 == eint_accdet_sync_flag)
				accdet_status = PLUG_OUT;
			else
			...
			pr_debug("[Accdet]MIC_BIAS can't change to this state!\n");
		}
		break;
	...
	case HOOK_SWITCH:
		if (current_status == 0) {
			...
			if (eint_accdet_sync_flag == 1) {
				...
				/* cable_type = HEADSET_NO_MIC; */
				/* accdet_status = HOOK_SWITCH; */
				pr_debug("[Accdet]HOOK_SWITCH state not change!\n");
			} else {
				pr_debug("[Accdet]Headset has plugged out\n");
			}
			mutex_unlock(&accdet_eint_irq_sync_mutex);
...
#endif
#if defined CONFIG_ACCDET_EINT
			pr_debug("[Accdet]don't send plug out event in hook switch\n");
			mutex_lock(&accdet_eint_irq_sync_mutex);
			if (eint_accdet_sync_flag == 1)
				accdet_status = PLUG_OUT;
			else
			...
			pr_debug("[Accdet]HOOK_SWITCH can't change to this state!\n");
		}
		break;
	case STAND_BY:
		if (current_status == 3) {
#if defined CONFIG_ACCDET_EINT
			pr_debug("[Accdet]accdet don't send plug out event in stand by\n");
#endif
		} else {
			pr_debug("[Accdet]STAND_BY can't change to this state\n");
		}
		break;

	default:
		pr_debug("[Accdet]check_cable_type:accdet current status error\n");
		break;
	...
	if (!IRQ_CLR_FLAG) {
		...
		if (eint_accdet_sync_flag == 1) {
			while ((pmic_pwrap_read(ACCDET_IRQ_STS) & IRQ_STATUS_BIT) &&
				(wait_clear_irq_times < 3)) {
				pr_debug("[Accdet]check_cable_type:Clear interrupt on-going2\n");
				wait_clear_irq_times++;
				msleep(20);
			}
		}
		...
		mutex_unlock(&accdet_eint_irq_sync_mutex);
		IRQ_CLR_FLAG = true;
		pr_debug("[Accdet]check_cable_type:Clear interrupt:Done[0x%x]!\n",
			pmic_pwrap_read(ACCDET_IRQ_STS));
	...
	pr_debug("[Accdet]cable type:[%s],status switch:[%s]->[%s]\n",
...
static inline void accdet_init(void)
{
	pr_debug("[Accdet]accdet hardware init\n");
	/* clock */
	pmic_pwrap_write(TOP_CKPDN_CLR, RG_ACCDET_CLK_CLR);
	...
	const char *buf, size_t count)
{
	...
	if (ret != 0) {
		pr_debug("accdet: Invalid values\n");
		return -EINVAL;
	}

	switch (call_status) {
	case CALL_IDLE:
		pr_debug("[Accdet]accdet call: Idle state!\n");
		break;

	case CALL_RINGING:

		pr_debug("[Accdet]accdet call: ringing state!\n");
		break;

	case CALL_ACTIVE:
		pr_debug("[Accdet]accdet call: active or hold state!\n");
		break;

	default:
		pr_debug("[Accdet]accdet call : Invalid values\n");
		break;
	}
	return count;
}
...
{
#ifdef CONFIG_ACCDET_PIN_RECOGNIZATION
	pr_debug("ACCDET show_pin_recognition_state = %d\n",
		cable_pin_recognition);
...
static DRIVER_ATTR(accdet_call_state, 0664, NULL,
	accdet_store_call_state);
	...
	const char *buf, size_t count)
{
	...
	if (start_flag == 1) {
		...
		if (IS_ERR(thread)) {
			...
			pr_debug(" failed to create kernel thread: %d\n", error);
		}
	}
	...
	const char *buf, size_t count)
{
	...
	pr_debug("[Accdet]store_accdet_set_headset_mode value=%d\n", value);
	...
	const char *buf, size_t count)
{
	...
	pr_debug("[Accdet]store_accdet_dump_register value=%d\n", value);
...
static DRIVER_ATTR(dump_register, S_IWUSR | S_IRUGO, NULL,
	store_accdet_dump_register);
static DRIVER_ATTR(set_headset_mode, S_IWUSR | S_IRUGO, NULL,
	store_accdet_set_headset_mode);
static DRIVER_ATTR(start_debug, S_IWUSR | S_IRUGO, NULL,
...
{
	pr_debug("[Accdet]accdet_remove begin!\n");
	...
	pr_debug("[Accdet]accdet_remove Done!\n");
}
...
#if defined CONFIG_ACCDET_EINT
	pr_debug("[Accdet]in suspend: ACCDET_IRQ_STS = 0x%x\n",
		pmic_pwrap_read(ACCDET_IRQ_STS));
#else
	pr_debug("[Accdet]suspend:ACCDET_CTRL=[0x%x],STATE=[0x%x]->[0x%x]\n",
...
{
#if defined CONFIG_ACCDET_EINT
	pr_debug("[Accdet]in resume1:ACCDET_IRQ_STS = 0x%x\n",
		pmic_pwrap_read(ACCDET_IRQ_STS));
#else
	pr_debug("[Accdet]resume: ACCDET_CTRL=[0x%x],STATE_SWCTRL=[0x%x]\n",
...
void mt_accdet_pm_restore_noirq(void)
{
	...
	pr_debug("[Accdet]pm_restore_noirq start!\n");
	/* enable ACCDET unit */
	pr_debug("accdet: enable_accdet\n");
	/* enable clock */
	pmic_pwrap_write(TOP_CKPDN_CLR, RG_ACCDET_CLK_CLR);
	...
	case 3:/* AB=3 */
		...
		break;
	default:
		pr_debug("[Accdet]pm_restore_noirq:current status error\n");
		break;
	}
...
long mt_accdet_unlocked_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case SET_CALL_STATE:
		call_status = (int)arg;
		pr_debug("[Accdet]accdet_ioctl:CALL_STATE=%d\n",
			call_status);
		break;
