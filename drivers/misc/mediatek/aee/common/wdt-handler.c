...
void aee_wdt_dump_info(void)
{
	...
	/* printk temporary buffer printk_buf[1024].
	 * To avoid char loss, add 4 bytes here
	 */
...
static void aee_save_reg_stack_sram(int cpu)
{
	...
	if (regs_buffer_bin[cpu].real_len != 0) {
		snprintf(str_buf, sizeof(str_buf),
			 ...
			 cpu,
			 ((wdt_percpu_preempt_cnt[cpu] &
			   PREEMPT_MASK) >> PREEMPT_SHIFT),
			 ((wdt_percpu_preempt_cnt[cpu] &
			   SOFTIRQ_MASK) >> SOFTIRQ_SHIFT),
			 ((wdt_percpu_preempt_cnt[cpu] &
			   HARDIRQ_MASK) >> HARDIRQ_SHIFT));
...
void aee_smp_send_stop(void)
{
	 ...
	 * //FIXME : temporarily disable for atf project bring up
	 */
#endif
	/* Wait up to one second for other CPUs to stop */
	timeout = USEC_PER_SEC;
...
void aee_wdt_irq_info(void)
{
	...
	aee_wdt_printf("\nQwdt at [%5lu.%06lu] ", (unsigned long)t,
			nanosec_rem / 1000);
	...
	wdt_log_length +=
	    dump_idle_info((wdt_log_buf + wdt_log_length),
			    (sizeof(wdt_log_buf) - wdt_log_length));
#endif
