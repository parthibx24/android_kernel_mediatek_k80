...
#else
#define HOST_MAX_MCLK			(208000000)
#endif
...
struct device_node *msdc_io_cfg_left_node;
...
void msdc_dump_register(struct msdc_host *host)
{
	...
	if ((host->id == 2) || (host->id == 3)) {
		...
		pr_err("sd%d R[%x]=0x%.8x\n", i, OFFSET_DAT1_TUNE_CRC,
			sdr_read32(DAT1_TUNE_CRC));
		pr_err("sd%d R[%x]=0x%.8x\n", i, OFFSET_DAT2_TUNE_CRC,
			sdr_read32(DAT2_TUNE_CRC));
		pr_err("sd%d R[%x]=0x%.8x\n", i, OFFSET_DAT3_TUNE_CRC,
...
{
#ifdef MTK_MSDC_BRINGUP_DEBUG
#if 0 /* wait pmic interface ready */
	...
	case 0:
		...
		pr_err(" VEMC_EN[0x0508][bit14=%s]=0x%x, VEMC_VOL[0x052E][bit7=%s]=0x%x\n",
			(((ldo_en >> 14) & 0x01) ? "enable" : "disable"), ldo_en,
			(((ldo_vol >> 7) & 0x01) ? "3.3V" : "3.0V"), ldo_vol);
		break;
	case 1:
		pwrap_read(0x0504, &ldo_en);
		pwrap_read(0x052A, &ldo_vol);
		pr_err(" VMC_EN[0x0504][bit12=%s]=0x%x, VMC_VOL[0x052A][bit4=%s]=0x%x\n",
			(((ldo_en >> 12) & 0x01) ? "enable" : "disable"), ldo_en,
			(((ldo_vol >> 4) & 0x01) ? "3.3V" : "1.8V"), ldo_vol);
		pwrap_read(0x0506, &ldo_en);
		pwrap_read(0x052C, &ldo_vol);
		pr_err(" VMCH_EN[0x0506][bit14=%s]=0x%x, VMCH_VOL[0x052C][bit7=%s]=0x%x\n",
			(((ldo_en >> 14) & 0x01) ? "enable" : "disable"), ldo_en,
			(((ldo_vol >> 7) & 0x01) ? "3.3V" : "3.0V"), ldo_vol);
		break;
	default:
...
void msdc_dump_gpio(struct msdc_host *host)
{
	...
	if (host->id == 0) {
		...
		pr_err("MSDC1 shall set pin mode to 1\n");
...
void msdc_dump_ioconfig_padctl(struct msdc_host *host)
{
	...
	if (host->id == 0)
		base = msdc_io_cfg_bottom_base;
	else if (host->id == 1)
		base = msdc_io_cfg_right_base;
	else
		return;
...
static u32 msdc_ldo_power(u32 on, MT65XX_POWER powerId, int voltage_uv,
							u32 *status)
{
#if 0 /* CONFIG_MTK_LEGACY but hwPowerOn(Dwon) API not exist, do nothing */
	if (on) {
		if (*status == 0) {
...
void msdc_set_tdrdsel(struct msdc_host *host, bool sd_18)
{
	switch (host->id) {
	case 0:
...
void msdc_set_pad_init(struct msdc_host	*host)
{
	...
	case 0:
		...
		* when rstb switch to msdc mode, need gpio pull up to drive high)
		*/
	...
	case 1:
		...
		*/
		sdr_set_field(MSDC1_GPIO_MODE7_MWR_ADDR, MSDC1_DAT0_PINMUX_BITS
		| MSDC1_DAT1_PINMUX_BITS, 0x99);
	...
	default:
		pr_err("error...msdc_set_pad_init out of range!!\n");
		break;
	}
}
...
static void msdc_emmc_power(struct msdc_host *host, u32 on)
{
	...
	msdc_ldo_power(on, MT6350_POWER_LDO_VEMC33, VOL_3000, &g_msdc0_flash);
	msdc_dump_ldo_sts(host);
}
...
static void msdc_sd_power(struct msdc_host *host, u32 on)
{
	...
	case 1:
		...
		msdc_set_tdrdsel(host, 0);
		if (host->hw->flags & MSDC_SD_NEED_POWER)
			msdc_ldo_power(1, MT6350_POWER_LDO_VMCH, VOL_3300, &g_msdc1_flash);
		else
			msdc_ldo_power(on, MT6350_POWER_LDO_VMCH, VOL_3300, &g_msdc1_flash);
		msdc_ldo_power(on, MT6350_POWER_LDO_VMC, VOL_3300, &g_msdc1_io);
		/* if (on)
...
static void msdc_sd_power_switch(struct msdc_host *host, u32 on)
{
	switch (host->id) {
	case 1:
		msdc_ldo_power(on, MT6350_POWER_LDO_VMC, VOL_1800, &g_msdc1_io);
		/* if (on)
...
static void msdc_select_clksrc(struct msdc_host *host, int clksrc)
{
	...
	else if (clksrc == MSDC0_CLKSRC_187MHZ)
	...
	if (host->id == 0) {
		...
		if (hz > 52000000) {
			...
			if (clksrc != MSDC0_CLKSRC_187MHZ) {
				pr_err("[SD%d] hz=(%dkHz), change clock source form (%d) to (%d)\n",
	...
	if (!data) {
		...
		if (0 == (host->autocmd & MSDC_AUTOCMD23)) {
			...
			if (mrq->sbc) {
				 ...
				 * RPMB region should remove MSDC_AUTOCMD12 flag
				 */
				if (host->hw->host_function == MSDC_EMMC) {
...
int msdc_tune_cmdrsp(struct msdc_host *host)
{
	...
	if (cur_rdly >= 32) {
		if (sel) {
			...
			sdr_set_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CMDRRDLY,
				cur_rrdly % 32);
		}
	}
	if (cur_rrdly >= 32) {
	...
	sdr_get_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CMDRRDLY, orig_rrdly);
	sdr_get_field(MSDC_PATCH_BIT1, MSDC_PB1_CMD_RSP_TA_CNTR, orig_cmdrtc);
	pr_err("msdc%d TUNE_CMD: rsmpl<%d> rdly<%d> rrdly<%d> cmdrtc<%d> sfreq.<%d>",
		host->id, orig_rsmpl, orig_rdly, orig_rrdly, orig_cmdrtc, host->sclk);
#ifdef STO_LOG
	if (unlikely(dumpMSDC())) {
		AddStorageTrace(STORAGE_LOGGER_MSG_MSDC_DO, msdc_do_request,
...
static void msdc_ops_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if ((ios->timing != MSDC_STATE_HS400) &&
		(host->hw->host_function == MSDC_EMMC))
		msdc_clock_src[host->id] = MSDC0_CLKSRC_208MHZ;
#endif
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	if (msdc_io_cfg_left_node == NULL) {
		...
		pr_err("of_iomap for IOCFG_L base @ 0x%p\n", msdc_io_cfg_left_base);
	}
	...
	if (msdc_io_cfg_top_node == NULL) {
		...
		pr_err("of_iomap for IOCFG_T base @ 0x%p\n", msdc_io_cfg_top_base);
	}
	...
	if (msdc_io_cfg_bottom_node == NULL) {
		...
		pr_err("of_iomap for IOCFG_B base @ 0x%p\n", msdc_io_cfg_bottom_base);
	}
	...
	if (msdc_io_cfg_right_node == NULL) {
		...
		pr_err("of_iomap for IOCFG_R base @ 0x%p\n", msdc_io_cfg_right_base);
	}
