...
#else
#define AUTOK_LATCH_CK_EMMC_TUNE_TIMES  (3)	/* 4.5IP eMMC 1KB fifo ZIZE */
#endif
			       ...
			       enum TUNE_TYPE tune_type_value)
{
	...
	wait_cond_tmo(!(sdr_read32(SDC_STS) & SDC_STS_SDCBUSY), tmo);
	if (tmo == 0) {
	...
	while ((sdr_read32(SDC_STS) & SDC_STS_SDCBUSY) && (tmo != 0)) {
		...
		if (tune_type_value == TUNE_LATCH_CK) {
			...
			wait_cond_tmo(!(sdr_read32(SDC_STS) & SDC_STS_SDCBUSY), write_tmo);
			if (write_tmo == 0) {
				AUTOK_RAWPRINT
				    ("[AUTOK]MSDC busy tmo2 while write cmd%d goto end...\n",
				     opcode);
				ret = E_RESULT_FATAL_ERR;
				goto end;
			}
...
skip_tune_latch_ck_and_tune_data:
	...
	if (opcode == MMC_STOP_TRANSMISSION) {
		tmo = AUTOK_DAT_TIMEOUT;
		wait_cond_tmo(((sdr_read32(MSDC_PS) & 0x10000) == 0x10000), tmo);
		if (tmo == 0) {
		...
		if (Bound_Cnt_R == 2) {
			...
			if (Bound_Cnt_F > 1) {
				...
				if (pBdPrev->Bound_Start == 0) {
					...
					/* rising edge sel */
					if (pBdPrev->Bound_Start + uBD_width / 2 - cycle_cnt / 2 <
					    0) {
						uDlySel_R = 0;
						uMgLost_R =
...
static int autok_ds_dly_sel(struct AUTOK_SCAN_RES *pInfo, unsigned int cycle_value,
			    unsigned int *pDlySel, u8 *param)
{
	unsigned int ret = 0;
	int uDlySel = 0;
...
static int autok_result_dump(struct msdc_host *host, u8 *autok_tune_res)
{
	...
	AUTOK_RAWPRINT("[AUTOK]DT  [D0:%d D1:%d D2:%d D3:%d]\r\n",
		       autok_tune_res[19], autok_tune_res[20], autok_tune_res[21],
		       autok_tune_res[22]);
	AUTOK_RAWPRINT("[AUTOK]DT  [D4:%d D5:%d D6:%d D7:%d]\r\n", autok_tune_res[23],
		       autok_tune_res[24], autok_tune_res[25], autok_tune_res[26]);
...
int autok_path_sel(struct msdc_host *host)
{
	...
	sdr_set_field(MSDC_PATCH_BIT2, MSDC_PB2_CRCSTSENSEL, AUTOK_CRC_LATCH_EN_HS_VALUE);
	/* LATCH_TA_EN Config for CMD Path HS FS mode */
	sdr_set_field(MSDC_PATCH_BIT2, MSDC_PB2_RESPSTENSEL, AUTOK_CMD_LATCH_EN_HS_VALUE);
...
int autok_init_sdr104(struct msdc_host *host)
{
	...
	/*      sdr_set_field(MSDC_CFG, MSDC_CFG_DVFS_EN, 1); */
...
int autok_init_hs200(struct msdc_host *host)
{
	...
	sdr_set_field(MSDC_PATCH_BIT2, MSDC_PB2_CRCSTSENSEL, AUTOK_CRC_LATCH_EN_HS200_PORT0_VALUE);
	/* LATCH_TA_EN Config for CMD Path non_HS400 */
	sdr_set_field(MSDC_PATCH_BIT2, MSDC_PB2_RESPSTENSEL, AUTOK_CMD_LATCH_EN_HS200_PORT0_VALUE);
...
int autok_init_hs400(struct msdc_host *host)
{
	...
	sdr_set_field(MSDC_PATCH_BIT2, MSDC_PB2_CRCSTSENSEL, AUTOK_CRC_LATCH_EN_HS400_PORT0_VALUE);
	/* LATCH_TA_EN Config for CMD Path HS400 */
	sdr_set_field(MSDC_PATCH_BIT2, MSDC_PB2_RESPSTENSEL, AUTOK_CMD_LATCH_EN_HS400_PORT0_VALUE);
...
int execute_online_tuning_hs400(struct msdc_host *host, u8 *res)
{
	...
	/* check tx tune res */
	if (tx_online_tune) {
		sdr_get_field(EMMC50_PAD_CMD_TUNE, MSDC_EMMC50_PAD_CMD_TUNE_TXDLY, pre_cmd_tx);
		sdr_get_field(EMMC50_PAD_DAT01_TUNE, MSDC_EMMC50_PAD_DAT0_TXDLY, pre_dat_tx);
				  ...
				  unsigned int latch_ck_initail_value)
{
	...
	for (j = latch_ck_initail_value; j < 8; j += (host->hclk / host->sclk)) {
		...
		for (k = 0; k < tune_time; k++) {
			...
			if ((ret & (E_RESULT_CMD_TMO | E_RESULT_RSP_CRC)) != 0) {
				AUTOK_RAWPRINT
				    ("[AUTOK]Error Autok CMD Failed while tune LATCH CK\r\n");
				break;
			} else if ((ret & (E_RESULT_DAT_CRC | E_RESULT_DAT_TMO)) != 0) {
	...
	for (i = 0; i < 32; i++) {
		...
		do {
			...
			for (j = 0; j < 64; j++) {
				...
				for (k = 0; k < AUTOK_CMD_TIMES; k++) {
					ret =
					    autok_send_tune_cmd(host, MMC_SEND_TUNING_BLOCK,
								TUNE_CMD);
					if ((ret & (E_RESULT_CMD_TMO | E_RESULT_RSP_CRC)) != 0) {
						RawData64 |= (u64) (1LL << j);
						break;
...
retry:
		...
		if (autok_ta_sel(g_ta_raw, &res_wcrc_ta) == 0) {
			msdc_autok_adjust_param(host, WRDAT_CRCS_TA_CNTR, &res_wcrc_ta,
						AUTOK_WRITE);
			AUTOK_RAWPRINT("[AUTOK]CMD_TA Sel:%d\r\n", res_wcrc_ta);
		} else {
			AUTOK_RAWPRINT
...
void autok_msdc_tx_setting(struct msdc_host *host, struct mmc_ios *ios)
{
	...
	if (host->id == 0) {
		if (ios->timing == MMC_TIMING_MMC_HS400) {
			sdr_set_field(EMMC50_CFG0,
				      MSDC_EMMC50_CFG_TXSKEWSEL, AUTOK_MSDC0_HS400_TXSKEW);
			sdr_set_field(MSDC_PAD_TUNE0,
				      MSDC_PAD_TUNE0_CLKTXDLY, AUTOK_MSDC0_HS400_CLKTXDLY);
			sdr_set_field(EMMC50_PAD_CMD_TUNE,
				      MSDC_EMMC50_PAD_CMD_TUNE_TXDLY, AUTOK_MSDC0_HS400_CMDTXDLY);
			sdr_set_field(EMMC50_PAD_DAT01_TUNE,
				      MSDC_EMMC50_PAD_DAT0_TXDLY, AUTOK_MSDC0_HS400_DAT0TXDLY);
			sdr_set_field(EMMC50_PAD_DAT01_TUNE,
				      MSDC_EMMC50_PAD_DAT1_TXDLY, AUTOK_MSDC0_HS400_DAT1TXDLY);
			sdr_set_field(EMMC50_PAD_DAT23_TUNE,
				      MSDC_EMMC50_PAD_DAT2_TXDLY, AUTOK_MSDC0_HS400_DAT2TXDLY);
			sdr_set_field(EMMC50_PAD_DAT23_TUNE,
				      MSDC_EMMC50_PAD_DAT3_TXDLY, AUTOK_MSDC0_HS400_DAT3TXDLY);
			sdr_set_field(EMMC50_PAD_DAT45_TUNE,
				      MSDC_EMMC50_PAD_DAT4_TXDLY, AUTOK_MSDC0_HS400_DAT4TXDLY);
			sdr_set_field(EMMC50_PAD_DAT45_TUNE,
				      MSDC_EMMC50_PAD_DAT5_TXDLY, AUTOK_MSDC0_HS400_DAT5TXDLY);
			sdr_set_field(EMMC50_PAD_DAT67_TUNE,
				      MSDC_EMMC50_PAD_DAT6_TXDLY, AUTOK_MSDC0_HS400_DAT6TXDLY);
			sdr_set_field(EMMC50_PAD_DAT67_TUNE,
				      MSDC_EMMC50_PAD_DAT7_TXDLY, AUTOK_MSDC0_HS400_DAT7TXDLY);
		} else {
			if (ios->timing == MMC_TIMING_MMC_DDR52) {
				sdr_set_field(MSDC_IOCON,
					      MSDC_IOCON_DDR50CKD, AUTOK_MSDC0_DDR50_DDRCKD);
			} else {
				sdr_set_field(MSDC_IOCON, MSDC_IOCON_DDR50CKD, 0);
			}
			sdr_set_field(MSDC_PAD_TUNE0,
				      MSDC_PAD_TUNE0_CLKTXDLY, AUTOK_MSDC0_CLKTXDLY);
			sdr_set_field(EMMC50_PAD_CMD_TUNE,
				      MSDC_EMMC50_PAD_CMD_TUNE_TXDLY, AUTOK_MSDC0_CMDTXDLY);
			sdr_set_field(EMMC50_PAD_DAT01_TUNE,
				      MSDC_EMMC50_PAD_DAT0_TXDLY, AUTOK_MSDC0_DAT0TXDLY);
			sdr_set_field(EMMC50_PAD_DAT01_TUNE,
				      MSDC_EMMC50_PAD_DAT1_TXDLY, AUTOK_MSDC0_DAT1TXDLY);
			sdr_set_field(EMMC50_PAD_DAT23_TUNE,
				      MSDC_EMMC50_PAD_DAT2_TXDLY, AUTOK_MSDC0_DAT2TXDLY);
			sdr_set_field(EMMC50_PAD_DAT23_TUNE,
				      MSDC_EMMC50_PAD_DAT3_TXDLY, AUTOK_MSDC0_DAT3TXDLY);
			sdr_set_field(EMMC50_PAD_DAT45_TUNE,
				      MSDC_EMMC50_PAD_DAT4_TXDLY, AUTOK_MSDC0_DAT4TXDLY);
			sdr_set_field(EMMC50_PAD_DAT45_TUNE,
				      MSDC_EMMC50_PAD_DAT5_TXDLY, AUTOK_MSDC0_DAT5TXDLY);
			sdr_set_field(EMMC50_PAD_DAT67_TUNE,
				      MSDC_EMMC50_PAD_DAT6_TXDLY, AUTOK_MSDC0_DAT6TXDLY);
			sdr_set_field(EMMC50_PAD_DAT67_TUNE,
				      MSDC_EMMC50_PAD_DAT7_TXDLY, AUTOK_MSDC0_DAT7TXDLY);
		}
	} else if (host->id == 1) {
		sdr_set_field(MSDC_IOCON, MSDC_IOCON_DDR50CKD, AUTOK_MSDC_DDRCKD);
		if (ios->timing == MMC_TIMING_UHS_SDR104) {
			sdr_set_field(MSDC_PAD_TUNE0,
				      MSDC_PAD_TUNE0_CLKTXDLY, AUTOK_MSDC1_CLK_SDR104_TX_VALUE);
		} else {
			sdr_set_field(MSDC_PAD_TUNE0,
				      MSDC_PAD_TUNE0_CLKTXDLY, AUTOK_MSDC1_CLK_TX_VALUE);
		}
	} else if (host->id == 2) {
		sdr_set_field(MSDC_IOCON, MSDC_IOCON_DDR50CKD, AUTOK_MSDC_DDRCKD);
		sdr_set_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CLKTXDLY, AUTOK_MSDC2_CLK_TX_VALUE);
	}
}
				 ...
				 enum ERROR_TYPE error_type)
{
	...
	if (host->id == 0) {
		switch (error_type) {
		case CMD_ERROR:
			sdr_get_field(MSDC_IOCON, MSDC_IOCON_RSPL, orig_resp_edge);
			sdr_set_field(MSDC_IOCON, MSDC_IOCON_RSPL, orig_resp_edge ^ 0x1);
			sdr_get_field(MSDC_IOCON, MSDC_IOCON_RSPL, cur_resp_edge);
		...
		case DATA_ERROR:
			...
			if (ios->timing == MMC_TIMING_MMC_DDR52) {
				...
				sdr_get_field(MSDC_IOCON, MSDC_IOCON_R_D_SMPL, orig_read_edge);
				sdr_set_field(MSDC_IOCON,
					      MSDC_IOCON_R_D_SMPL, orig_read_edge ^ 0x1);
				sdr_set_field(MSDC_PATCH_BIT0, MSDC_PB0_RD_DAT_SEL, 0);
				sdr_get_field(MSDC_IOCON, MSDC_IOCON_R_D_SMPL, cur_read_edge);
				sdr_get_field(MSDC_PATCH_BIT0,
			...
			sdr_get_field(MSDC_IOCON, MSDC_IOCON_R_D_SMPL, orig_read_edge);
			sdr_set_field(MSDC_PATCH_BIT0, MSDC_PB0_RD_DAT_SEL, 0);
			sdr_set_field(MSDC_IOCON, MSDC_IOCON_R_D_SMPL, orig_read_edge ^ 0x1);
			sdr_get_field(MSDC_IOCON, MSDC_IOCON_R_D_SMPL, cur_read_edge);
			AUTOK_RAWPRINT
			    ("[AUTOK][read err]PB0[3]_INVALID pre_edge = %d cur_edge = %d\r\n",
			     orig_read_edge, cur_read_edge);
#endif
			break;
		case CRC_STATUS_ERROR:
			sdr_get_field(MSDC_PATCH_BIT2, MSDC_PB2_CFGCRCSTSEDGE, orig_crc_fifo_edge);
			sdr_set_field(MSDC_PATCH_BIT2,
				      MSDC_PB2_CFGCRCSTSEDGE, orig_crc_fifo_edge ^ 0x1);
			sdr_get_field(MSDC_PATCH_BIT2, MSDC_PB2_CFGCRCSTSEDGE, cur_crc_fifo_edge);
			AUTOK_RAWPRINT
			    ("[AUTOK][write err]orig_fifo_edge = %d cur_fifo_edge = %d\r\n",
...
struct AUTOK_TX_RES sdio_tx_online_select(unsigned int tx_raw_scan, unsigned int cycle_value,
					  enum TUNE_TX_TYPE tx_type)
{
	...
	for (i = 0; i < tx_bond.fail_bd_cnt; i++) {
		AUTOK_RAWPRINT("[ATUOK TX]pre fail bd: S-%d E-%d\r\n",
			       tx_bond.fail_bd_info[i].start, tx_bond.fail_bd_info[i].end);
	}
...
struct AUTOK_TX_RES autok_online_tuning_data_TX(struct msdc_host *host, unsigned int opcode,
						unsigned int cycle_value)
{
	int ret = 0;
	void __iomem *base = host->base;
	unsigned int response = 0;
	unsigned int tune_tx_value = 0;
	unsigned char tune_cnt = 0;
	unsigned char i = 0;
	unsigned char tune_crc_cnt[32] = {0};
	unsigned char tune_pass_cnt[32] = {0};
	unsigned char tune_tmo_cnt[32] = {0};
	char tune_result[33] = {0};
	unsigned int dat_tx[8] = {0};
	unsigned int dat_tx_result = 0;
	struct AUTOK_TX_RES dat_tx_sel = {0};
	/* store tx setting */
	sdr_get_field(EMMC50_PAD_DAT01_TUNE, MSDC_EMMC50_PAD_DAT0_TXDLY, dat_tx[0]);
	sdr_get_field(EMMC50_PAD_DAT01_TUNE, MSDC_EMMC50_PAD_DAT1_TXDLY, dat_tx[1]);
	sdr_get_field(EMMC50_PAD_DAT23_TUNE, MSDC_EMMC50_PAD_DAT2_TXDLY, dat_tx[2]);
	sdr_get_field(EMMC50_PAD_DAT23_TUNE, MSDC_EMMC50_PAD_DAT3_TXDLY, dat_tx[3]);
	sdr_get_field(EMMC50_PAD_DAT45_TUNE, MSDC_EMMC50_PAD_DAT4_TXDLY, dat_tx[4]);
	sdr_get_field(EMMC50_PAD_DAT45_TUNE, MSDC_EMMC50_PAD_DAT5_TXDLY, dat_tx[5]);
	sdr_get_field(EMMC50_PAD_DAT67_TUNE, MSDC_EMMC50_PAD_DAT6_TXDLY, dat_tx[6]);
	sdr_get_field(EMMC50_PAD_DAT67_TUNE, MSDC_EMMC50_PAD_DAT7_TXDLY, dat_tx[7]);
			...
			if (opcode == MMC_WRITE_BLOCK) {
				...
				while (((response >> 9) & 0xF) != 4) {
					...
					if ((ret & (E_RESULT_RSP_CRC | E_RESULT_CMD_TMO)) != 0) {
						AUTOK_RAWPRINT
						    ("[AUTOK]------tune data TX cmd13 error------\r\n");
						AUTOK_RAWPRINT
	...
	if (dat_tx_sel.tx_err != 0) {
		...
		sdr_set_field(EMMC50_PAD_DAT01_TUNE, MSDC_EMMC50_PAD_DAT0_TXDLY, dat_tx[0]);
		sdr_set_field(EMMC50_PAD_DAT01_TUNE, MSDC_EMMC50_PAD_DAT1_TXDLY, dat_tx[1]);
		sdr_set_field(EMMC50_PAD_DAT23_TUNE, MSDC_EMMC50_PAD_DAT2_TXDLY, dat_tx[2]);
		sdr_set_field(EMMC50_PAD_DAT23_TUNE, MSDC_EMMC50_PAD_DAT3_TXDLY, dat_tx[3]);
		sdr_set_field(EMMC50_PAD_DAT45_TUNE, MSDC_EMMC50_PAD_DAT4_TXDLY, dat_tx[4]);
		sdr_set_field(EMMC50_PAD_DAT45_TUNE, MSDC_EMMC50_PAD_DAT5_TXDLY, dat_tx[5]);
		sdr_set_field(EMMC50_PAD_DAT67_TUNE, MSDC_EMMC50_PAD_DAT6_TXDLY, dat_tx[6]);
		sdr_set_field(EMMC50_PAD_DAT67_TUNE, MSDC_EMMC50_PAD_DAT7_TXDLY, dat_tx[7]);
	}
end:
	return dat_tx_sel;
}
...
struct AUTOK_TX_PARA autok_offline_tuning_TX(struct msdc_host *host, unsigned int value)
{
	...
	unsigned char tune_pass_cnt[32] = {0};
	unsigned char tune_tmo_cnt[32] = {0};
	unsigned int cmd_tx_result = 0;
	struct AUTOK_TX_RES cmd_tx_sel = {0};
	struct AUTOK_TX_RES dat_tx_sel = {0};
...
/* sdr_set_field(SDC_FIFO_CFG, SDC_FIFO_CFG_WR_VALID_SEL, 0); */
/* sdr_set_field(SDC_FIFO_CFG, SDC_FIFO_CFG_RD_VALID_SEL, 0); */
...
int hs200_execute_tuning(struct msdc_host *host, u8 *res)
{
	...
	/* AUTOK_RAWPRINT("[AUTOK][HS200]=========ckgen_debug:%d ========\r\n", ckgen_debug); */

	/* sdr_set_field(MSDC_PATCH_BIT1, 0xf << 8, 0); */

	/* sdr_set_field(MSDC_PATCH_BIT2, 0xf << 20, 5); */
