...
#ifndef CONFIG_OF
static void __iomem *u_msdc_base[HOST_MAX_NUM] = { MSDC0_BASE, MSDC1_BASE };
#endif
static struct msdc_hw *p_msdc_hw[HOST_MAX_NUM] = { NULL, NULL };
...
static unsigned int simp_msdc_ldo_power(unsigned int on, unsigned int  powerId, u32 powerVolt)
{
...
static unsigned int simp_mmc_power_up(struct simp_mmc_host *host, bool on)
{
	...
	case 0:
		simp_msdc_ldo_power(on, 2, VOL_3000);
		break;
	case 1:
		simp_msdc_ldo_power(on, 1, VOL_3000);
		simp_msdc_ldo_power(on, 0, VOL_3000);
		break;
	default:
		break;
	}
...
{
	/* step1: turn on cg */
	sdr_set_field((msdc_topckgen_base + 0x24), 0x1 << 17, 1); /* msdc0 */
	sdr_set_field((msdc_topckgen_base + 0x24), 0x1 << 18, 1); /* msdc1 */

	/* step2: set mux */
	sdr_set_field((msdc_topckgen_base + 0), 0x7 << 11, 2); /* msdc0 */
	sdr_set_field((msdc_topckgen_base + 0), 0x7 << 20, 2); /* msdc1 */

	/* step3: enbale mainpll clock */
	sdr_set_field((msdc_topckgen_base + 0x30), 0x1 << 0, 1); /* mainpll_D2 */
	sdr_set_field((msdc_topckgen_base + 0x30), 0x1 << 8, 1); /* mainpll_D4 */
	sdr_set_field((msdc_topckgen_base + 0x30), 0x1 << 16, 1); /* mainpll_D8 */

	/* step4: turn off cg */
	sdr_set_field((msdc_topckgen_base + 0x24), 0x1 << 17, 0); /* msdc0 */
	sdr_set_field((msdc_topckgen_base + 0x24), 0x1 << 18, 0); /* msdc1 */

#if MTK_MMC_DUMP_DBG
	pr_err("[%s]: msdc cg[0x10000024][bit18 for msdc1, bit17 for msdc0]=0x%x, msdc mux[0x10000000][bit20~22 for msdc1, bit11~13 for msdc0]=0x%x\n",
				 __func__, sdr_read32(msdc_topckgen_base+0x24), sdr_read32(msdc_topckgen_base));
#endif
...
static unsigned int simp_msdc_init(struct simp_mmc_host *mmc_host)
{
	...
	case 0:
		 ...
		 */
		sdr_set_field(MSDC0_SELGP_CLR, 0xFFFFFFFF, 0x7FF);
		sdr_set_field(MSDC0_GPIO_MODE5_MWR_ADDR,
			MSDC0_DAT4_PINMUX_BITS | MSDC0_DAT3_PINMUX_BITS | MSDC0_DAT2_PINMUX_BITS |
			MSDC0_DAT1_PINMUX_BITS |
		...
		sdr_set_field(MSDC0_GPIO_MODE6_MWR_ADDR,
			MSDC0_RST_PINMUX_BITS | MSDC0_DAT7_PINMUX_BITS | MSDC0_DAT6_PINMUX_BITS |
			MSDC0_DAT5_PINMUX_BITS,
			0x9999);

		sdr_set_field(MSDC0_DRVING0_BASE, MSDC0_DAT7_DRVING, p_msdc_hw[0]->dat_drv);
		sdr_set_field(MSDC0_DRVING0_BASE, MSDC0_DAT6_DRVING, p_msdc_hw[0]->dat_drv);
		sdr_set_field(MSDC0_DRVING0_BASE, MSDC0_DAT5_DRVING, p_msdc_hw[0]->dat_drv);
		sdr_set_field(MSDC0_DRVING0_BASE, MSDC0_DAT4_DRVING, p_msdc_hw[0]->dat_drv);

		sdr_set_field(MSDC0_DRVING1_BASE, MSDC0_DAT3_DRVING, p_msdc_hw[0]->dat_drv);
		sdr_set_field(MSDC0_DRVING1_BASE, MSDC0_DAT2_DRVING, p_msdc_hw[0]->dat_drv);
		sdr_set_field(MSDC0_DRVING1_BASE, MSDC0_DAT1_DRVING, p_msdc_hw[0]->dat_drv);
		sdr_set_field(MSDC0_DRVING1_BASE, MSDC0_DAT0_DRVING, p_msdc_hw[0]->dat_drv);
		sdr_set_field(MSDC0_DRVING1_BASE, MSDC0_CLK_DRVING, p_msdc_hw[0]->clk_drv);
		sdr_set_field(MSDC0_DRVING1_BASE, MSDC0_RST_DRVING, p_msdc_hw[0]->rst_drv);
		sdr_set_field(MSDC0_DRVING1_BASE, MSDC0_CMD_DRVING, p_msdc_hw[0]->cmd_drv);
		break;
	...
	case 1:
		...
		sdr_set_field(MSDC1_DRVING_BASE, MSDC1_CLK_DRVING, p_msdc_hw[1]->clk_drv);
		sdr_set_field(MSDC1_DRVING_BASE, MSDC1_CMD_DRVING, p_msdc_hw[1]->cmd_drv);
		sdr_set_field(MSDC1_DRVING_BASE, MSDC1_DAT3_DRVING, p_msdc_hw[1]->dat_drv);
		sdr_set_field(MSDC1_DRVING_BASE, MSDC1_DAT2_DRVING, p_msdc_hw[1]->dat_drv);
		sdr_set_field(MSDC1_DRVING_BASE, MSDC1_DAT1_DRVING, p_msdc_hw[1]->dat_drv);
		sdr_set_field(MSDC1_DRVING_BASE, MSDC1_DAT0_DRVING, p_msdc_hw[1]->dat_drv);
		break;
