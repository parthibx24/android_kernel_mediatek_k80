...
/* defaults */
#define TDM_MAX_BUFFER_SIZE     (48*1024)
#define TDM_MIN_PERIOD_SIZE     64
#define TDM_MAX_PERIOD_SIZE     TDM_MAX_BUFFER_SIZE
...
static struct snd_pcm_hardware mtk_capture_hardware = {
	...
	.buffer_bytes_max = TDM_MAX_BUFFER_SIZE,
	.period_bytes_max = TDM_MAX_PERIOD_SIZE,
	.periods_min = 1,
	.periods_max = 4096,
	.fifo_size = 0,
};
				...
				void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (DMA_Read_Ptr + read_size < Vul_Block->u4BufferSize) {
		...
		if (copy_to_user((void __user *)Read_Data_Ptr,
			...
			pr_err
			("%s, Fail 1 Read_Data_Ptr:%p,pucVirtBufAddr:%p,DMAReadIdx:0x%x,DMA_Read_Ptr:%zu,read_size:%zu",
			__func__, Read_Data_Ptr, Vul_Block->pucVirtBufAddr, Vul_Block->u4DMAReadIdx, DMA_Read_Ptr,
			read_size);
			return 0;
		}
