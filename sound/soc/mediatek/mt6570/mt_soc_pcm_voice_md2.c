...
static int mtk_voice_md2_close(struct snd_pcm_substream *substream)
{
	...
	SetModemPcmEnable(MODEM_2, false);
...
static int mtk_voice1_ext_prepare(struct snd_pcm_substream *substream)
{
	...
	EnableAfe(true);
	Voice2IntPcm.mPcmModeWidebandSel = (runtimeStream->rate == 8000) ?
	...
	SetModemPcmConfig(MODEM_2, Voice2IntPcm);
...
static int mtk_voice_md2_probe(struct platform_device *pdev)
{
	...
	return snd_soc_register_platform(&pdev->dev, &mtk_soc_voice_md2_platform);
}
