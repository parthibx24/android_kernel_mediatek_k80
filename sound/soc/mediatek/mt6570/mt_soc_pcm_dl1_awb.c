...
static snd_pcm_uframes_t mtk_dl1_awb_pcm_pointer(struct snd_pcm_substream
						 *substream)
{
	uint32_t Frameidx = 0;
				...
				void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (Awb_Block->u4DataRemained > Awb_Block->u4BufferSize) {
		pr_warn("mtk_dl1_awb_pcm_copy u4DataRemained=%x > u4BufferSize=%x",
...
static struct page *mtk_dl1_capture_pcm_page(struct snd_pcm_substream
